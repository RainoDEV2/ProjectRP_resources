...
function closeBanking() {
    $.post("https://prp-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#pinCode').val('');
                    $.post("https://prp-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("https://prp-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("https://prp-atms/removeCard", JSON.stringify({
    ...
    $(document).on('click','[data-action=ATMwithdraw]',function(){
        ...
        if(amount !== undefined && amount !== null && amount !== 0) {
            $.post("https://prp-atms/doATMWithdraw", JSON.stringify({
