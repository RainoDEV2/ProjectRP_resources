...
RegisterNetEvent('prp-atms:client:updateBankInformation', function(banking)
    SendNUIMessage({
...
RegisterNetEvent('prp-atms:client:loadATM', function(cards)
    if cards ~= nil and cards[1] ~= nil then
        ...
        ProjectRP.Functions.Notify("Please visit a branch to order a card", "error")
    end
end)
...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('prp-atms:server:doAccountWithdraw', data)
    end
end)

RegisterNUICallback("loadBankingAccount", function(data, cb)
    ProjectRP.Functions.TriggerCallback('prp-atms:server:loadBankAccount', function(banking)
...
RegisterNUICallback("removeCard", function(data, cb)
    ProjectRP.Functions.TriggerCallback('prp-debitcard:server:deleteCard', function(hasDeleted)
        if hasDeleted then
            SetNuiFocus(false, false)
            SendNUIMessage({
                status = "closeATM"
            })
            ProjectRP.Functions.Notify('Card has deleted.', 'success')
        else
            ProjectRP.Functions.Notify('Failed to delete card.', 'error')
        end
    end, data)
end)