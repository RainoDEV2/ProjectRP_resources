...
AddEventHandler('ProjectRP:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent('prp-vehicleshop:server:addPlayer', citizenid, gameTime)
    TriggerServerEvent('prp-vehicleshop:server:checkFinance')
end)
...
RegisterNetEvent('ProjectRP:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('prp-vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'prp-vehicleshop:client:showVehOptions'
        }
    }
}
...
local financeMenu = {
    {
        ...
        params = {
            event = 'prp-vehicleshop:client:getVehicles'
        }
    }
}
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'prp-vehicleshop:client:TestDriveReturn'
        }
    }
}
...
RegisterNetEvent('prp-vehicleshop:client:TestDrive', function()
        ...
        ProjectRP.Functions.SpawnVehicle(Config.Shops[getShopInsideOf()]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            SetTimeout(Config.Shops[closestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    testDriveVeh = 0
                    inTestDrive = false
                    ProjectRP.Functions.DeleteVehicle(veh)
...
RegisterNetEvent('prp-vehicleshop:client:TestDriveReturn', function()
    local ped = PlayerPedId()
    ...
    if veh == testDriveVeh then
        ...
        ProjectRP.Functions.Notify('This is not your test drive vehicle', 'error')
    end
end)

RegisterNetEvent('prp-vehicleshop:client:vehCategories', function()
    local categoryMenu = {
        {
            header = '< Go Back',
            params = {
                event = 'prp-vehicleshop:client:homeMenu'
            }
        }
    }
    for k,v in pairs(Config.Shops[getShopInsideOf()]['Categories']) do
        categoryMenu[#categoryMenu + 1] = {
            header = v,
            params = {
                event = 'prp-vehicleshop:client:openVehCats',
                args = {
                    catName = k
                }
            }
    ...
    exports['prp-menu']:openMenu(categoryMenu)
end)

RegisterNetEvent('prp-vehicleshop:client:openVehCats', function(data)
    local vehicleMenu = {
        {
            header = '< Go Back',
            params = {
    ...
    for k,v in pairs(ProjectRP.Shared.Vehicles) do
        if ProjectRP.Shared.Vehicles[k]["category"] == data.catName and ProjectRP.Shared.Vehicles[k]["shop"] == getShopInsideOf() then
            vehicleMenu[#vehicleMenu + 1] = {
                ...
                params = {
                    isServer = true,
                    event = 'prp-vehicleshop:server:swapVehicle',
                    args = {
                        toVehicle = v.model,
...
RegisterNetEvent('prp-vehicleshop:client:openFinance', function(data)
    ...
    if dialog then
        ...
        TriggerServerEvent('prp-vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
    end
end)
        ...
        inputs = {
            {
                ...
                isRequired = true,
                name = 'downPayment',
                text = 'Down Payment Amount - Min '..MinimumDown..'%'
            },
            {
                type = 'number',
                isRequired = true,
...
RegisterNetEvent('prp-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    ProjectRP.Functions.SpawnVehicle(vehicle, function(veh)
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        exports['prp-fuel']:SetFuel(veh, 100)
...
RegisterNetEvent('prp-vehicleshop:client:getVehicles', function()
    ProjectRP.Functions.TriggerCallback('prp-vehicleshop:server:getVehicles', function(vehicles)
        ...
        for k,v in pairs(vehicles) do
            if v.balance ~= 0 then
                ...
                ownedVehicles[#ownedVehicles + 1] = {
                    ...
                    txt = 'Plate: ' ..plate,
                    params = {
                        event = 'prp-vehicleshop:client:getVehicleFinance',
                        args = {
                            vehiclePlate = plate,
                            balance = v.balance,
        ...
        exports['prp-menu']:openMenu(ownedVehicles)
    end)
end)

RegisterNetEvent('prp-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        {
            header = '< Go Back',
            params = {
        ...
        },
        {
            ...
            params = {
                event = 'prp-vehicleshop:client:financePayment',
                args = {
                    vehData = data,
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = 'prp-vehicleshop:server:financePaymentFull',
                args = {
    ...
    exports['prp-menu']:openMenu(vehFinance)
end)

RegisterNetEvent('prp-vehicleshop:client:financePayment', function(data)
    local dialog = exports['prp-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('prp-vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
    end
end)
