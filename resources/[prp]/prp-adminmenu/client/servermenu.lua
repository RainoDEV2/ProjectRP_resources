...
ServerMenuButton1:On("select",function()
    ...
    for k, v in ipairs(elements) do
        local WeatherButton = WeatherMenu:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            ProjectRP.Functions.Notify(Lang:t("weather.weather_changed", {value = selection.label}))
        end})
    end
end)
...
ServerMenuButton2:On("select", function(item, value)
    ...
    ProjectRP.Functions.Notify(Lang:t("time.changed", {time = value}))
end)
...
local function OpenDealerMenu(dealer)
    local EditDealer = MenuV:CreateMenu(false, Lang:t("menu.edit_dealer") .. dealer["name"], menuLocation, r, g, b, menuSize, 'projectrp', 'menuv')
...
ServerMenuButton3:On('Select', function(item)
    ...
    ProjectRP.Functions.TriggerCallback('prp-adminmenu:callback:getdealers', function(dealers)
