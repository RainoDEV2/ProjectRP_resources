    ...
    local BanMenuButton3 = BanMenu:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                ...
                ProjectRP.Functions.Notify(Lang:t("error.invalid_reason_length_ban"), 'error')
            end
        end
    })
end
    ...
    local KickMenuButton2 = KickMenu:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                ...
                ProjectRP.Functions.Notify(Lang:t("error.missing_reason"), 'error')
            end
        end
    })
end
    ...
    local PermsMenuButton2 = PermsMenu:AddButton({
        ...
        select = function(btn)
            if selectedgroup ~= 'Unknown' then
                ...
                ProjectRP.Functions.Notify(Lang:t("error.changed_perm_failed"), 'error')
            end
        end
    })
end
    ...
    local GiveItemMenuButton3 = GiveItemMenu:AddButton({
        ...
        select = function(btn)
            if itemname ~= 'Unknown' and itemamount ~= 0 then
                TriggerServerEvent('ProjectRP:CallCommand', "giveitem", {player.id, itemname, itemamount})
                itemname = 'Unknown'
                itemamount = 0
            else
                ProjectRP.Functions.Notify(Lang:t("error.give_item"), 'error')
            end
        end
    })
end
...
local function OpenSoundMenu(player)
    ...
    local SoundMenuButton1 = SoundMenu:AddButton({
        ...
        select = function(btn)
            TriggerServerEvent('prp-admin:server:getsounds')
        end
    })
    ...
    local SoundMenuButton5 = SoundMenu:AddButton({
        ...
        select = function(btn)
            if soundname ~= 'Unknown' and soundvolume ~= 0 and soundrange ~= 0 then
                TriggerServerEvent('prp-admin:server:playsound', player.id, soundname, soundvolume, soundrange)
            else
                ProjectRP.Functions.Notify(Lang:t("error.give_item"), 'error')
            end
        end
        ...
        for k, v in ipairs(elements) do
            local PlayerGeneralButton = PlayerGeneralMenu:AddButton({
                ...
                select = function(btn)
                    ...
                    TriggerServerEvent('prp-admin:server:'..values, PlayerDetails)
                end
            })
        end    
    end)
            ...
            local PlayerExtraButton = PlayerExtraMenu:AddButton({
                ...
                select = function(btn)
                    ...
                    if values == 'inventory' then
                        TriggerEvent('prp-admin:client:inventory', PlayerDetails.id)
