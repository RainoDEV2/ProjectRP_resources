...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords3' then
        ...
        ProjectRP.Functions.Notify(Lang:t("success.coords_copied"), "success")
    ...
    elseif dataType == 'heading' then
        ...
        ProjectRP.Functions.Notify(Lang:t("success.heading_copied"), "success")
    end
end
...
DevMenuButton6:On('change', function(item, newValue, oldValue)
    dev = not dev
    TriggerEvent('prp-admin:client:ToggleDevmode')
    if dev then
        SetPlayerInvincible(PlayerId(), true)
        while dev do
            TriggerServerEvent("ProjectRP:Server:SetMetaData", "hunger", ProjectRP.Functions.GetPlayerData().metadata["hunger"] + 10)
            TriggerServerEvent("ProjectRP:Server:SetMetaData", "thirst", ProjectRP.Functions.GetPlayerData().metadata["thirst"] + 10)    
            Wait(20000)
        end
