...
RegisterNetEvent('prp-admin:client:inventory', function(targetPed)
...
RegisterNetEvent('prp-admin:client:SendStaffChat', function(name, msg)
...
RegisterNetEvent('prp-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if ProjectRP.Shared.Vehicles[vehname] ~= nil and next(ProjectRP.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('prp-admin:server:SaveCar', props, ProjectRP.Shared.Vehicles[vehname], `veh`, plate)
        else
            ProjectRP.Functions.Notify('You cant store this vehicle in your garage..', 'error')
        end
    else
...
RegisterNetEvent('prp-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        ProjectRP.Functions.Notify('+'..ammo..' Ammo for the '..ProjectRP.Shared.Weapons[GetHashKey(weapon)]["label"], 'success')
    else
        local weapon = GetSelectedPedWeapon(ped)
        if weapon ~= nil then
            SetPedAmmo(ped, weapon, ammo)
            ProjectRP.Functions.Notify('+'..ammo..' Ammo for the '..ProjectRP.Shared.Weapons[weapon]["label"], 'success')
        else
            ProjectRP.Functions.Notify('You dont have a weapon in your hands..', 'error')
        end
    end
end)
