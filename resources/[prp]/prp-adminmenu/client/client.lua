...
menu_button11:On("select",function()
    ...
    for k,v in ipairs(elements) do
        local menu_button14 = menu6:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('prp-weathersync:server:setWeather', selection.value)
            ProjectRP.Functions.Notify('Weather Changed To: '..selection.label)
        end})
    end
end)
...
local function CopyToClipboard(dataType)
    ...
    if dataType == 'coords3' then
        ...
        ProjectRP.Functions.Notify("Coordinates copied to clipboard!", "success")
    ...
    elseif dataType == 'heading' then
        ...
        ProjectRP.Functions.Notify("Heading copied to clipboard!", "success")
    end
end
...
local function OpenCarModelsMenu(category)
    ...
    for k, v in pairs(category) do
        local menu_button10 = menu14:AddButton({
             ...
             select = function(btn)
                 TriggerServerEvent('ProjectRP:CallCommand', "car", { k })
             end
        })
    end
end
...
menu12_button2:On('Select', function(item)
    TriggerServerEvent('ProjectRP:CallCommand', "fix", {})
end)

menu12_button3:On('Select', function(item)
    TriggerServerEvent('ProjectRP:CallCommand', "admincar", {})
end)

menu12_button4:On('Select', function(item)
    TriggerServerEvent('ProjectRP:CallCommand', "dv", {})
end)

names_button:On('change', function()
    TriggerEvent('prp-admin:client:toggleNames')
end)
blips_button:On('change', function()
    TriggerEvent('prp-admin:client:toggleBlips')
end)
    ...
    for k, v in ipairs(elements) do
        local menu_button10 = EditDealer:AddButton({
            ...
            select = function(btn)
                ...
                if values == "goto" then
                    TriggerServerEvent('ProjectRP:CallCommand', "dealergoto", { dealer["name"] })
                elseif values == "remove" then
                    TriggerServerEvent('ProjectRP:CallCommand', "deletedealer", { dealer["name"] })
                    EditDealer:Close()
                    menu7:Close()
                end
            end
...
menu_button4:On('Select', function(item)
    menu7:ClearItems()
    ProjectRP.Functions.TriggerCallback('test:getdealers', function(dealers)
...
local function OpenPermsMenu(permsply)
    ProjectRP.Functions.TriggerCallback('prp-admin:server:getrank', function(rank)
        if rank then
            ...
            local menu_button21 = menu10:AddButton({
                ...
                select = function(btn)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('prp-admin:server:setPermissions', permsply.id, selectedgroup)
			            ProjectRP.Functions.Notify('Authority group changed!', 'success')
                        selectedgroup = 'Unknown'
                    else
                        ProjectRP.Functions.Notify('Choose a group!', 'error')
                    end
                end
            })
        else
    ...
    local menu_button18 = menu9:AddButton({
        ...
        select = function(btn)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('prp-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
                ProjectRP.Functions.Notify('You must give a reason!', 'error')
            end
        end
    })
end
    ...
    local menu_button17 = menu8:AddButton({
        ...
        select = function(btn)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('prp-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
                ProjectRP.Functions.Notify('You must give a Reason and set a Length for the ban!', 'error')
            end
        end
    })
end
        ...
        local menu_button10 = Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('prp-admin:server:'..values, player)
...
menu_button2:On('select', function(item)
    menu4:ClearItems()
    ProjectRP.Functions.TriggerCallback('test:getplayers', function(players)
...
menu_button13:On("select", function(item, value)
    TriggerServerEvent("prp-weathersync:server:setTime", value, value)
    ProjectRP.Functions.Notify("Time changed to " .. value .. " hs 00 min")
