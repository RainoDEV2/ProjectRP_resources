...
local function IsWhitelisted(CitizenId)
    ...
    local Perms = ProjectRP.Functions.GetPermission(Player.PlayerData.source)
...
RegisterNetEvent('prp-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('prp-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('prp-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
...
RegisterNetEvent('prp-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('prp-lapraces:client:StartRaceEditor', source, RaceName)
        else
            TriggerClientEvent('ProjectRP:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'You have not been authorized to race\'s to create.', 'error')
    end
end)

RegisterNetEvent('prp-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('ProjectRP:Notify', src, 'You were the only one in the race, the race had ended', 'error')
            TriggerClientEvent('prp-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('prp-lapraces:client:JoinRace', src, Races[RaceId], RaceData.Laps)
    TriggerClientEvent('prp-phone:client:UpdateLapraces', -1)
    local creatorsource = ProjectRP.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData
                              .source
...
RegisterNetEvent('prp-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('ProjectRP:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('prp-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('prp-lapraces:server:SetupRace', function(RaceId, Laps)
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('prp-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
                ...
                TriggerClientEvent('ProjectRP:Notify', source, 'The race is already running', 'error')
            end
        else
            TriggerClientEvent('ProjectRP:Notify', source, 'The race is already running', 'error')
        end
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'This race does not exist :(', 'error')
    end
end)

RegisterNetEvent('prp-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('prp-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('prp-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('prp-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = ProjectRP.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('prp-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('prp-phone:client:UpdateLapraces', -1)
        else
            TriggerClientEvent('ProjectRP:Notify', src, 'You are not the creator of the race..', 'error')
        end
    else
...
RegisterNetEvent('prp-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
...
ProjectRP.Functions.CreateCallback('prp-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(ProjectRP.Functions.GetPlayer(source).PlayerData.citizenid))
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(ProjectRP.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:CanRaceSetup', function(source, cb)
    cb(Config.RaceSetupAllowed)
end)

ProjectRP.Functions.CreateCallback('prp-lapraces:server:GetTrackData', function(source, cb, RaceId)
...
ProjectRP.Commands.Add("cancelrace", "Cancel going race..", {}, false, function(source, args)
    local Player = ProjectRP.Functions.GetPlayer(source)
        ...
        if RaceName ~= nil then
            ...
            if Races[RaceId].Started then
                ...
                for cid, _ in pairs(Races[RaceId].Racers) do
                    local RacerData = ProjectRP.Functions.GetPlayerByCitizenId(cid)
                    if RacerData ~= nil then
                        TriggerClientEvent('prp-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                    end
                end
                table.remove(AvailableRaces, AvailableKey)
                Races[RaceId].LastLeaderboard = {}
                ...
                TriggerClientEvent('ProjectRP:Notify', source, 'This race has not started yet.', 'error')
            end
        end
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'You have not been authorized to do this.', 'error')
    end
end)

ProjectRP.Commands.Add("togglesetup", "Turn on / off racing setup", {}, false, function(source, args)
    local Player = ProjectRP.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('ProjectRP:Notify', source, 'No more races can be created!', 'error')
        else
            TriggerClientEvent('ProjectRP:Notify', source, 'Races can be created again!', 'success')
        end
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'You have not been authorized to do this.', 'error')
    end
