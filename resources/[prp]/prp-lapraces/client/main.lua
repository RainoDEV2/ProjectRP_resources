...
local function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            ProjectRP.Functions.Notify('You cant go to fast', 'error')
        end
    else
        ProjectRP.Functions.Notify('You cant go too fast', 'error')
    end
end
...
local function SaveRace()
    ...
    ProjectRP.Functions.Notify('Race: '..CreatorData.RaceName..' is saved!', 'success')
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        ProjectRP.Functions.Notify('You have not placed any checkpoints yet..', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        ProjectRP.Functions.Notify('You must have at least 10 checkpoints', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        ProjectRP.Functions.Notify('You can not go higher than 15')
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        ProjectRP.Functions.Notify('You cannot go lower than 2')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    ProjectRP.Functions.Notify('Press [9] again to confirm', 'error', 5000)
                else
                    for id, CheckpointData in pairs(CreatorData.Checkpoints) do
                        if CheckpointData.blip ~= nil then
                            RemoveBlip(CheckpointData.blip)
                    ...
                    CreatorData.RaceName = nil
                    CreatorData.Checkpoints = {}
                    ProjectRP.Functions.Notify('Race-editor canceled!', 'error')
...
local function FinishRace()
    ...
    if CurrentRaceData.BestLap ~= 0 then
        ProjectRP.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        ProjectRP.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('prp-lapraces:client:StartRaceEditor', function(RaceName)
...
RegisterNetEvent('prp-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = RaceData.Racers
    end
end)

RegisterNetEvent('prp-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        ...
        ProjectRP.Functions.Notify('Youre already in a race..', 'error')
    end
end)

RegisterNetEvent('prp-lapraces:client:LeaveRace', function(data)
    ProjectRP.Functions.Notify('You have completed the race!')
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
            CurrentRaceData.Checkpoints[k].blip = nil
...
RegisterNetEvent('prp-lapraces:client:RaceCountdown', function()
    ...
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    ProjectRP.Functions.Notify('The race will start in 10 seconds', 'error', 2500)
        ...
        ProjectRP.Functions.Notify('You are not currently in a race..', 'error')
    end
end)

RegisterNetEvent('prp-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            ProjectRP.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is finished on spot: '..Place, 'error', 3500)
        end
    end
end)

RegisterNetEvent('prp-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            ProjectRP.Functions.Notify('Go back to the start or you will be kicked from the race: '..ToFarCountdown..'s', 'error', 500)
                        else
