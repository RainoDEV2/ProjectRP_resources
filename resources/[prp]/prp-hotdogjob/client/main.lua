...
function StartWorking()
    ProjectRP.Functions.TriggerCallback('prp-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            local PlayerPed = PlayerPedId()
            ...
            UpdateUI()
            CheckLoop()
            ProjectRP.Functions.Notify('You have $250 deposit paid!..', 'success')
        else
            ProjectRP.Functions.Notify('You have no money for the deposit..', 'error')
        end
    end)
end
...
function ToggleSell()
    ...
    if StandObject ~= nil then
        if dist < 5.0 then
            ...
            ProjectRP.Functions.Notify('You are to far from your Hot Dog Stand..', 'error')
        end
    else
        ProjectRP.Functions.Notify('You do not have a hotdog stand', 'error')
    end
end
...
function SellToPed(ped)
    ...
    while pedDist > OffsetData.Distance do
	    ...
	    SellingData.Hotdog = nil
            ProjectRP.Functions.Notify('You are too far from your stall ...', 'error')
            break
        end
        Citizen.Wait(100)
    end
    ...
    while pedDist < OffsetData.Distance and SellingData.HasTarget do
        ...
        if PlayerDist < 4 then
            if SellingData.Hotdog ~= nil then
                ...
                ProjectRP.Functions.DrawText3D(pedCoords.x, pedCoords.y, pedCoords.z, '[7] Sale '..HotdogsForSale..'x in front of $'..(HotdogsForSale * SellingPrice)..' / [8] Reject')
                if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then
                    ProjectRP.Functions.Notify(HotdogsForSale..'x Hotdog(\'s) sold in front of $'..(HotdogsForSale * SellingPrice), 'success')
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    ProjectRP.Functions.Notify('Customer Refused!', 'error')
                    SellingData.HasTarget = false
...
function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        ...
        if Config.MyLevel == 1 then
            ProjectRP.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hot Dog Made!')
            ...
            if Luck == LuckyNumber then
                ProjectRP.Functions.Notify('You have '..LuckyAmount..' '..Config.Stock[Quality].Label..' Hotdog\'s made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + LuckyAmount
            else
                ProjectRP.Functions.Notify('You have a '..Config.Stock[Quality].Label..' Hotdog made!')
                Config.Stock[Quality].Current = Config.Stock[Quality].Current + 1
            end
        end
    else
        ProjectRP.Functions.Notify('You have no ('..Config.Stock[Quality].Label..') more about this in front of council..')
    end
    PreparingFood = false
end
...
function StopWorking()
    if DoesEntityExist(StandObject) then
        ProjectRP.Functions.TriggerCallback('prp-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                ...
                ProjectRP.Functions.Notify('You have a $250 deposit returned!', 'success')
            else
                ProjectRP.Functions.Notify('Something is my friend..', 'error')
            end
        end)
    else
        ProjectRP.Functions.Notify('Your hot dog stand was nowhere to be seen, You will not receive your deposit back!', 'error')
        IsWorking = false
        StandObject = nil
        IsPushing = false
        IsSelling = false
...
AddEventHandler('prp-hotdogjob:staff:DeletStand', function()
    ...
    if Object ~= nil then
        ...
        if ObjectDistance <= 5 then
            ...
            Citizen.Wait(100)
            DeleteEntity(Object)
            ProjectRP.Functions.Notify('Hotdogstand mode removed!')
        end
    end
end)
...
RegisterNetEvent('prp-hotdog:client:hotdogshop')
AddEventHandler('prp-hotdog:client:hotdogshop', function()
    local ShopItems = {}
    ShopItems.label = "Hotdog Shop"
    ShopItems.items = Config.hotdogitems
    ShopItems.slots = #Config.hotdogitems
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "hotdog_"..math.random(1, 99), ShopItems)
end)