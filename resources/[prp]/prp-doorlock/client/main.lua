	...
	if success then
		...
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("ProjectRP:Server:RemoveItem", "advancedlockpick", 1, false)
				TriggerEvent('inventory:client:ItemBox', ProjectRP.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("ProjectRP:Server:RemoveItem", "lockpick", 1, false)
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		ProjectRP.Functions.TriggerCallback('prp-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function()
	ProjectRP.Functions.TriggerCallback('prp-doorlock:server:setupDoors', function(result)
...
RegisterNetEvent('prp-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('prp-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	local doorData = {}
		...
		inputs = {
			...
			},
			{
				...
				options = {
					...
					{ value = "pickable", text = "Lockpickable" }
				}
			}
		}
	})
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		if not model or model == 0 then ProjectRP.Functions.Notify("Did not receive a model hash, if the door is transparent, make sure you aim at the frame of the door", 'error') canContinue = true return end
		result = DoorSystemFindExistingDoor(coords.x, coords.y, coords.z, model)
		if result then ProjectRP.Functions.Notify("This door is already registered", 'error') canContinue = true return end
		...
		TriggerServerEvent('prp-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		if not model[1] or model[1] == 0 or not model[2] or model[2] == 0 then ProjectRP.Functions.Notify("Did not receive a model hash, if the door is transparent, make sure you aim at the frame of the door", 'error') return end
		if entity[1] == entity[2] then ProjectRP.Functions.Notify("Both doors can't be the same entity", 'error') canContinue = true return end
		doorData.doorHash = {}
		for i = 1, 2 do
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('prp-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('prp-doorlock:client:newDoorAdded', function(data, id)
	Config.DoorList[id] = data
	TriggerEvent('prp-doorlock:client:setState', PlayerData.source, id, data.locked, false, true, true)
end)
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('prp-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end)
