...
ProjectRP.Functions.CreateCallback('prp-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

ProjectRP.Functions.CreateCallback('prp-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('prp-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('prp-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		TriggerClientEvent('prp-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('prp-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	...
	local label = "\n\n-- "..data.doorname.." ".. "created by" .." "..Player.PlayerData.name.."\nConfig.DoorList['"..data.doorname.."'] = {"
	file:write(label)
	...
	TriggerClientEvent('prp-doorlock:client:newDoorAdded', -1, configData, data.doorname)
end)

-- Commands

ProjectRP.Commands.Add('newdoor', "Add a new door to the doorlock system", {}, false, function(source)
	TriggerClientEvent('prp-doorlock:client:addNewDoor', source)
