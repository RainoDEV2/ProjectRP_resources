	...
	while true do
		if startVineyard then
			if tasking then
				...
				TriggerEvent("prp-vineyard:client:startVineyard")
...
function pickgrapes()
	...
	if success then
		TriggerServerEvent("prp-vineyard:server:getGrapes")
		tasking = false
		DeleteBlip()
	end
end
...
function pickProcess()
    ProjectRP.Functions.Progressbar("pick_grape", "Picking Grapes ..", math.random(6000,8000), false, true, {
...
RegisterNetEvent('prp-vineyard:client:grapeJuice')
AddEventHandler('prp-vineyard:client:grapeJuice', function()
	PrepareAnim()
	grapeJuiceProcess()
end)

RegisterNetEvent('prp-vineyard:client:loadIngredients')
AddEventHandler('prp-vineyard:client:loadIngredients', function()
...
function grapeJuiceProcess()
    ProjectRP.Functions.Progressbar("grape_juice", "Processing Grapes ..", math.random(15000,20000), false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerServerEvent("prp-vineyard:server:receiveGrapeJuice")
        ClearPedTasks(PlayerPedId())
