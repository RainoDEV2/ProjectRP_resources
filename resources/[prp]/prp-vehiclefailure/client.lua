...
RegisterNetEvent('prp-vehiclefailure:client:RepairVehicle')
AddEventHandler('prp-vehiclefailure:client:RepairVehicle', function()
...
function CleanVehicle(vehicle)
	...
	ProjectRP.Functions.Progressbar("cleaning_vehicle", "Cleaning the car...", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
	}, {}, {}, {}, function() -- Done
		ProjectRP.Functions.Notify("Vehicle cleaned!")
		SetVehicleDirtLevel(vehicle, 0.1)
        SetVehicleUndriveable(vehicle, false)
		WashDecalsFromVehicle(vehicle, 1.0)
		TriggerServerEvent('prp-vehiclefailure:server:removewashingkit', vehicle)
		TriggerEvent('inventory:client:ItemBox', ProjectRP.Shared.Items["cleaningkit"], "remove")
...
RegisterNetEvent('prp-vehiclefailure:client:SyncWash')
AddEventHandler('prp-vehiclefailure:client:SyncWash', function(veh)
	SetVehicleDirtLevel(veh, 0.1)
	SetVehicleUndriveable(veh, false)
	WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('prp-vehiclefailure:client:CleanVehicle')
AddEventHandler('prp-vehiclefailure:client:CleanVehicle', function()
	local vehicle = ProjectRP.Functions.GetClosestVehicle()
...
RegisterNetEvent('prp-vehiclefailure:client:RepairVehicleFull')
AddEventHandler('prp-vehiclefailure:client:RepairVehicleFull', function()
	local vehicle = ProjectRP.Functions.GetClosestVehicle()
	if vehicle ~= nil and vehicle ~= 0 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
function RepairVehicleFull(vehicle)
	...
	ProjectRP.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(20000, 30000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		ProjectRP.Functions.Notify("Vehicle repaired!")
...
function RepairVehicle(vehicle)
	...
	ProjectRP.Functions.Progressbar("repair_vehicle", "Repairing vehicle..", math.random(10000, 20000), false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
...
Citizen.CreateThread(function()
    while true do
        ...
        if showPro then
            ProjectRP.Functions.DrawText3D(pos.x, pos.y, pos.z, TimeLeft .. '~g~%')
        end
    end
end)
...
-- 								--exports['prp-vehicletuning']:SetVehicleStatus(ProjectRP.Functions.GetPlate(vehicle), "brakes", exports['prp-vehicletuning']:GetVehicleStatus(ProjectRP.Functions.GetPlate(vehicle), "brakes") - 0.01)
-- 							end
