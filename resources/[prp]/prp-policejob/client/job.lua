...
function TakeOutVehicle(vehicleInfo)
    ...
    ProjectRP.Functions.SpawnVehicle(vehicleInfo, function(veh)
        ...
        if vehicleInfo == 'npolvic' then
            ...
            SetVehicleLivery(veh, 2)
            SetVehicleMod(veh, 1, 3) -- Front bumper
            SetVehicleMod(veh, 27, 1)
            SetVehicleMod(veh, 28, 1)
            SetVehicleMod(veh, 29, 1)
            SetVehicleMod(veh, 30, 1)
            SetVehicleMod(veh, 31, 1)
            SetVehicleMod(veh, 32, 2) -- Back seats
        end
        exports['prp-fuel']:SetFuel(veh, 100.0)
        closeMenuFull()
        TaskWarpPedIntoVehicle(PlayerPedId(), veh, -1)
        TriggerEvent("vehiclekeys:client:SetOwner", ProjectRP.Functions.GetPlate(veh))
        TriggerServerEvent("inventory:server:addTrunkItems", ProjectRP.Functions.GetPlate(veh), Config.CarItems)
        SetVehicleEngineOn(veh, true, true)
    end, coords, true)
end
...
local function IsArmoryWhitelist() -- being removed
    ...
    if ProjectRP.Functions.GetPlayerData().job.name == 'police' then
        retval = true
    end
    return retval
end
...
function MenuImpound()
    ...
    ProjectRP.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        local shouldContinue = false
        if result == nil then
            ProjectRP.Functions.Notify("There are no impounded vehicles", "error", 5000)
        else
            shouldContinue = true
        ...
        if shouldContinue then
            ...
            exports['prp-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound, price)
    ...
    local totalFuel = exports['prp-fuel']:GetFuel(vehicle)
    ...
    ProjectRP.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
            ...
            if player ~= -1 and distance < 5.0 then
                ...
                ProjectRP.Functions.TriggerCallback('police:GetPlayerStatus', function(result)
                    if result then
                        for k, v in pairs(result) do
                            ProjectRP.Functions.Notify(''..v..'')
                        end
                    end
                end, playerId)
            else
                ...
                if #(pos - v) < 4.5 then
                    if onDuty then
                        ...
                        if #(pos - v) < 1.5 then
                            ...
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent("inventory:server:OpenInventory", "stash", "policestash_"..ProjectRP.Functions.GetPlayerData().citizenid)
                                TriggerEvent("inventory:client:SetCurrentStash", "policestash_"..ProjectRP.Functions.GetPlayerData().citizenid)
                            end
                        elseif #(pos - v) < 2.5 then
...
RegisterNetEvent('prp-police:client:SpawnSpikeStrip')
AddEventHandler('prp-police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        if PlayerJob.name == "police" and PlayerJob.onduty then
            ...
            TriggerServerEvent('ProjectRP:Server:RemoveItem', 'spikestrip', 1)
        end
    else
        ProjectRP.Functions.Notify('There are no spikestrips left..', 'error')
    end
end)
