...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time)
    if time > 0 and StatusList[statusId] then
        if (CurrentStatusList == nil or CurrentStatusList[statusId] == nil) or
            ...
            ProjectRP.Functions.Notify(''..CurrentStatusList[statusId].text..'', 'error')
        end
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    local blooddropList = {}
    ProjectRP.Functions.Progressbar('clear_blooddrops', 'Clearing blood..', 5000, false, true, {
        ...
        if Blooddrops and next(Blooddrops) then
            ...
            ProjectRP.Functions.Notify('Blood cleared :)')
        end
    end, function() -- Cancel
        ProjectRP.Functions.Notify('Blood not cleared..', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    ProjectRP.Functions.Progressbar('clear_casings', 'Removing bullet sleeves..', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        if Casings and next(Casings) then
            ...
            ProjectRP.Functions.Notify('Bullet sleeves removed :)')
        end
    end, function() -- Cancel
        ProjectRP.Functions.Notify('Bullet sleeves not removed', 'error')
    end)
end)
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                ...
                if IsControlJustReleased(0, 47) then
                    ...
                    local info = {
                        ...
                        ammolabel = Config.AmmoLabels[ProjectRP.Shared.Weapons[Casings[CurrentCasing].type]['ammotype']],
...
--                     if ProjectRP.Shared.Weapons[currentWeapon]['ammotype'] ~= 'AMMO_PISTOL' then
--                         automatic = true
--                     end
