...
RegisterNetEvent("prp-policejob:client:UseCamera", function()
    if not active then
        active = true
                    ...
                    while active and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == vehicle) and true do
                        ...
                        elseif IsControlJustPressed(1, 176) then
                            if not takethapicbro then
                                takethapicbro = true
                                ProjectRP.Functions.TriggerCallback("prp-policejob:evidence-camera:WebHook", function(hook)
                                    if hook then
                                        ...
                                        ProjectRP.Functions.Notify('Photo taken', 'success', 900)
                                        Wait(1350)
                                        ProjectRP.Functions.Notify('Uploading to the Cloud', 'success', 1100)
                                        Wait(1850)
                                        ProjectRP.Functions.Notify('Photo uploaded!', 'success', 1400)
                                    else
                                        ProjectRP.Functions.Notify('Contact Server Dev\'s about webhook', 'error', 7500)
                                    end
                                end)
                            end
                        end
