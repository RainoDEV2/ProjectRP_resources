...
ProjectRP.Commands.Add("spikestrip", "Place Spike Strip (Police Only)", {}, false, function(source)
    local src = source
...
ProjectRP.Commands.Add("grantlicense", "Grant a license to someone", {{name = "id", help = "ID of a person"}, {name = "license", help = "License Type"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "hunting" or args[2] == "fishing" then
            ...
            if SearchedPlayer then
                ...
                TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, "You have been granted a license",
                    "success", 5000)
                TriggerClientEvent('ProjectRP:Notify', src, "You granted a license", "success", 5000)
            end
        else
            TriggerClientEvent('ProjectRP:Notify', src, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, "You must be a Sergeant to grant licenses!", "error")
    end
end)

ProjectRP.Commands.Add("revokelicense", "Revoke a license from someone", {{name = "id", help = "ID of a person"}, {name = "license", help = "License Type"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 2 then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "hunting" or args[2] == "fishing" then
            ...
            if SearchedPlayer then
                ...
                SearchedPlayer.Functions.SetMetaData("licences", licenseTable)
                TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, "You've had a license revoked",
                    "error", 5000)
                TriggerClientEvent('ProjectRP:Notify', src, "You revoked a license", "success", 5000)
            end
        else
            TriggerClientEvent('ProjectRP:Notify', src, "Invalid license type", "error")
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, "You must be a Sergeant to revoke licenses!", "error")
    end
end)

ProjectRP.Commands.Add("pobject", "Place/Delete An Object (Police Only)", {{name = "type",help = "Type object you want or 'delete' to delete"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
...
ProjectRP.Commands.Add("cuff", "Cuff Player (Police Only)", {}, false, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("escort", "Escort Player", {}, false, function(source, args)
    local src = source
    TriggerClientEvent("police:client:EscortPlayer", src)
end)

ProjectRP.Commands.Add("callsign", "Give Yourself A Callsign", {{name = "name", help = "Name of your callsign"}}, false, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    Player.Functions.SetMetaData("callsign", table.concat(args, " "))
end)

ProjectRP.Functions.CreateUseableItem("spikestrip", function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent("prp-police:client:SpawnSpikeStrip", source)
    end
end)

ProjectRP.Commands.Add("clearcasings", "Clear Area of Casings (Police Only)", {}, false, function(source)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("jail", "Jail Player (Police Only)", {{name = "id", help = "Player ID"}, {name = "time", help = "Time they have to be in jail"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        if time > 0 then
            ...
            TriggerClientEvent('ProjectRP:Notify', src, 'Cannot sentence for 0', 'error')
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, 'For on-duty police only', 'error')
    end
...
ProjectRP.Commands.Add("clearblood", "Clear The Area of Blood (Police Only)", {}, false, function(source)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("seizecash", "Seize Cash (Police Only)", {}, false, function(source)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("sc", "Soft Cuff (Police Only)", {}, false, function(source)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("cam", "View Security Camera (Police Only)", {{name = "camid", help = "Camera ID"}}, false, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("flagplate", "Flag A Plate (Police Only)", {{name = "plate", help = "License"}, {name = "reason", help = "Reason of flagging the vehicle"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        local reason = {}
        ...
        TriggerClientEvent('ProjectRP:Notify', src, "Vehicle (" .. args[1]:upper() .. ") is flagged for: " .. table.concat(reason, " "))
    else
        TriggerClientEvent('ProjectRP:Notify', src, 'For on-duty police only', 'error')
    end
end)

ProjectRP.Commands.Add("unflagplate", "Unflag A Plate (Police Only)", {{name = "plate", help = "License plate"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                ...
                TriggerClientEvent('ProjectRP:Notify', src, "Vehicle (" .. args[1]:upper() .. ") is unflagged")
            else
                TriggerClientEvent('ProjectRP:Notify', src, 'Vehicle not flagged', 'error')
            end
        else
            TriggerClientEvent('ProjectRP:Notify', src, 'Vehicle not flagged', 'error')
        end
...
ProjectRP.Commands.Add("plateinfo", "Run A Plate (Police Only)", {{name = "plate",help = "License plate"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if Plates and Plates[args[1]:upper()] then
            if Plates[args[1]:upper()].isflagged then
                TriggerClientEvent('ProjectRP:Notify', src, 'Vehicle ' .. args[1]:upper() .. ' has been flagged for: ' .. Plates[args[1]:upper()].reason)
            else
                TriggerClientEvent('ProjectRP:Notify', src, 'Vehicle not flagged', 'error')
            end
        else
    ...
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                ...
                TriggerClientEvent('ProjectRP:Notify', OtherPlayer.PlayerData.source, 'You were paid $500', 'success')
                TriggerClientEvent('ProjectRP:Notify', src, 'You paid the tow truck driver')
            else
                TriggerClientEvent('ProjectRP:Notify', src, 'Not a tow truck driver', 'error')
            end
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, 'For on-duty police only', 'error')
...
ProjectRP.Commands.Add("paylawyer", "Pay Lawyer (Police, Judge Only)", {{name = "id",help = "ID of the player"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "lawyer" then
                ...
                TriggerClientEvent('ProjectRP:Notify', OtherPlayer.PlayerData.source, 'You were paid $500', 'success')
                TriggerClientEvent('ProjectRP:Notify', src, 'You paid a lawyer')
            else
                TriggerClientEvent('ProjectRP:Notify', src, 'Person is not a lawyer', "error")
            end
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, 'For on-duty police only', 'error')
...
ProjectRP.Commands.Add("anklet", "Attach Tracking Anklet (Police Only)", {}, false, function(source)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("ankletlocation", "Get the location of a persons anklet", {{"cid", "Citizen ID of the person"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if args[1] then
            local citizenid = args[1]
            local Target = ProjectRP.Functions.GetPlayerByCitizenId(citizenid)
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('ProjectRP:Notify', src, 'This person doesn\'t have an anklet on.', 'error')
                end
            end
        end
    else
...
ProjectRP.Commands.Add("removeanklet", "Remove Tracking Anklet (Police Only)", {{"cid", "Citizen ID of person"}}, true,function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        if args[1] then
            ...
            if Target then
                if Target.PlayerData.metadata["tracker"] then
                    ...
                    TriggerClientEvent('ProjectRP:Notify', src, 'This person does not have an anklet', 'error')
                end
            end
        end
    else
...
ProjectRP.Commands.Add("takedrivinglicense", "Seize Drivers License (Police Only)", {}, false, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
...
ProjectRP.Commands.Add("takedna", "Take a DNA sanple from a person (empty evidence bag needed) (Police Only)", {{"id", "ID of the person"}}, true, function(source, args)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    local OtherPlayer = ProjectRP.Functions.GetPlayer(tonumber(args[1]))
    if ((Player.PlayerData.job.name == "police") and Player.PlayerData.job.onduty) and OtherPlayer then
        if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
            ...
            if Player.Functions.AddItem("filled_evidence_bag", 1, false, info) then
                TriggerClientEvent("inventory:client:ItemBox", src, ProjectRP.Shared.Items["filled_evidence_bag"], "add")
            end
        else
            TriggerClientEvent('ProjectRP:Notify', src, "You must have an empty evidence bag with you", "error")
        end
    end
end)

RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    local Target = ProjectRP.Functions.GetPlayer(source)
...
ProjectRP.Commands.Add('911p', 'Police Report', {{name='message', help='Message to be sent'}}, false, function(source, args)
	local src = source
...
ProjectRP.Functions.CreateUseableItem("handcuffs", function(source, item)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
...
ProjectRP.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(item.name) then
...
ProjectRP.Functions.CreateCallback('police:server:isPlayerDead', function(source, cb, playerId)
    local Player = ProjectRP.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

ProjectRP.Functions.CreateCallback('police:GetPlayerStatus', function(source, cb, playerId)
    local Player = ProjectRP.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
ProjectRP.Functions.CreateCallback('police:IsSilencedWeapon', function(source, cb, weapon)
    local Player = ProjectRP.Functions.GetPlayer(source)
    local itemInfo = Player.Functions.GetItemByName(ProjectRP.Shared.Weapons[weapon]["name"])
    local retval = false
    if itemInfo then
...
ProjectRP.Functions.CreateCallback('police:GetDutyPlayers', function(source, cb)
    local dutyPlayers = {}
    local players = ProjectRP.Functions.GetPRPPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
ProjectRP.Functions.CreateCallback('police:GetImpoundedVehicles', function(source, cb)
    local vehicles = {}
...
ProjectRP.Functions.CreateCallback('police:IsPlateFlagged', function(source, cb, plate)
    local retval = false
...
ProjectRP.Functions.CreateCallback('police:GetCops', function(source, cb)
    local amount = 0
    local players = ProjectRP.Functions.GetPRPPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.name == "police" and v.PlayerData.job.onduty then
...
ProjectRP.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    local retval = false
    local players = ProjectRP.Functions.GetPRPPlayers()
    for k, v in pairs(players) do
...
RegisterNetEvent('police:server:TakeOutImpound', function(plate)
    ...
    TriggerClientEvent('ProjectRP:Notify', src, "Vehicle unimpounded!", 'success')
end)

RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    local CuffedPlayer = ProjectRP.Functions.GetPlayer(playerId)
...
AddEventHandler('police:server:SendPoliceEmergencyAlert', function(streetLabel, coords, callsign)
    local alertData = {
        title = "Colleague Requesting Backup",
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(source)
    local EscortPlayer = ProjectRP.Functions.GetPlayer(playerId)
...
RegisterNetEvent('police:server:JailPlayer', function(playerId, time)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer then
            ...
            TriggerClientEvent('ProjectRP:Notify', src, "You sent the person to prison for " .. time .. " months")
        end
    end
end)
...
RegisterNetEvent('police:server:SearchPlayer', function(playerId)
    local src = source
    local SearchedPlayer = ProjectRP.Functions.GetPlayer(playerId)
    if SearchedPlayer then
        TriggerClientEvent('ProjectRP:Notify', src, 'Found $'..SearchedPlayer.PlayerData.money["cash"]..' on the civilian')
        TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, "You are being searched")
    end
end)
...
RegisterNetEvent('police:server:SeizeCash', function(playerId)
    ...
    if SearchedPlayer then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, ProjectRP.Shared.Items["moneybag"], "add")
        TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, 'Your cash was confiscated')
    end
end)
...
RegisterNetEvent('police:server:SeizeDriverLicense', function(playerId)
    ...
    if SearchedPlayer then
        ...
        if driverLicense then
            ...
            TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, 'Your driving license has been confiscated')
        else
            TriggerClientEvent('ProjectRP:Notify', src, 'No drivers license', 'error')
        end
    end
end)
...
RegisterNetEvent('police:server:RobPlayer', function(playerId)
    ...
    if SearchedPlayer then
        ...
        TriggerClientEvent('ProjectRP:Notify', SearchedPlayer.PlayerData.source, "You have been robbed of $" .. money)
        TriggerClientEvent('ProjectRP:Notify', Player.PlayerData.source, "You have stolen $" .. money)
    end
end)
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            ...
            TriggerClientEvent('ProjectRP:Notify', src, "Vehicle taken into depot for $" .. price .. "!")
        else
            exports.oxmysql:execute(
                'UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                {2, body, engine, fuel, plate})
            TriggerClientEvent('ProjectRP:Notify', src, "Vehicle seized")
        end
    end
end)
...
RegisterNetEvent('evidence:server:CreateCasing', function(weapon, coords)
    ...
    local weaponInfo = ProjectRP.Shared.Weapons[weapon]
...
RegisterNetEvent('police:server:SetTracker', function(targetId)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('ProjectRP:Notify', targetId, 'Your anklet is taken off.', 'error', 5000)
        TriggerClientEvent('ProjectRP:Notify', src, 'You took off an ankle bracelet from ' .. Target.PlayerData.charinfo.firstname .. " " .. Target.PlayerData.charinfo.lastname, 'error', 5000)
        TriggerClientEvent('police:client:SetTracker', targetId, false)
    else
        Target.Functions.SetMetaData("tracker", true)
        TriggerClientEvent('ProjectRP:Notify', targetId, 'You put on an ankle strap.', 'error', 5000)
        TriggerClientEvent('ProjectRP:Notify', src, 'You put on an ankle strap to ' .. Target.PlayerData.charinfo.firstname .. " " .. Target.PlayerData.charinfo.lastname, 'error', 5000)
...
RegisterNetEvent('police:server:GiveWeaponLicense', function()
	local src = source
	local Player = ProjectRP.Functions.GetPlayer(src)
	if Player then
        local licenseTable = Player.PlayerData.metadata["licences"]
        if licenseTable["weapon"] == true then
            TriggerClientEvent('ProjectRP:Notify', Player.PlayerData.source, "You already have a weapons license", "error", 5000)
        else
            licenseTable["weapon"] = true
            Player.Functions.SetMetaData("licences", licenseTable)
            TriggerClientEvent('ProjectRP:Notify', Player.PlayerData.source, "You have been granted a license", "success", 5000)
        end
	end
end)
