...
local function saveVehicle()
    ...
    local myCar = ProjectRP.Functions.GetVehicleProperties(veh)
...
AddEventHandler('lscustoms:SetLivery', function(arg)
    ...
    if PlayerData.job.name == "mechanic" or PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" then
...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("prp-customs:attemptPurchase", type, upgradeLevel)
                        ...
                        if not isPlyInBennys then
                            ...
                            if IsControlJustReleased(1, 38) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if (v.useJob and isAuthorized((ProjectRP.Functions.GetPlayerData().job.name), k)) or not v.useJob then
                                        TriggerEvent('event:control:bennys', k)
                                    else
                                        ProjectRP.Functions.Notify("You are not authorized", "error")
                                    end
                                end
                            end
                        else
...
--#[Event Handlers]#--
RegisterNetEvent("prp-customs:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
    ProjectRP.Functions.Notify("Purchase Successful")
end)

RegisterNetEvent("prp-customs:purchaseFailed", function()
    isPurchaseSuccessful = false
    attemptingPurchase = false
    ProjectRP.Functions.Notify("Not enough money", "error")
end)
