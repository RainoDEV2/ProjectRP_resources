...
-- exports['prp-target']:AddTargetModel(Config.ATMModels, {
-- 	options = {
-- 		{
-- 			type = "client",
...
RegisterNUICallback('close', function(data, cb)
	...
	ProjectRP.Functions.Notify(NotifyMessage, NotifyType, 5000)
			...
			for k, v in pairs(Config.ATMModels) do
				...
				if nearATM then
					...
					if not beenHacked then
						...
						ProjectRP.Functions.Notify('Already hacked!', 'error', 3000)
					end
				else
					if not hackStarted and k == #Config.ATMModels then
						ProjectRP.Functions.Notify('You cannot do that here.', 'error', 3000)
					end
				end
			end
		else
			ProjectRP.Functions.Notify('You are missing something!', 'error', 3000)
		end
