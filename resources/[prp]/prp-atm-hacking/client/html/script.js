...
var prpgreen = '#20bb44', prpred = '#c10114'; //values from prp-notify css
var pool = [];
var ended = false;
...
function gameTimer() {
	...
	if(ctime<=0) {
		stopTimer();
		document.getElementById('a' + attempt).style.borderColor = prpred;
		document.getElementById('timer').style.color = prpred;
		document.getElementById('a0').style.borderColor = prpred;
	}
}
...
function Guess() {
	...
	if(!pool.includes(g1)) {	
		...
		if(attempt > attempts && clue != '!!!!') {
			...
			document.getElementById('attempt').style.color = prpred;
			stopTimer();
			document.getElementById('a10').style.borderColor = prpred;
			document.getElementById('a0').style.borderColor = prpred;
		}
	}
}
...
function getClue() {
	...
	if(pMatch==4) {
		success = true;
		stopTimer();
		document.getElementById('a' + attempt).style.borderColor = prpgreen;
		document.getElementById('a0').style.borderColor = prpgreen;
	}
	return clue;
}
...
function stopHack() {
	if(success == true) {
		...
		document.getElementById('end').style.backgroundColor = prpgreen;
	} else {
		document.getElementById('a').innerHTML = "<td id='end' colspan='5' style='height:" + (46/attempts) + "vh;'>Hack Failed!</td>";
		document.getElementById('end').style.backgroundColor = prpred;
	}
}
