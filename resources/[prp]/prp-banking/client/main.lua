...
RegisterNetEvent('prp-banking:client:syncBanks')
AddEventHandler('prp-banking:client:syncBanks', function(data)
    banks = data
    if showing then
        showing = false
    end
...
function openAccountScreen()
    ProjectRP.Functions.TriggerCallback('prp-banking:getBankingInformation', function(banking)
        if banking ~= nil then
            InBank = true
...
function atmRefresh()
    ProjectRP.Functions.TriggerCallback('prp-banking:getBankingInformation', function(infor)
        InBank = true
        SetNuiFocus(true, true)
        SendNUIMessage({
            status = "refreshatm",
...
RegisterNetEvent('prp-banking:openBankScreen')
AddEventHandler('prp-banking:openBankScreen', function()
    openAccountScreen()
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        letSleep = true
        if playerLoaded and ProjectRP ~= nil and not InBank then 
...
RegisterNetEvent('prp-banking:transferError')
AddEventHandler('prp-banking:transferError', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('prp-banking:successAlert')
AddEventHandler('prp-banking:successAlert', function(msg)
    SendNUIMessage({
