...
RegisterServerEvent('prp-banking:server:modifyBank')
AddEventHandler('prp-banking:server:modifyBank', function(bank, k, v)
    if banks[tonumber(bank)] then
        banks[tonumber(bank)][k] = v
        TriggerClientEvent('prp-banking:client:syncBanks', -1, banks)
    end
end)

exports('modifyBank', function(bank, k, v)
    TriggerEvent('prp-banking:server:modifyBank', bank, k, v)
end)
...
RegisterServerEvent('prp-banking:createNewCard')
AddEventHandler('prp-banking:createNewCard', function()
    local src = source
    ...
    TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")**" .. " created new card")
end)

RegisterServerEvent('prp-base:itemUsed')
AddEventHandler('prp-base:itemUsed', function(_src, data)
    if data.item == "moneybag" then
        TriggerClientEvent('prp-banking:client:usedMoneyBag', _src, data)
    end
end)

RegisterServerEvent('prp-banking:server:unpackMoneyBag')
AddEventHandler('prp-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
        local xPlayer = ProjectRP.Functions.GetPlayer(_src)
...
RegisterServerEvent('prp-banking:initiateTransfer')
AddEventHandler('prp-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    local _startChar = ProjectRP.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then 
        ...
        if receiptName ~= false or receiptName ~= nil then 
            local userOnline = exports.prp-base:checkOnline(cid)
            
            if userOnline ~= false then
                -- User is online so we can do a straght transfer 
                local _targetUser = exports.prp-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('prp-banking:openBankScreen', _src)
                    TriggerClientEvent('prp-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('prp-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
ProjectRP.Functions.CreateCallback('prp-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
...
RegisterServerEvent('prp-banking:createBankCard')
AddEventHandler('prp-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('prp-banking:openBankScreen', src)
    TriggerClientEvent('ProjectRP:Notify', src, 'You have successfully ordered a Debit Card.', 'success')
    
    TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterServerEvent('prp-banking:doQuickDeposit')
AddEventHandler('prp-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('prp-banking:openBankScreen', src)
            TriggerClientEvent('prp-banking:successAlert', src, 'You made a cash deposit of $'..amount..' successfully.')
            TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterServerEvent('prp-banking:toggleCard')
AddEventHandler('prp-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
...
RegisterServerEvent('prp-banking:doQuickWithdraw')
AddEventHandler('prp-banking:doQuickWithdraw', function(amount, branch)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then 
            TriggerClientEvent('prp-banking:openBankScreen', src)
            TriggerClientEvent('prp-banking:successAlert', src, 'You made a cash withdrawal of $'..amount..' successfully.')
            TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterServerEvent('prp-banking:updatePin')
AddEventHandler('prp-banking:updatePin', function(pin)
    if pin ~= nil then 
        local src = source
        local xPlayer = ProjectRP.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        TriggerClientEvent('prp-banking:openBankScreen', src)
        TriggerClientEvent('prp-banking:successAlert', src, 'You have successfully updated your debit card pin.')
    end
end)

RegisterServerEvent('prp-banking:savingsDeposit')
AddEventHandler('prp-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('prp-banking:openBankScreen', src)
        TriggerClientEvent('prp-banking:successAlert', src, 'You made a savings deposit of $'..tostring(amount)..' successfully.')
        TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterServerEvent('prp-banking:savingsWithdraw')
AddEventHandler('prp-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('prp-banking:openBankScreen', src)
        TriggerClientEvent('prp-banking:successAlert', src, 'You made a savings withdrawal of $'..tostring(amount)..' successfully.')
        TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterServerEvent('prp-banking:createSavingsAccount')
AddEventHandler('prp-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = ProjectRP.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('prp-banking:openBankScreen', src)
    TriggerClientEvent('prp-banking:successAlert', src, 'You have successfully opened a savings account.')
    TriggerEvent('prp-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)
