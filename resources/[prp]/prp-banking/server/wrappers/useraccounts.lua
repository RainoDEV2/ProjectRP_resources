        ...
        if not self.cardActive then
            ...
            exports.oxmysql:execute('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('prp-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = ProjectRP.Functions.GetPlayer(self.source)
...
RegisterServerEvent('prp-banking:server:registerCurrentAccount')
AddEventHandler('prp-banking:server:registerCurrentAccount', function(cid)
...
RegisterServerEvent('prp-banking:server:registerSavingsAccount')
AddEventHandler('prp-banking:server:registerSavingsAccount', function(cid)
