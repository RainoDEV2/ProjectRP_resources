            ...
            if PlayerJob.name == "trucker" then
                if IsControlJustReleased(0, 178) then
                    if IsPedInAnyVehicle(PlayerPedId()) and isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                        getNewLocation()
                        CurrentPlate = ProjectRP.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId(), false))
                    end
                end
                            ...
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                                        ...
                                        ProjectRP.Functions.Notify('This is not a commercial vehicle!', 'error')
                                    end
                                else
                                    ProjectRP.Functions.Notify('You must be the driver to do this..')
                                end
                            else
                                MenuGarage()
                ...
                if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 4.5 then
				    ...
				    ClearAllBlipRoutes()
                                ...
                                ProjectRP.Functions.Notify("You haven't done any work yet..", "error")
                            end
                        end
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == ProjectRP.Functions.GetPlate(vehicle) then
                                    ...
                                    if #(pos - vector3(trunkpos.x, trunkpos.y, trunkpos.z)) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, 38) then
                                            isWorking = true
                                            ProjectRP.Functions.Progressbar("work_carrybox", "Take A Box Of Products", 2000, false, true, {
                            ...
                            elseif hasBox then
                                if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        ...
                                        ProjectRP.Functions.Progressbar("work_dropbox", "Deliver Box Of Products", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                ProjectRP.Functions.Notify("You Have Delivered All Products, To The Next Point")
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        ProjectRP.Functions.Notify("You Went To All The Shops .. Time For Your Payslip!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
	    ClearAllBlipRoutes()
            CurrentBlip = nil
