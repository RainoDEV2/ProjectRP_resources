...
RegisterNetEvent('prp-djbooth:server:playMusic', function(song, zoneName)
    local src = source
    ...
    TriggerClientEvent('prp-djbooth:client:playMusic', src)
end)

RegisterNetEvent('prp-djbooth:server:stopMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
...
RegisterNetEvent('prp-djbooth:server:pauseMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[data.zoneName].coords
...
RegisterNetEvent('prp-djbooth:server:resumeMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[data.zoneName].coords
...
RegisterNetEvent('prp-djbooth:server:changeVolume', function(volume, zoneName)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[zoneName].coords
