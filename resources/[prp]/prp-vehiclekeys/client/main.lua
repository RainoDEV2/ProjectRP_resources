...
function LockVehicle()
    ...
    local veh = ProjectRP.Functions.GetClosestVehicle(pos)
    ...
    if veh ~= nil and #(pos - vehpos) < 7.5 then
        ProjectRP.Functions.TriggerCallback('vehiclekeys:CheckHasKey', function(result)
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 2) then
                        ...
                        ProjectRP.Functions.Notify("Vehicle locked!")
                    else
                        ProjectRP.Functions.Notify("Something went wrong with the locking system!")
                    end
                else
                    Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 1) then
                        ...
                        Wait(200)
                        SetVehicleLights(veh, 0)
                        ProjectRP.Functions.Notify("Vehicle unlocked!")
                    else
                        ProjectRP.Functions.Notify("Something went wrong with the locking system!")
                    end
                end
            else
                ProjectRP.Functions.Notify('You don\'t have the keys of the vehicle..', 'error')
            end
        end, plate)
    end
end
...
function LockpickDoor(isAdvanced)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local vehicle = ProjectRP.Functions.GetClosestVehicle(pos)
...
function lockpickFinish(success)
    ...
    if success then
        ...
        ProjectRP.Functions.Notify('Opened Door!', 'success')
        SetVehicleDoorsLocked(vehicle, 1)
        lockpicked = true
        lockpickedPlate = ProjectRP.Functions.GetPlate(vehicle)
    else
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
    end
    ...
    if not HasKey then
        ...
        ProjectRP.Functions.Progressbar("hotwire_vehicle", "Engaging the ignition switch", hotwireTime, false, true, {
            ...
            if (math.random() <= Config.HotwireChance) then
                ...
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
                ProjectRP.Functions.Notify("Hotwire failed!", "error")
            end
            IsHotwiring = false
        end, function() -- Cancel
            StopAnimTask(ped, "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
        ...
        if math.random() <= chance then
            ...
            if closestPed ~= nil then
                ...
                if IsPedInAnyVehicle(ped) then
                    ...
                    if ProjectRP.Shared.Vehicles[modelName] ~= nil then
                        Name = ProjectRP.Shared.Vehicles[modelName]["brand"] .. ' ' .. ProjectRP.Shared.Vehicles[modelName]["name"]
                    else
                        Name = "Unknown"
                    end
                    local modelPlate = ProjectRP.Functions.GetPlate(vehicle)
...
function RobVehicle(target)
    ...
    ProjectRP.Functions.Progressbar("rob_keys", "Attempting Robbery..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random()
        if chance <= Config.RobberyChance then
            ...
            local plate = ProjectRP.Functions.GetPlate(GetVehiclePedIsIn(target, true))
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            TriggerEvent('vehiclekeys:client:SetOwner', plate)
            ProjectRP.Functions.Notify('You Got The Keys!', 'success')
            Wait(10000)
            IsRobbing = false
        else
            PoliceCall()
            ...
            TaskReactAndFleePed(target, PlayerPedId())
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            ProjectRP.Functions.Notify('They Called The Cops!', 'error')
            Wait(10000)
            IsRobbing = false
        end
    end)
            ...
            if GetPedInVehicleSeat(vehicle, -1) == ped then
                ...
                if usingAdvanced then
					...
					if success then
						...
						ProjectRP.Functions.Notify("Lockpicking succeeded!")
						HasKey = true
						TriggerEvent("vehiclekeys:client:SetOwner", GetVehicleNumberPlateText(vehicle))
						IsHotwiring = false
					else
						ProjectRP.Functions.Notify("Lockpicking failed!", "error")
					end
				else
...
RegisterNetEvent('vehiclekeys:client:SetOwner', function(plate)
    ...
    local CurrentVehPlate = ProjectRP.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId(), true))
...
RegisterNetEvent('vehiclekeys:client:GiveKeys', function(target)
    local plate = ProjectRP.Functions.GetPlate(GetVehiclePedIsIn(PlayerPedId(), true))
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if entering ~= 0 then
                sleep = 2000
                local plate = ProjectRP.Functions.GetPlate(entering)
                ProjectRP.Functions.TriggerCallback('vehiclekeys:CheckOwnership', function(result)
                    ...
                    if aiming and (target ~= nil and target ~= 0) then
                        if DoesEntityExist(target) and not IsEntityDead(target) and not IsPedAPlayer(target) then
                            if IsPedInAnyVehicle(target, false) then
                                if HasPedGotWeapon(PlayerPedId(), GetHashKey('WEAPON_MUSKET')) then 
