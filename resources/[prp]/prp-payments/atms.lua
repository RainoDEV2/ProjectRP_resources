...
-- This script is a simple replacement for PRP-Banking and PRP-ATMs
-- It uses PRP-Input and server callbacks to retreive info about accounts and cash
-- This requires PRP-Target and has the location of every ATM, ATM prop and bank window
...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function() createBlips() end)

local bossroles = {}
local gangroles = {}
Citizen.CreateThread(function()
	if Config.useATM or Config.useBanks then
		for k, v in pairs(ProjectRP.Shared.Jobs) do ---Grabs the list of jobs
			for l, b in pairs(ProjectRP.Shared.Jobs[tostring(k)].grades) do -- Grabs the list of grades
				if ProjectRP.Shared.Jobs[tostring(k)].grades[l].isboss == true then -- Checks the grade if is boss
		...
		for k, v in pairs(ProjectRP.Shared.Gangs) do 
			for l, b in pairs(ProjectRP.Shared.Gangs[tostring(k)].grades) do
				if tostring(k) ~= "none" then
					if ProjectRP.Shared.Gangs[tostring(k)].grades[l].isboss == true then
	...
	if Config.useATM then
		exports['prp-target']:AddTargetModel(Config.ATMModels, { options = { { event = "prp-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = "Use ATM", id = "atm" },}, distance = 1.5, })
		for k,v in pairs(Config.ATMLocations) do
			exports['prp-target']:AddCircleZone("jimatm"..k, vector3(tonumber(v.x), tonumber(v.y), tonumber(v.z)+0.2), 0.5, { name="jimatm"..k, debugPoly=false, useZ=true, }, 
			{ options = { { event = "prp-payments:Client:ATM:use", icon = "fas fa-money-check-alt", label = "Use ATM", id = "atm" },
						--[[{ event = "prp-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = "Transfer Money", id = "transfer" },]]
						}, distance = 1.5 })
		end
	end
	if Config.useBanks then
		for k,v in pairs(Config.BankLocations) do
			exports['prp-target']:AddCircleZone("jimbank"..k, vector3(tonumber(v.x), tonumber(v.y), tonumber(v.z)), 2.0, { name="jimbank"..k, debugPoly=false, useZ=true, }, 
			{ options = { { event = "prp-payments:Client:ATM:use", icon = "fas fa-piggy-bank", label = "Use Bank", id = "bank" },
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = "Transfer Money", id = "transfer" },
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-money-check-dollar", label = "Access Savings", id = "savings" },
							
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-building", label = "Access Society Account", id = "society", job = bossroles },
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = "Society Money Transfer", id = "societytransfer", job = bossroles },
							
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-building", label = "Gang Society Account", id = "gang", gang = gangroles },
							{ event = "prp-payments:Client:ATM:use", icon = "fas fa-arrow-right-arrow-left", label = "Gang Money Transfer", id = "gangtransfer", gang = gangroles }, }, distance = 2.5 })
		end
	end
end)
...
local function GrabAccount(type, job)
	local p = promise.new()
	ProjectRP.Functions.TriggerCallback('prp-payments:ManageWrapper', function(cb) p:resolve(cb) end, type, job)
	return Citizen.Await(p)
end

RegisterNetEvent('prp-payments:Client:ATM:use', function(data)
	--this grabs all the info from names to savings account numbers in the databases
	while name == nil do 
	ProjectRP.Functions.TriggerCallback('prp-payments:ATM:Find', function(cb1, cb2, cb3, cb4, cb5, cb6, cb7) name = cb1 cash = cb2 bank = cb3 account = cb4 cid = cb5 savbal = cb6 aid = cb7 end) 
		Citizen.Wait(100) 
	end
	...
	if data.id == "atm" then
		...
		for k, v in pairs(Config.ATMModels) do
			...
			if atm then
				...
				ProjectRP.Functions.Progressbar("accessing_atm", "Accessing ATM", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
				end, function()
	...
	elseif data.id == "bank" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
				      { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }
		PlayATMAnimation('enter')
		ProjectRP.Functions.Progressbar("accessing_atm", "Accessing Bank", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("ProjectRP:Notify", "Cancelled!", "error")
			return
		end)	
	...
	elseif data.id == "transfer" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
					  ...
					  { type = 'number', isRequired = true, name = 'amount', text = '💸 Amount to transfer' }, }
		PlayATMAnimation('enter')
		ProjectRP.Functions.Progressbar("accessing_atm", "Accessing Transfers", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("ProjectRP:Notify", "Cancelled!", "error")
			return
		end)
	...
	elseif data.id == "savings" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
					  { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }					
		PlayATMAnimation('enter')
		ProjectRP.Functions.Progressbar("accessing_atm", "Accessing Savings", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("ProjectRP:Notify", "Cancelled!", "error")
			return
		end)
	...
	elseif data.id == "society" then
		...
		setinputs = { { type = 'radio', name = 'billtype', text = setview, options = setoptions },
				      { type = 'number', isRequired = true, name = 'amount', text = '💵 Amount to transfer' }, }
		PlayATMAnimation('enter')
		ProjectRP.Functions.Progressbar("accessing_atm", "Accessing Society Account", atmbartime, false, true, { disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = false, }, {}, {}, {}, function() -- Done
		end, function()
			TriggerEvent("ProjectRP:Notify", "Cancelled!", "error")
			return
		end)
	...
	Wait(atmbartime+500)
	local dialog = exports['prp-input']:ShowInput({ header = setheader, txt = "test", submitText = "Transfer", inputs = setinputs })
	if dialog then
		if not dialog.amount then return end
		PlayATMAnimation('exit') Citizen.Wait(1000)
		TriggerServerEvent('prp-payments:server:ATM:use', dialog.amount, dialog.billtype, dialog.account, data.id, society, gsociety)
	end
	gsociety, society, name = nil
end)


RegisterNetEvent('prp-payments:client:ATM:give', function()
	local onlineList = {}
	local nearbyList = {}
	ProjectRP.Functions.TriggerCallback('prp-payments:MakePlayerList', function(cb) onlineList = cb if onlineList[1] == nil then Wait(200) end
		...
		local dialog = exports['prp-input']:ShowInput({ header = "Give someone cash", submitText = "Give",
		inputs = {
		...
		if dialog then
			...
			TriggerServerEvent('prp-payments:server:ATM:give', dialog.citizen, dialog.price)
		end
	end)
end)