...
local function SetupItems(shop)
    ...
    local playerJob = ProjectRP.Functions.GetPlayerData().job.name
    local items = {}
    for i = 1, #products do
...
RegisterNetEvent('prp-shops:client:UpdateShop', function(shop, itemData, amount)
    TriggerServerEvent('prp-shops:server:UpdateShopItems', shop, itemData, amount)
end)

RegisterNetEvent('prp-shops:client:SetShopItems', function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent('prp-shops:client:RestockShopItems', function(shop, amount)
                                    ...
                                    if hasLicense then
                                        ...
                                        for i = 1, #products do
                                            if not products[i].requiredJob then
                                                ...
                                                for i2 = 1, #products[i].requiredJob do
                                                    if ProjectRP.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] and not products[i].requiresLicense then
                                                        ShopItems.items[#ShopItems.items+1] = products[i]
                                                    end
                                                end
                                            end
                                        end
                                        ProjectRP.Functions.Notify("The dealer declines to show you firearms", "error")
                                        Wait(500)
                                        ProjectRP.Functions.Notify("Speak with law enforcement to get a firearms license", "error")
                                        Wait(1000)
                                    end
                                else
                                    ShopItems.items = SetupItems(shop)
