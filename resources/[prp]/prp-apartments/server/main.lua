...
RegisterNetEvent('prp-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
...
RegisterNetEvent('prp-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('ProjectRP:Notify', src, "You got a apartment ("..label..")")
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('ProjectRP:Notify', src, "You have changed apartments")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
ProjectRP.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
ProjectRP.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
ProjectRP.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
ProjectRP.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
ProjectRP.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = exports.oxmysql:executeSync('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
ProjectRP.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = ProjectRP.Functions.GetPlayer(src)
