...
local function EnterApartment(house, apartmentId, new)
    ...
    openHouseAnim()
    Citizen.Wait(250)
    ProjectRP.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            ProjectRP.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                TriggerServerEvent("ProjectRP:Server:SetMetaData", "currentapartment", CurrentApartment)
            end, house)
        else
            if offset > 230 then
                offset = 210
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("prp-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['prp-interior']:DespawnInterior(houseObj, function()
        ...
        DoScreenFadeIn(1000)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
        TriggerServerEvent("ProjectRP:Server:SetMetaData", "currentapartment", nil)
    end)
end
...
local function SetClosestApartment()
    ...
    if current ~= ClosestHouse and LocalPlayer.state['isLoggedIn'] and not InApartment then
        ...
        ProjectRP.Functions.TriggerCallback('apartments:IsOwner', function(result)
            IsOwned = result
        end, ClosestHouse)
    end
end
...
function OwnerList()
    ProjectRP.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        ped = PlayerPedId();
        MenuTitle = "Rang the door at: "
        ClearMenu()

        if next(apartments) == nil then
            ProjectRP.Functions.Notify("There is nobody home..", "error", 3500)
            closeMenuFull()
        else
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    ProjectRP.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
...
RegisterNetEvent('apartments:client:SpawnInApartment', function(apartmentId, apartment)
    ...
    if rangDoorbell ~= nil then
        ...
        if doorbelldist > 5 then
            ProjectRP.Functions.Notify("You are to far away from the Doorbell")
            return
        end
    end
...
RegisterNetEvent('prp-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)
...
RegisterNetEvent('apartments:client:RingDoor', function(player, house)
    ...
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "doorbell", 0.1)
    ProjectRP.Functions.Notify("Someone Is At The Door!")
end)
