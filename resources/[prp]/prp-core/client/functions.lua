ProjectRP.Functions = {}
ProjectRP.RequestId = 0

-- Player

function ProjectRP.Functions.GetPlayerData(cb)
    if cb then
        cb(ProjectRP.PlayerData)
    else
        return ProjectRP.PlayerData
    end
end
...
function ProjectRP.Functions.HasItem(item)
...
function ProjectRP.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
ProjectRP.Functions.Draw2DText = function(x, y, text, scale)
    SetTextFont(4)
...
function ProjectRP.Functions.Notify(text, textype, length)
...
function ProjectRP.Debug(resource, obj, depth)
    TriggerServerEvent('ProjectRP:DebugSomething', resource, obj, depth)
end

function ProjectRP.Functions.TriggerCallback(name, cb, ...)
    ProjectRP.ServerCallbacks[name] = cb
    TriggerServerEvent('ProjectRP:Server:TriggerCallback', name, ...)
end

function ProjectRP.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
...
function ProjectRP.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end
function ProjectRP.Functions.GetObjects()
...
function ProjectRP.Functions.GetPeds(ignoreList)
...
function ProjectRP.Functions.GetClosestPed(coords, ignoreList)
    ...
    local ignoreList = ignoreList or {}
    local peds = ProjectRP.Functions.GetPeds(ignoreList)
...
function ProjectRP.Functions.GetClosestPlayer(coords)
    ...
    local closestPlayers = ProjectRP.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function ProjectRP.Functions.GetPlayersFromCoords(coords, distance)
...
function ProjectRP.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function ProjectRP.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
ProjectRP.Functions.SpawnObject = function(model, coords, cb)
...
ProjectRP.Functions.SpawnLocalObject = function(model, coords, cb)
    local model = (type(model) == 'number' and model or GetHashKey(model))
...
ProjectRP.Functions.DeleteObject = function(object)
...
function ProjectRP.Functions.SpawnVehicle(model, cb, coords, isnetworked)
...
function ProjectRP.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function ProjectRP.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return ProjectRP.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end
