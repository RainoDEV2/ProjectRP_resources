...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function()
    ProjectRP.Functions.TriggerCallback('prp-houserobbery:server:GetHouseConfig', function(HouseConfig)
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = {name = ProjectRP.Shared.Items["lockpick"]["name"], image = ProjectRP.Shared.Items["lockpick"]["image"]},
        [2] = {name = ProjectRP.Shared.Items["screwdriverset"]["name"], image = ProjectRP.Shared.Items["screwdriverset"]["image"]},
    }
    while true do
        inRange = false
...
function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['prp-interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
RegisterNetEvent('prp-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('prp-houserobbery:client:enterHouse', function(house)
            ...
            if closestHouse ~= nil then
                if CurrentCops >= Config.MinimumHouseRobberyPolice then
                    if not Config.Houses[closestHouse]["opened"] then
                        ...
                        ProjectRP.Functions.Notify('The door is already open..', 'error', 3500)
                    end
                else
                    ProjectRP.Functions.Notify('Not enough Police..', 'error', 3500)
                end
            end
        else
            ProjectRP.Functions.TriggerCallback('ProjectRP:HasItem', function(result)
                if closestHouse ~= nil then
                    if result then
                        ...
                        ProjectRP.Functions.Notify('It looks like you are missing something...', 'error', 3500)
                    end
                end
            end, "screwdriverset")
        end
...
function lockpickFinish(success)
    if success then
        ...
        ProjectRP.Functions.Notify('It worked!', 'success', 2500)
    else
        if usingAdvanced then
            local itemInfo = ProjectRP.Shared.Items["advancedlockpick"]
        ...
        ProjectRP.Functions.Notify('It did not work..', 'error', 2500)
    end
end

RegisterNetEvent('prp-houserobbery:client:setHouseState', function(house, state)
...
RegisterNetEvent('prp-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('prp-houserobbery:client:SetBusyState', function(cabin, house, bool)
