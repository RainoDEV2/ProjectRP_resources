...
PRPCore.Functions.CreateCallback('prp-scenes:server:GetScenes', function(source, cb)
    cb(scenes)
end)
...
function UpdateAllScenes()
    ...
    MySQL.Async.fetchAll('SELECT * FROM scenes', {}, function(result)
        ...
        TriggerClientEvent('prp-scenes:client:UpdateAllScenes', -1, scenes)
    end)
end
...
RegisterNetEvent('prp-scenes:server:DeleteScene', function(id)
    MySQL.Async.execute('DELETE FROM scenes WHERE id = ?', {id}, function(result)
        UpdateAllScenes()
    end)
end)

RegisterNetEvent('prp-scenes:server:CreateScene', function(sceneData)
    local src = source 
    ...
    MySQL.Async.insert('INSERT INTO scenes (creator, text, color, viewdistance, expiration, fontsize, fontstyle, coords, date_creation, date_deletion) VALUES (? ,?, ?, ?, ?, ?, ?, ?, NOW(), DATE_ADD(NOW(), INTERVAL ? HOUR))', {
        PRPCore.Functions.GetPlayer(src).PlayerData.citizenid, 
        sceneData.text,
        sceneData.color,
