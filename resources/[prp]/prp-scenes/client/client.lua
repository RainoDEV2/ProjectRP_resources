...
-- RegisterKeyMapping('deletescene', 'Delete Scene', 'keyboard', Config.DeleteSceneKey) -- disabled
...
RegisterNetEvent('prp-scenes:client:UpdateAllScenes', function(_scenes)
    scenes = _scenes
end)

RegisterNetEvent("PRPCore:Client:OnPlayerLoaded", function()
    GetScenes()
end)
...
function GetScenes()
    PRPCore.Functions.TriggerCallback('prp-scenes:server:GetScenes', function(_scenes)
        scenes = _scenes
    end)
end
        ...
        CreateThread(function()
            while creationLaser do
                ...
                if IsControlJustReleased(0, 38) then
                    creationLaser = false
                    if hit then
                        TriggerServerEvent('prp-scenes:server:CreateScene', data)
                    else
                        PRPCore.Functions.Notify("Laser did not hit anything.", "error")
                    end
...
function DeleteScene(coords)
    ...
    if closestScene then
        PRPCore.Functions.Notify("Scene deleted!", "success")
        TriggerServerEvent('prp-scenes:server:DeleteScene', closestScene)
    else
        PRPCore.Functions.Notify("No scene was close enough.", "error")
    end
end
