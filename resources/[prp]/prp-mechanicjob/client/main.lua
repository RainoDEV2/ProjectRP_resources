...
AddEventHandler('ProjectRP:Client:OnPlayerLoaded', function()
    ...
    ProjectRP.Functions.TriggerCallback('prp-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    ProjectRP.Functions.TriggerCallback('prp-vehicletuning:server:GetDrivingDistances', function(retval)
...
function SpawnListVehicle(model)
    ...
    ProjectRP.Functions.SpawnVehicle(model, function(veh)
...
function PartsMenu()
    ClearMenu()
    local plate = ProjectRP.Functions.GetPlate(Config.Plates[ClosestPlate].AttachedVehicle)
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..ProjectRP.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil)
end
...
function RepairPart(part)
    ...
    ProjectRP.Functions.TriggerCallback('prp-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            ProjectRP.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                ProjectRP.Functions.Notify("Repair Cancelled", "error")
            end)
        else
            ProjectRP.Functions.Notify('There Are Not Enough Materials In The Safe', 'error')
        end
    end, "mechanicstash")
end
...
AddEventHandler('prp-vehicletuning:client:RepaireeePart', function(part)
    ...
    ProjectRP.Functions.Notify("The "..Config.ValuesLabels[part].." Is Repaired!")
end)
            ...
            if #(pos - vehpos) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then
                        ...
                        ProjectRP.Functions.Notify("Status Unknown", "error")
                    end
                else
                    ProjectRP.Functions.Notify("Not A Valid Vehicle", "error")
                end
            else
                ProjectRP.Functions.Notify("You Are Not Close Enough To The Vehicle", "error")
            end
        else
            ProjectRP.Functions.Notify("You Must Be In The Vehicle First", "error")
        end
    else
        ProjectRP.Functions.Notify("You Must Be Outside The Vehicle", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            ProjectRP.Functions.Notify("You Are Not The Driver Or On A Bicycle", "error")
        end
    else
        ProjectRP.Functions.Notify("You Are Not In A Vehicle", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:repairPart', function(part, level, needAmount)
    -- if CanReapair() then
        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if #(pos - vehpos) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            ProjectRP.Functions.Progressbar("repair_advanced", "Repair Vehicle", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                                ...
                                if part == "body" then
                                    ...
                                    TriggerServerEvent("ProjectRP:Server:RemoveItem", Config.RepairCost[part], needAmount)
                                    TriggerEvent("inventory:client:ItemBox", ProjectRP.Shared.Items[Config.RepairCost[part]], "remove")
                                elseif part ~= "engine" then
                                    TriggerServerEvent("vehiclemod:server:updatePart", plate, part, GetVehicleStatus(plate, part) + level)
                                    TriggerServerEvent("ProjectRP:Server:RemoveItem", Config.RepairCost[part], level)
                                    TriggerEvent("inventory:client:ItemBox", ProjectRP.Shared.Items[Config.RepairCost[part]], "remove")
                                end
                            end, function() -- Cancel
                                openingDoor = false
                            ...
                            ProjectRP.Functions.Notify("Not A Valid Part", "error")
                        end
                    else
                        ProjectRP.Functions.Notify("Not A Valid Vehicle", "error")
                    end
            ...
            ProjectRP.Functions.Notify("Youre Not In a Vehicle", "error")
        end
    -- end
end)
...
function SendStatusMessage(statusList)
    if statusList ~= nil then
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Vehicle Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..ProjectRP.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..ProjectRP.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..ProjectRP.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..ProjectRP.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..ProjectRP.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..ProjectRP.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..ProjectRP.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
