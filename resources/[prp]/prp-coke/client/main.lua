...
Citizen.CreateThread(function()
    ProjectRP.Functions.TriggerCallback('coke:processcoords', function(servercoords)
        process = servercoords
	end)
end)

Citizen.CreateThread(function()
    ProjectRP.Functions.TriggerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not inUse then
			if dist <= 2.0 then
				sleep = 5
				DrawText3D(coord.x, coord.y, coord.z, 'Press ~g~[ E ]~w~ to rent a Plane for $'..Config.rentalAmount)
				if IsControlJustPressed(1, 51) then
					ProjectRP.Functions.TriggerCallback('coke:pay', function(success)
						if success then
							main()
						end
					end)
...
function main()
	...
	ProjectRP.Functions.Notify("Go to the airfield.", "success")
...
function PlaneSpawn()
    ...
    while true do
		Citizen.Wait(1)
		DrawText3D(location.parking.x, location.parking.y, location.parking.z, "Rented Plane.")
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				ProjectRP.Functions.Notify("Failed, your plane was Destroyed", "error")
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	ProjectRP.Functions.Notify("Get in the plane and pick up the delivery marked on your GPS", "success")
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					ProjectRP.Functions.Progressbar("picking_", "Picking up the delivery..", lockpickTime, false, true, {
					...
					Citizen.Wait(2000)
					ProjectRP.Functions.Notify("Picked up the delivery. Return to the airfield marked on your GPS.", "success")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	ProjectRP.Functions.Notify("Deliver the plane back to a hangar", "success")
		...
		while hangar do
			...
			if IsPedInAnyPlane(GetPlayerPed(-1)) and disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					ProjectRP.Functions.Progressbar("lockpick_vehicledoor", "Parking..", 1000, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
			...
			if IsControlJustPressed(1, 51) then		
				...
				ProjectRP.Functions.TriggerCallback('ProjectRP:HasItem', function(result)
					if result then
						...
						ProjectRP.Functions.Notify("You need coke brick to do this", "error")
						isProcessing = false
					end
				end, 'coke_brick')
			end
...
function processing()
	...
	SetEntityHeading(player, 191.734)
	FreezeEntityPosition(player, true)
	playAnim("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 30000)

	ProjectRP.Functions.Progressbar("coke-", "Breaking down the coke..", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
