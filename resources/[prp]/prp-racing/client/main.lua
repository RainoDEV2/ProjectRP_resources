...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        ...
        if ClosestCheckpoint then
            ...
            ProjectRP.Functions.Notify("You can't go that fast!", 'error')
        end
    else
        ProjectRP.Functions.Notify("You can't go that fast!", 'error')
    end
...
function SaveRace()
    ...
    TriggerServerEvent('prp-racing:server:SaveRace', CreatorData)
    ProjectRP.Functions.Notify("The race was saved "..'('..CreatorData.RaceName..')', 'success')
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, 162) or IsDisabledControlJustPressed(0, 162) then
                    if CreatorData.Checkpoints and next(CreatorData.Checkpoints) then
                        ...
                        ProjectRP.Functions.Notify("You have not placed any checkpoints to delete.", 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 311) or IsDisabledControlJustPressed(0, 311) then
                    if CreatorData.Checkpoints and #CreatorData.Checkpoints >= Config.MinimumCheckpoints then
                        ...
                        ProjectRP.Functions.Notify("You need a minimum number of checkpoints to save"..'('..Config.MinimumCheckpoints..')', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, 40) or IsDisabledControlJustPressed(0, 40) then
                    if CreatorData.TireDistance < Config.MaxTireDistance then
                        ...
                        ProjectRP.Functions.Notify("The max distance allowed is "..Config.MaxTireDistance)
                    end
                end
                ...
                if IsControlJustPressed(0, 39) or IsDisabledControlJustPressed(0, 39) then
                    if CreatorData.TireDistance > Config.MinTireDistance then
                        ...
                        ProjectRP.Functions.Notify("The min distance allowed is "..Config.MinTireDistance)
                    end
                end
            else
            ...
            if IsControlJustPressed(0, 163) or IsDisabledControlJustPressed(0, 163) then
                if not CreatorData.ConfirmDelete then
                    ...
                    ProjectRP.Functions.Notify("Press [9] again to confirm.", 'error')
                else
                    DeleteCreatorCheckpoints()
                    ...
                    CreatorData.RaceName = nil
                    CreatorData.Checkpoints = {}
                    ProjectRP.Functions.Notify("You canceled the editor.", 'error')
...
function FinishRace()
    TriggerServerEvent('prp-racing:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    ProjectRP.Functions.Notify("You finished the race in "..SecondsToClock(CurrentRaceData.TotalTime), 'success')
    if CurrentRaceData.BestLap ~= 0 then
        ProjectRP.Functions.Notify("You got a best lap of "..SecondsToClock(CurrentRaceData.BestLap), 'success')
    end
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            TriggerServerEvent('prp-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('prp-racing:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
RegisterNetEvent('prp-racing:server:ReadyJoinRace', function(RaceData)
    RaceData.RacerName = RaceData.SetupRacerName
    TriggerServerEvent('prp-racing:server:JoinRace', RaceData)
end)

RegisterNetEvent('prp-racing:client:StartRaceEditor', function(RaceName, RacerName)
    if not RaceData.InCreator then
        ...
        ProjectRP.Functions.Notify("You are already making a race.", 'error')
    end
end)

RegisterNetEvent('prp-racing:client:UpdateRaceRacerData', function(RaceId, RaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = RaceData.Racers
    end
end)

RegisterNetEvent('prp-racing:client:JoinRace', function(Data, Laps, RacerName)
    if not RaceData.InRace then
        Data.RacerName = RacerName
        RaceData.InRace = true
        SetupRace(Data, Laps)
        ProjectRP.Functions.Notify("You joined the race.")
        TriggerServerEvent('prp-racing:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        ProjectRP.Functions.Notify("You are already in a race.", 'error')
    end
end)

RegisterNetEvent('prp-racing:client:UpdateRaceRacers', function(RaceId, Racers)
    if CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = Racers
    end
end)

RegisterNetEvent('prp-racing:client:LeaveRace', function(data)
    DeleteCurrentRaceCheckpoints()
    FreezeEntityPosition(GetVehiclePedIsIn(PlayerPedId(), false), false)
end)

RegisterNetEvent('prp-racing:client:RaceCountdown', function()
    TriggerServerEvent('prp-racing:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    ProjectRP.Functions.Notify("The race will start in 10 seconds.", 'primary', 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    ProjectRP.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        if CurrentRaceData.RaceName ~= nil then
            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
            ProjectRP.Functions.Notify("GO!", 'success', 1000)
            SetBlipScale(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].blip, 1.0)
...
RegisterNetEvent('prp-racing:client:PlayerFinish', function(RaceId, Place, RacerName)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            ProjectRP.Functions.Notify(RacerName.." finished in place: "..Place, 'primary', 3500)
        end
    end
end)

RegisterNetEvent('prp-racing:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            ProjectRP.Functions.Notify("Return to the start or you will be kicked from the race: "..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('prp-racing:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
...
RegisterNetEvent("prp-racing:Client:OpenMainMenu", function(data)
    local type = data.type
    local name = data.name

    exports['prp-menu']:openMenu({
        {
            header = "Ready to Race, "..name..'?',
            isMenuHeader = true
        }, 
        {
            header = "Current Race",
            ...
            params = {
                event = "prp-racing:Client:CurrentRaceMenu",
        ...
        },
        {
            ...
            params = {
                event = "prp-racing:Client:AvailableRacesMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "prp-racing:Client:RaceRecordsMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "prp-racing:Client:SetupRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
            ...
            params = {
                event = "prp-racing:Client:CreateRaceMenu",
                args = { type = type, name = name }
            }
        },
        {
...
RegisterNetEvent("prp-racing:Client:CurrentRaceMenu", function(data)
    ...
    exports['prp-menu']:openMenu({
        ...
        }, 
        {
            header = "Start Race",
            txt = "",
            disabled = (not (CurrentRaceData.Creator == ProjectRP.Functions.GetPlayerData().citizenid) or CurrentRaceData.Started),
            params = {
                isServer = true,
                event = "prp-racing:server:StartRace",
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = "prp-racing:server:LeaveRace",
        ...
        },
        {
            header = "← Go Back",
            params = {
                event = "prp-racing:Client:OpenMainMenu",
...
RegisterNetEvent("prp-racing:Client:AvailableRacesMenu", function(data)
    ProjectRP.Functions.TriggerCallback('prp-racing:server:GetRaces', function(Races)
        local menu = {
            {
                header = "Available Races",
                isMenuHeader = true
        ...
        for _,race in ipairs(Races) do
            ...
            menu[#menu+1] = {
                ...
                txt = string.format("%s lap(s) | %sm | %s racer(s)", race.Laps, RaceData.Distance, racers),
                disabled = CurrentRaceData.RaceId == RaceData.RaceId,
                params = {
                    isServer = true,
                    event = "prp-racing:server:JoinRace",
                    args = race
                }
            }
        end
        ...
        if #menu == 2 then 
            ProjectRP.Functions.Notify("There are no pending races at the moment.")
            TriggerEvent('prp-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
...
RegisterNetEvent("prp-racing:Client:RaceRecordsMenu", function(data)
    ProjectRP.Functions.TriggerCallback('prp-racing:server:GetRacingLeaderboards', function(Races)
        local menu = {
            {
                header = "Race Records",
                isMenuHeader = true
        ...
        if #menu == 2 then 
            ProjectRP.Functions.Notify("No races exist yet to see records of!")
            TriggerEvent('prp-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
...
RegisterNetEvent("prp-racing:Client:SetupRaceMenu", function(data)
    ProjectRP.Functions.TriggerCallback('prp-racing:server:GetListedRaces', function(Races)
        ...
        if #tracks == 1 then 
            ProjectRP.Functions.Notify("There are no available tracks at the moment to use.")
            TriggerEvent('prp-racing:Client:OpenMainMenu', { type = data.type, name = data.name })
            return
        end
        ...
        local dialog = exports['prp-input']:ShowInput({
            ...
            inputs = {
                ...
                {
                    text = "Number of Laps (0 is Sprint)",
                    name = "laps", 
                    type = "number",
                    isRequired = true
                },
        ...
        TriggerServerEvent('prp-racing:server:SetupRace', dialog.track, tonumber(dialog.laps), data.name)
    end)
end)

RegisterNetEvent("prp-racing:Client:CreateRaceMenu", function(data)
    local dialog = exports['prp-input']:ShowInput({
    ...
    if #dialog.trackname < Config.MinTrackNameLength then
        ProjectRP.Functions.Notify('The name is too short.', "error")
        TriggerEvent("prp-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
        return
    end

    if #dialog.trackname > Config.MaxTrackNameLength then
        ProjectRP.Functions.Notify('The name is too long.', "error") 
        TriggerEvent("prp-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
        return
    end

    ProjectRP.Functions.TriggerCallback('prp-racing:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        if not IsAuthorized then return end
        if not NameAvailable then 
            ProjectRP.Functions.Notify("There is already a race with that name.", "error")
            TriggerEvent("prp-racing:Client:CreateRaceMenu", { type = data.type, name = data.name })
            return 
        end

        TriggerServerEvent('prp-racing:server:CreateLapRace', dialog.trackname, data.name)
