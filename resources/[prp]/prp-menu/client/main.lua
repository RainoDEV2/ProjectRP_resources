...
RegisterNetEvent('prp-menu:client:openMenu', function(data)
    openMenu(data)
end)

RegisterNetEvent('prp-menu:client:closeMenu', function()
    closeMenu()
end)
    ...
    if sendData then
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isPRPCommand then
                    TriggerServerEvent('ProjectRP:CallCommand', data.params.event, data.params.args)
