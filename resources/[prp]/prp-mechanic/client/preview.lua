...
local function printDifferences(vehicle, properties, newproperties)
	...
	if not Config.PreviewPhone then
		...
		if vehlist[1] ~= nil then 
			...
			TriggerServerEvent("prp-mechanic:server:giveList", info)
		end
	end
end

RegisterNetEvent("prp-mechanic:client:giveList", function(item)
	local list = {}
	local newlist = ""
...
local function preview(playerPed, vehicle)
	...
	local properties = ProjectRP.Functions.GetVehicleProperties(vehicle)
	...
	while true do
		...
		elseif not previewing then
			local newproperties = ProjectRP.Functions.GetVehicleProperties(vehicle)
			ProjectRP.Functions.SetVehicleProperties(vehicle, properties)
...
RegisterNetEvent('prp-mechanic:client:Preview:Menu', function()
	...
	if IsPedInAnyVehicle(playerPed, false) then
		local PreviewMenu = {}
			--PreviewMenu[#PreviewMenu+1] = { header = "Test", txt = "Vehicle Death Simulator", params = { event = "prp-mechanic:client:Preview:test" }, }
		...
		for i = 1, #list do
			if GetNumVehicleMods(vehicle, list[i].id) ~= 0 then
				PreviewMenu[#PreviewMenu+1] = { header = "", txt = list[i].name..Loc[Config.Lan]["common"].amountoption..(GetNumVehicleMods(vehicle, list[i].id)+1).." ]", params = { event = "prp-mechanic:client:Preview:Multi", args = list[i] }, }
			end
		end
		
		if not IsThisModelABike(GetEntityModel(vehicle)) then 
			PreviewMenu[#PreviewMenu+1] = { header = "", txt = Loc[Config.Lan]["windows"].menuheader, params = { event = "prp-mechanic:client:Preview:Windows:Check" }, }
		end
		exports['prp-menu']:openMenu(PreviewMenu)
		preview(playerPed, vehicle)

	end
end)

RegisterNetEvent("prp-mechanic:client:Preview:Multi", function(data)
	local playerPed = PlayerPedId()
	local validMods = {}
	...
	local modMenu = {
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Menu" } },
			{ header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "prp-mechanic:client:Preview:Multi:Apply", args = { id = -1, mod = data.id, name = data.name }, } } }
		for k,v in pairs(validMods) do
			modMenu[#modMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Preview:Multi:Apply', args = { id = tostring(v.id), mod = data.id, name = data.name }, } }
		end
	exports['prp-menu']:openMenu(modMenu)
end)

RegisterNetEvent('prp-mechanic:client:Preview:Multi:Apply', function(data)
	local playerPed	= PlayerPedId()
	...
	if GetVehicleMod(vehicle, data.mod) == tonumber(data.id) then
		TriggerEvent('prp-mechanic:client:Preview:Multi', returndata)
...
RegisterNetEvent('prp-mechanic:client:Preview:Livery', function()
	local playerPed = PlayerPedId()
	local validMods = {}
	local vehicle = nil
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) 
	...
	if DoesEntityExist(vehicle) then
		...
		if oldlivery == true then
			...
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Preview:Apply', args = { id = tostring(v.id), old = true } } }
			end
		elseif oldlivery ~= true then
			...
			for k,v in pairs(validMods) do
				LiveryMenu[#LiveryMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Preview:Apply', args = { id = tostring(v.id) } } }
			end
		end
...
RegisterNetEvent('prp-mechanic:client:Preview:Apply', function(data)
	local playerPed	= PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	if data.old then
		...
		if GetVehicleLivery(vehicle) == tonumber(data.id) then
			...
			TriggerEvent('prp-mechanic:client:Preview:Livery')
			return
		end
	else
		if modName == "NULL" then modName = Loc[Config.Lan]["common"].stock end
...
RegisterNetEvent('prp-mechanic:client:Preview:Plates', function()
	local playerPed	= PlayerPedId()
	local vehicle = nil
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) 
		if DoesEntityExist(vehicle) then
			...
			for k, v in pairs(Loc[Config.Lan].vehiclePlateOptions) do
				...
				PlateMenu[#PlateMenu + 1] = { header = k..". "..v.name, txt = installed, params = { event = 'prp-mechanic:client:Preview:Plates:Apply', args = v.id  } }
			end
...
RegisterNetEvent('prp-mechanic:client:Preview:Plates:Apply', function(index)
	local playerPed	= PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	if GetVehicleNumberPlateTextIndex(vehicle) == tonumber(index) then TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["plates"].already, "error") TriggerEvent('prp-mechanic:client:Preview:Plates') 
	elseif GetVehicleNumberPlateTextIndex(vehicle) ~= tonumber(index) then
		SetVehicleNumberPlateTextIndex(vehicle, index)
		emptyHands(playerPed)
		TriggerEvent('prp-mechanic:client:Preview:Plates')
	end
end)

RegisterNetEvent('prp-mechanic:client:Preview:Paint', function()
	local playerPed	= PlayerPedId()
	local validMods = {}
	local vehicle = nil
	if IsPedInAnyVehicle(playerPed, false) then vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	local PaintMenu = {	
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Menu", } } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].primary, txt = Loc[Config.Lan]["common"].current..": "..vehPrimaryColour, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].primary } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].secondary, txt = Loc[Config.Lan]["common"].current..": "..vehSecondaryColour, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].secondary } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].pearl, txt = Loc[Config.Lan]["common"].current..": "..vehPearlescentColour, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].pearl } }
		PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].wheel, txt = Loc[Config.Lan]["common"].current..": "..vehWheelColour, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].wheel } }
		if not IsThisModelABike(GetEntityModel(vehicle)) then
			PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].interior, txt = Loc[Config.Lan]["common"].current..": "..interiorColor, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].interior } }
			PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].dashboard, txt = Loc[Config.Lan]["common"].current..": "..dashboardColor, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = Loc[Config.Lan]["paint"].dashboard } }
		end
	exports['prp-menu']:openMenu(PaintMenu)
end)

RegisterNetEvent('prp-mechanic:client:Preview:Paints:Choose', function(data)
	local playerPed	= PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	if DoesEntityExist(vehicle) then
		exports['prp-menu']:openMenu({
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Paint" } },
			{ header = Loc[Config.Lan]["paint"].classic, txt = "", params = { event = "prp-mechanic:client:Preview:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].classic } } },
			{ header = Loc[Config.Lan]["paint"].metallic, txt = "", params = { event = "prp-mechanic:client:Preview:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metallic } } },
			{ header = Loc[Config.Lan]["paint"].matte, txt = "", params = { event = "prp-mechanic:client:Preview:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].matte } } },
			{ header = Loc[Config.Lan]["paint"].metals, txt = "", params = { event = "prp-mechanic:client:Preview:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metals } } },
		})
	end
end)

RegisterNetEvent('prp-mechanic:client:Preview:Paints:Choose:Colour', function(data)
	local playerPed	= PlayerPedId()
	local vehicle = nil
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	local vehPrimaryColour, vehSecondaryColour = GetVehicleColours(vehicle)
	...
	local PaintMenu = {	
		...
		{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Paints:Choose", args = data.paint } } }
	local installed = nil
	if data.finish == Loc[Config.Lan]["paint"].classic then
		for k, v in pairs(Loc[Config.Lan].vehicleResprayOptionsClassic) do
			...
			PaintMenu[#PaintMenu + 1] = { header = k..". "..v.name, txt = installed, params = { event = 'prp-mechanic:client:Preview:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } } } end
...
RegisterNetEvent('prp-mechanic:client:Preview:Paints:Apply', function(data)
	local playerPed	= PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	if IsPedInAnyVehicle(playerPed, false) then vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	local vehPrimaryColour, vehSecondaryColour = GetVehicleColours(vehicle)
	...
	TriggerEvent('prp-mechanic:client:Preview:Paints:Choose:Colour', data)
end)

--WHEELS
RegisterNetEvent('prp-mechanic:client:Preview:Rims:Apply', function(data)
	local playerPed = PlayerPedId()
	...
	if data.mod == -1 then TriggerEvent('prp-mechanic:client:Preview:Rims:Choose', data) else TriggerEvent('prp-mechanic:client:Preview:Rims:SubMenu', data) end
end)

RegisterNetEvent('prp-mechanic:client:Preview:Rims:Check', function()
	local playerPed = PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	WheelMenu[#WheelMenu + 1] = { header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Menu" } }
	if not IsThisModelABike(GetEntityModel(vehicle)) then 
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label1, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 0, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label2, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 1, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label3, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 2, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label4, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 3, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label5, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 4, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label6, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 5, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label7, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 7, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label8, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 8, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label9, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 9, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label10, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 10, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label11, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 11, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label12, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 12, bike = false } } }
	elseif IsThisModelABike(GetEntityModel(vehicle)) then 
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label13, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 6, bike = false } } }
		WheelMenu[#WheelMenu + 1] = { header = Loc[Config.Lan]["rims"].label14, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = 6, bike = true } } }
	end			
	exports['prp-menu']:openMenu(WheelMenu)
end)

RegisterNetEvent('prp-mechanic:client:Preview:Rims:Choose', function(data)
	local playerPed = PlayerPedId()
	local validMods = {}
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	originalWheel = GetVehicleWheelType(vehicle)
	...
	for i = 1, (GetNumVehicleMods(vehicle, 23) +1) do
		...
		local RimsMenu = {
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Rims:Check" } },
			{ header = Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "prp-mechanic:client:Preview:Rims:Apply",  args = { mod = -1, wheeltype = data.wheeltype, } } } }
		for k, v in pairs(validMods) do
			if not validMods[k][1] then
				RimsMenu[#RimsMenu + 1] = { header = v.name, txt = v.install, params = { event = 'prp-mechanic:client:Preview:Rims:Apply', args = { mod = v.id, wheeltype = data.wheeltype, wheelname = GetLabelText(GetModTextLabel(vehicle, 23, GetVehicleMod(vehicle, 23))) } } }
			else
				if k == "NULL" then setheader = Loc[Config.Lan]["rims"].labelcustom else setheader = k end
				RimsMenu[#RimsMenu + 1] = { header = setheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods[k], params = { event = 'prp-mechanic:client:Preview:Rims:SubMenu', args = { mod = v.id, wheeltype = data.wheeltype, wheeltable = validMods[k], bike = data.bike } } }
			end
		end
...
RegisterNetEvent('prp-mechanic:client:Preview:Rims:SubMenu', function(data)
	local playerPed = PlayerPedId()
	local validMods = {}
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	local RimsMenu = {
		...
		{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Rims:Choose", args = { wheeltype = data.wheeltype } } } }
	for i=1, #data.wheeltable do
		RimsMenu[#RimsMenu + 1] = { header = data.wheeltable[i].name, txt = data.wheeltable[i].install, params = { event = 'prp-mechanic:client:Preview:Rims:Apply', args = { mod = data.wheeltable[i].id, wheeltype = data.wheeltype, wheeltable = data.wheeltable, bike = data.bike } } }
	end
	exports['prp-menu']:openMenu(RimsMenu)
end)
...
RegisterNetEvent('prp-mechanic:client:Preview:Windows:Apply', function(data)
	local playerPed = PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	if GetVehicleWindowTint(vehicle) == tonumber(data.mod) then TriggerEvent('ProjectRP:Notify', data.name..Loc[Config.Lan]["common"].already, "error") TriggerEvent('prp-mechanic:client:Preview:Windows:Check')
	elseif GetVehicleWindowTint(vehicle) ~= tonumber(data.mod) then
		SetVehicleWindowTint(vehicle, tonumber(data.mod))
		TriggerEvent('prp-mechanic:client:Preview:Windows:Check')
		emptyHands(playerPed)
	end
end)

RegisterNetEvent('prp-mechanic:client:Preview:Windows:Check', function()
	local playerPed = PlayerPedId()
	if IsPedInAnyVehicle(playerPed, false) then	vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	if DoesEntityExist(vehicle) then
		...
		exports['prp-menu']:openMenu({
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Preview:Menu" } },
			{ header = Loc[Config.Lan]["windows"].label1, txt = applied1, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 0, name = Loc[Config.Lan]["windows"].label1 } } },
			{ header = Loc[Config.Lan]["windows"].label2, txt = applied2, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 4, name = Loc[Config.Lan]["windows"].label2 } } },
			{ header = Loc[Config.Lan]["windows"].label3, txt = applied3, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 5, name = Loc[Config.Lan]["windows"].label3 } } },
			{ header = Loc[Config.Lan]["windows"].label4, txt = applied4, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 3, name = Loc[Config.Lan]["windows"].label4 } } },
			{ header = Loc[Config.Lan]["windows"].label5, txt = applied5, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 2, name = Loc[Config.Lan]["windows"].label5 } } },
			{ header = Loc[Config.Lan]["windows"].label6, txt = applied6, params = { event = "prp-mechanic:client:Preview:Windows:Apply", args = { mod = 1, name = Loc[Config.Lan]["windows"].label6 } } },
		})
	end
end)