...
--========================================================== Hood
RegisterNetEvent('prp-mechanic:client:Hood:Apply', function(data)
	...
	if GetVehicleMod(vehicle, 7) == tonumber(data.id) then
		TriggerEvent('ProjectRP:Notify', modName..Loc[Config.Lan]["hood"].installed, "error")
		TriggerEvent('prp-mechanic:client:Hood:Check')
	elseif GetVehicleMod(vehicle, 7) ~= tonumber(data.id) then
		...
		{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function()
			...
			TriggerEvent('prp-mechanic:client:Hood:Check')
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["hood"].installed, "success")
		end, function()
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["hood"].failed, "error")
			emptyHands(playerPed)
		end)
	end
end)

RegisterNetEvent('prp-mechanic:client:Hood:Check', function()
	...
	if DoesEntityExist(vehicle) then
		if GetNumVehicleMods(vehicle, 7) == 0 then TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["common"].noOptions, "error") return	end				
		...
		local HoodMenu = {
				...
				{ header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall,	params = { event = "prp-mechanic:client:Hood:Apply", args = { id = -1 } } }, }
			for k,v in pairs(validMods) do
				HoodMenu[#HoodMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Hood:Apply', args = { id = tostring(v.id) } } }
			end
		exports['prp-menu']:openMenu(HoodMenu)
	end
end)