...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function() ProjectRP.Functions.TriggerCallback('prp-mechanic:GetNosLoadedVehs', function(vehs) VehicleNitrous = vehs while VehicleNitrous == nil do Wait(100) end end) end)

AddEventHandler('onResourceStart', function(resource)
    if GetCurrentResourceName() == resource then
		ProjectRP.Functions.TriggerCallback('prp-mechanic:GetNosLoadedVehs', function(vehs) VehicleNitrous = vehs while VehicleNitrous == nil do Wait(100) end end)
	end
	Wait(500)
end)

RegisterNetEvent('prp-mechanic:client:applyNOS', function()
	...
	if DoesEntityExist(vehicle) then
		if not IsToggleModOn(vehicle, 18) then TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].notinstalled, "error") return end
		...
		function() -- On success
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].installing, "success")
			time = math.random(7000,10000)
			playAnim("mini@repair", "fixing_a_ped", time, 16)
			ProjectRP.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing.."NOS..", time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, }, 
			{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 8, }, {}, {}, function()
				ClearPedTasks(playerPed)
				if VehicleNitrous[trim(GetVehicleNumberPlateText(vehicle))] ~= nil then TriggerServerEvent("prp-mechanic:server:giveNOS") end
				TriggerServerEvent('prp-mechanic:server:LoadNitrous', trim(GetVehicleNumberPlateText(vehicle)))
				SetVehicleDoorShut(vehicle, 4, false)
				updateCar(vehicle)
				TriggerServerEvent('prp-mechanic:server:removeNOS')
				TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].installed, "success")
			end, function() -- Cancel
				TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].failed, "error")
				SetVehicleDoorShut(vehicle, 4, false)
				ClearPedTasks(playerPed)
			end)
...
RegisterNetEvent('prp-mechanic:client:UpdateNitroLevel', function(Plate, level) VehicleNitrous[Plate].level = level end)

RegisterNetEvent('prp-mechanic:client:LoadNitrous', function(Plate)
...
RegisterNetEvent('prp-mechanic:client:UnloadNitrous', function(Plate)
			...
			if VehicleNitrous[Plate] then
				if VehicleNitrous[Plate].hasnitro then
					if IsControlJustPressed(0, 21) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
						...
						TriggerServerEvent('prp-mechanic:server:SyncPurge', VehToNet(CurrentVehicle), true)
						elseif kmh > 25.0 and boosting == false then
							...
							TriggerServerEvent('prp-mechanic:server:SyncTrail', VehToNet(CurrentVehicle), true)
							...
							CreateThread(function()
								while NitrousActivated do
									TriggerServerEvent('prp-mechanic:server:SyncPurge', VehToNet(CurrentVehicle), false)
									if VehicleNitrous[Plate].level - 1 >= 0 then
										TriggerServerEvent('prp-mechanic:server:SyncFlame', VehToNet(CurrentVehicle), 1.25)
										--CreateVehicleExhaustBackfire(CurrentVehicle, 1.25)
										TriggerServerEvent('prp-mechanic:server:UpdateNitroLevel', Plate, (VehicleNitrous[Plate].level - NitrousUseRate))
									...
									elseif VehicleNitrous[Plate].level - 1 <= 0 then
										TriggerServerEvent('prp-mechanic:server:UnloadNitrous', Plate)
										TriggerServerEvent("prp-mechanic:server:giveNOS")
										SetNitroBoostScreenEffectsEnabled(false)
										--SetVehicleLightTrailEnabled(CurrentVehicle, false)
										TriggerServerEvent('prp-mechanic:server:SyncTrail', VehToNet(CurrentVehicle), false)
    ...
    while true do
        ...
        if IsInVehicle then
			...
			if previousVehicle ~= CurrentVehicle then
				...
				TriggerServerEvent('prp-mechanic:server:SyncTrail', VehToNet(previousVehicle), false)
				SetNitroBoostScreenEffectsEnabled(false)
				TriggerServerEvent('prp-mechanic:server:SyncPurge', VehToNet(previousVehicle), false)
...
RegisterNetEvent('prp-mechanic:client:SyncPurge', function(netid, enabled)
...
RegisterNetEvent('prp-mechanic:client:SyncTrail', function(netid, enabled)
...
RegisterNetEvent('prp-mechanic:client:SyncFlame', function(netid, scale)
...
-- Mechanic Remove NOS
RegisterNetEvent('prp-mechanic:client:giveNOS', function()
	...
	ProjectRP.Functions.Progressbar("accepted_key", Loc[Config.Lan]["nos"].removing, 8000, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, }, 
	{ animDict = "mini@repair", anim = "fixing_a_ped", flags = 16, }, {}, {}, function()
		SetVehicleMod(vehicle, 16, -1)
		SetVehicleDoorShut(vehicle, 4, false)
		updateCar(vehicle)
		TriggerServerEvent('prp-mechanic:server:UnloadNitrous', trim(GetVehicleNumberPlateText(vehicle)))
		TriggerServerEvent('prp-mechanic:server:giveNOS')
		emptyHands(playerPed)				
		TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].removed, "success")
	end, function()
		TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["nos"].remfail, "error")
		emptyHands(playerPed)
	end)
end)