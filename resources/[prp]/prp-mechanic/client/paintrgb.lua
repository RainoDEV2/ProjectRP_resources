...
RegisterNetEvent('prp-mechanic:client:RGBApply', function(data)
    ...
    spraying = false
	updateCar(vehicle)
	TriggerEvent("prp-mechanic:client:rgbORhex")
	emptyHands(ped)
end)

RegisterNetEvent('prp-mechanic:client:RGBPicker', function()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local vehicle = getClosest(coords)
	...
	if DoesEntityExist(vehicle) then
    	...
    	if dialog then
        	if not dialog.Red or not dialog.Green or not dialog.Blue then return TriggerEvent("prp-mechanic:client:RGBPicker") end
        	...
        	TriggerEvent('prp-mechanic:client:RGBApply', data)
		end
    end
end)

RegisterNetEvent('prp-mechanic:client:HEXPicker', function()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local vehicle = getClosest(coords)
        	...
        	inputs = {            
				...
				{ type = 'radio', name = 'finish', text = Loc[Config.Lan]["paintrgb"].finish,
					options = { { value = "147", text = Loc[Config.Lan]["paint"].classic },
								...
								{ value = "120", text = Loc[Config.Lan]["paintrgb"].chrome } } } } })
    	if dialog then
        	if not dialog.hex then return TriggerEvent("prp-mechanic:client:HEXPicker") end
			if dialog.hex then
				if string.len(dialog.hex:gsub("#","")) ~= 6 then
					TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["paintrgb"].error, "error")
					return TriggerEvent("prp-mechanic:client:HEXPicker")
				end
			end
			local data = { finish = tonumber(dialog.finish), 
					   		select = dialog.select,
...
RegisterNetEvent('prp-mechanic:client:rgbORhex', function()
	local ped = PlayerPedId()
	local coords = GetEntityCoords(ped)
	local vehicle = getClosest(coords)
	...
	if DoesEntityExist(vehicle) then
		...
		PaintMenu[#PaintMenu+1] = { header = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Paints:Check" } }			 
		PaintMenu[#PaintMenu+1] = { header = Loc[Config.Lan]["paintrgb"].hex, params = { event = "prp-mechanic:client:HEXPicker" }, }
		PaintMenu[#PaintMenu+1] = { header = Loc[Config.Lan]["paintrgb"].rgb, params = { event = "prp-mechanic:client:RGBPicker" }, }
