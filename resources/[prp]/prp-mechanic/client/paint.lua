...
--========================================================== Paint
RegisterNetEvent('prp-mechanic:client:Paints:Apply', function(data)
	...
	if colourCheck == data.id then
		TriggerEvent('ProjectRP:Notify', data.finish.." "..data.name..Loc[Config.Lan]["common"].already, "error")
		TriggerEvent('prp-mechanic:client:Paints:Choose:Paint', data)
	elseif colourCheck ~= data.id then
		...
		ProjectRP.Functions.Progressbar("drink_something", Loc[Config.Lan]["common"].installing..data.paint.." "..data.finish.." "..data.name, time, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false, },
		...
		{}, {}, function()  
			...
			spraying = false
			updateCar(vehicle)
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["paint"].installed, "success")
			TriggerEvent('prp-mechanic:client:Paints:Choose:Paint', data)
			emptyHands(playerPed)
			end, function() -- Cancel
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["paint"].failed, "error")
			spraying = false
			TriggerEvent('prp-mechanic:client:Paints:Choose:Paint', data)
			emptyHands(playerPed)
		end)
...
RegisterNetEvent('prp-mechanic:client:Paints:Check', function()
	...
	if type(dashboardColor) == "number" then dashboardColor = Loc[Config.Lan]["common"].stock end
		...
		local PaintMenu = {	
			...
			if not IsPedInAnyVehicle(playerPed, false) then
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["xenons"].customheader, params = { event = "prp-mechanic:client:rgbORhex" } }
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].primary, txt = Loc[Config.Lan]["common"].current..": "..vehPrimaryColour, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].primary } }
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].secondary, txt = Loc[Config.Lan]["common"].current..": "..vehSecondaryColour, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].secondary } }
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].pearl, txt = Loc[Config.Lan]["common"].current..": "..vehPearlescentColour, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].pearl } }
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].wheel, txt = Loc[Config.Lan]["common"].current..": "..vehWheelColour, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].wheel } }
			else
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].interior, txt = Loc[Config.Lan]["common"].current..": "..interiorColor, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].interior } }
				PaintMenu[#PaintMenu + 1] = { header = Loc[Config.Lan]["paint"].dashboard, txt = Loc[Config.Lan]["common"].current..": "..dashboardColor, params = { event = "prp-mechanic:client:Paints:Choose", args = Loc[Config.Lan]["paint"].dashboard } }
			end
		exports['prp-menu']:openMenu(PaintMenu)
	end
end)

RegisterNetEvent('prp-mechanic:client:Paints:Choose', function(data)
	if Config.CosmeticsJob then if not jobChecks() then return end end
	local playerPed	= PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	if not nearPoint(coords) then return end
	...
	if DoesEntityExist(vehicle) then
		exports['prp-menu']:openMenu({
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Paints:Check" } },
			{ header = Loc[Config.Lan]["paint"].classic, txt = "", params = { event = "prp-mechanic:client:Paints:Choose:Paint", args = { paint = data, finish = Loc[Config.Lan]["paint"].classic } } },
			{ header = Loc[Config.Lan]["paint"].metallic, txt = "", params = { event = "prp-mechanic:client:Paints:Choose:Paint", args = { paint = data, finish = Loc[Config.Lan]["paint"].metallic } } },
			{ header = Loc[Config.Lan]["paint"].matte, txt = "", params = { event = "prp-mechanic:client:Paints:Choose:Paint", args = { paint = data, finish = Loc[Config.Lan]["paint"].matte } } },
			{ header = Loc[Config.Lan]["paint"].metals, txt = "", params = { event = "prp-mechanic:client:Paints:Choose:Paint", args = { paint = data, finish = Loc[Config.Lan]["paint"].metals } } },
		})
	end
end)

RegisterNetEvent('prp-mechanic:client:Paints:Choose:Paint', function(data)
	local playerPed	= PlayerPedId()
    local coords = GetEntityCoords(playerPed)
	if not nearPoint(coords) then return end
	local vehicle = nil
	...
	if DoesEntityExist(vehicle) then
		local PaintMenu = {	
			...
			{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Paints:Choose", args = data.paint } } }
		local installed = nil
		if data.finish == Loc[Config.Lan]["paint"].classic then
			for k, v in pairs(Loc[Config.Lan].vehicleResprayOptionsClassic) do
				...
				PaintMenu[#PaintMenu + 1] = { header = k..". "..v.name, txt = installed, params = { event = 'prp-mechanic:client:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } } } end
