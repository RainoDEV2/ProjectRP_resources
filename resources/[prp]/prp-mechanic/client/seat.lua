...
--========================================================== Seat
RegisterNetEvent('prp-mechanic:client:Seat:Apply', function(mod)
	...
	if GetVehicleMod(vehicle, 32) == tonumber(mod) then TriggerEvent('ProjectRP:Notify', modName..Loc[Config.Lan]["common"].already, "error") TriggerEvent('prp-mechanic:client:Seat:Check')
	elseif GetVehicleMod(vehicle, 32) ~= tonumber(mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function()
			...
			updateCar(vehicle)
			TriggerEvent('prp-mechanic:client:Seat:Check')
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["seat"].installed, "success")
		end, function() -- Cancel
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["seat"].failed, "error")
			emptyHands(playerPed)
		end)
	end
end)

RegisterNetEvent('prp-mechanic:client:Seat:Check', function()
	...
	if DoesEntityExist(vehicle) then
		if GetNumVehicleMods(vehicle, 32) == 0 then	TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["common"].noOptions, "error") return end				
		...
		local SeatMenu = {
				...
				{ header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "prp-mechanic:client:Seat:Apply", args = -1 } } }
			for k,v in pairs(validMods) do
				SeatMenu[#SeatMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Seat:Apply', args = tostring(v.id) } }
			end
		exports['prp-menu']:openMenu(SeatMenu)
	end
end)