...
--========================================================== Interior
RegisterNetEvent('prp-mechanic:client:Interior:Apply', function(data)
	...
	if GetVehicleMod(vehicle, tonumber(data.bumperid)) == tonumber(data.mod) then
		...
		TriggerEvent('prp-mechanic:client:Interior:Choose', tonumber(data.bumperid))
	elseif GetVehicleMod(vehicle, tonumber(data.bumperid)) ~= tonumber(data.mod) then
		...
		{ animDict = "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", anim = "machinic_loop_mechandplayer", flags = 16, }, {}, {}, function()
			...
			updateCar(vehicle)
			TriggerEvent('prp-mechanic:client:Interior:Choose', tonumber(data.bumperid))
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["interior"].installed, "success")
		end, function() -- Cancel
			TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["interior"].failed, "error")
			emptyHands(playerPed)
		end)
	end
end)

RegisterNetEvent('prp-mechanic:client:Interior:Check', function()
	...
	if DoesEntityExist(vehicle) then		
		...
		if internal ~= true then TriggerEvent("ProjectRP:Notify", Loc[Config.Lan]["common"].noOptions, "error") return end
		...
		if GetNumVehicleMods(vehicle, 5) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label1, txt = "["..(GetNumVehicleMods(vehicle, 5)+1)..Loc[Config.Lan]["common"].menuinstalled..installed1, params = { event = "prp-mechanic:client:Interior:Choose", args = 5 } } end
		if GetNumVehicleMods(vehicle, 28) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label2, txt = "["..(GetNumVehicleMods(vehicle, 28)+1)..Loc[Config.Lan]["common"].menuinstalled..installed2, params = { event = "prp-mechanic:client:Interior:Choose", args = 28 } } end
		if GetNumVehicleMods(vehicle, 29) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label3, txt = "["..(GetNumVehicleMods(vehicle, 29)+1)..Loc[Config.Lan]["common"].menuinstalled..installed3, params = { event = "prp-mechanic:client:Interior:Choose", args = 29 } } end
		if GetNumVehicleMods(vehicle, 30) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label4, txt = "["..(GetNumVehicleMods(vehicle, 30)+1)..Loc[Config.Lan]["common"].menuinstalled..installed4, params = { event = "prp-mechanic:client:Interior:Choose", args = 30 } } end
		if GetNumVehicleMods(vehicle, 31) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label5, txt = "["..(GetNumVehicleMods(vehicle, 31)+1)..Loc[Config.Lan]["common"].menuinstalled..installed5, params = { event = "prp-mechanic:client:Interior:Choose", args = 31 } } end
		if GetNumVehicleMods(vehicle, 33) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label6, txt = "["..(GetNumVehicleMods(vehicle, 33)+1)..Loc[Config.Lan]["common"].menuinstalled..installed6, params = { event = "prp-mechanic:client:Interior:Choose", args = 33 } } end
		if GetNumVehicleMods(vehicle, 34) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label7, txt = "["..(GetNumVehicleMods(vehicle, 34)+1)..Loc[Config.Lan]["common"].menuinstalled..installed7, params = { event = "prp-mechanic:client:Interior:Choose", args = 34 } } end
		if GetNumVehicleMods(vehicle, 35) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label8, txt = "["..(GetNumVehicleMods(vehicle, 35)+1)..Loc[Config.Lan]["common"].menuinstalled..installed8, params = { event = "prp-mechanic:client:Interior:Choose", args = 35 } } end
		if GetNumVehicleMods(vehicle, 36) ~= 0 then InteriorMenu[#InteriorMenu + 1] = { header = Loc[Config.Lan]["interior"].label9, txt = "["..(GetNumVehicleMods(vehicle, 36)+1)..Loc[Config.Lan]["common"].menuinstalled..installed9, params = { event = "prp-mechanic:client:Interior:Choose", args = 36 } } end		
		exports['prp-menu']:openMenu(InteriorMenu)
	end
end)

RegisterNetEvent('prp-mechanic:client:Interior:Choose', function(mod)
	local playerPed	= PlayerPedId()
    local coords = GetEntityCoords(playerPed)
	local validMods = { }
	if not outCar() then return end
	...
	if DoesEntityExist(vehicle) then		
		...
		local ModMenu = {
				...
				{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "prp-mechanic:client:Interior:Check" } },
				{ header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "prp-mechanic:client:Interior:Apply", args = { mod = -1, bumperid = tonumber(mod) } } }}
			for k,v in pairs(validMods) do
				ModMenu[#ModMenu + 1] = { header = k..". "..v.name, txt = v.install, params = { event = 'prp-mechanic:client:Interior:Apply', args = { mod = v.id, bumperid = mod } } }
			end
