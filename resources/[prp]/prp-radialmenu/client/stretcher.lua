...
RegisterNetEvent('prp-radialmenu:client:TakeStretcher')
AddEventHandler('prp-radialmenu:client:TakeStretcher', function()
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if Vehicle ~= 0 then
        ...
        if Obj ~= nil or Obj ~= 0 then
            ...
            ProjectRP.Functions.Notify("Something went wrong..", 'error')
        end
    else
        ProjectRP.Functions.Notify("You're not near an Ambulance..", 'error')
    end
end)

RegisterNetEvent('prp-radialmenu:client:RemoveStretcher')
AddEventHandler('prp-radialmenu:client:RemoveStretcher', function()
    local PlayerPed = PlayerPedId()
    ...
    if StretcherObject ~= nil then
        ...
        if Dist < 3.0 then
            if DoesEntityExist(StretcherObject) then
                ...
                TriggerServerEvent('prp-radialmenu:server:RemoveStretcher', PlayerPos, StretcherObject)
                IsAttached = false
                StretcherObject = nil
                IsLayingOnBed = false
            end
        else
            ProjectRP.Functions.Notify('You\'re too far away!', 'error')
        end
    end
end)
...
RegisterNetEvent('prp-radialmenu:client:RemoveStretcherFromArea')
AddEventHandler('prp-radialmenu:client:RemoveStretcherFromArea', function(PlayerPos, BObject)
    local Ped = PlayerPedId()
...
function LayOnStretcher()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('prp-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(player), "lay")
        else
            LoadAnim(inBedDicts)
            if Object ~= nil or Object ~= 0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
RegisterNetEvent('prp-radialmenu:Stretcher:client:BusyCheck')
AddEventHandler('prp-radialmenu:Stretcher:client:BusyCheck', function(OtherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
        ...
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('prp-radialmenu:server:BusyResult', true, OtherId, type)
        else
            TriggerServerEvent('prp-radialmenu:server:BusyResult', false, OtherId, type)
        end
    else
...
RegisterNetEvent('prp-radialmenu:client:Result')
AddEventHandler('prp-radialmenu:client:Result', function(IsBusy, type)
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    local PlayerPed = PlayerPedId()
    local PlayerPos = GetEntityCoords(PlayerPed)
    ...
    if type == "lay" then
        if not IsBusy then
            ...
            ProjectRP.Functions.Notify("This stretcher is already in use!", "error")
            IsLayingOnBed = false
        end
    else
        if not IsBusy then
...
function AttachToStretcher()
    ...
    if StretcherObject ~= nil then
        if ClosestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('prp-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(ClosestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(StretcherObject)
                LoadAnim("anim@heists@box_carry@")
                TaskPlayAnim(PlayerPed, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
