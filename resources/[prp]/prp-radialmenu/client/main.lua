...
AddEventHandler('prp-radialmenu:client:noPlayers', function(data)
    ProjectRP.Functions.Notify('There arent any people close', 'error', 2500)
end)
...
AddEventHandler('prp-radialmenu:client:openDoor', function(data)
    ...
    if closestVehicle ~= 0 then
        ...
        ProjectRP.Functions.Notify('There is no vehicle in sight...', 'error', 2500)
    end
end)
        ...
        if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
            ...
            if DoesExtraExist(veh, extra) then
                if IsVehicleExtraTurnedOn(veh, extra) then
                    ...
                    ProjectRP.Functions.Notify('Extra ' .. extra .. ' Deactivated', 'error', 2500)
                else
                    SetVehicleExtra(veh, extra, 0)
                    ProjectRP.Functions.Notify('Extra ' .. extra .. ' Activated', 'success', 2500)
                end
            else
                ProjectRP.Functions.Notify('Extra ' .. extra .. ' is not present on this vehicle ', 'error', 2500)
            end
        else
            ProjectRP.Functions.Notify('You\'re not a driver of a vehicle!', 'error', 2500)
        end
    end
end)
...
AddEventHandler('prp-radialmenu:trunk:client:Door', function(plate, door, open)
    ...
    if veh ~= 0 then
        local pl = ProjectRP.Functions.GetPlate(veh)
    ...
    if not HasHarnass then
        ...
        if IsSeatFree then
            if kmh <= 100.0 then
                ...
                ProjectRP.Functions.Notify('You are now on the  '..data.title..'!')
            else
                ProjectRP.Functions.Notify('This vehicle is going too fast..')
            end
        else
            ProjectRP.Functions.Notify('This seat is occupied..')
        end
    else
        ProjectRP.Functions.Notify('You have a race harness on you cant switch..', 'error')
    end
end)
