...
function initItem(source, item)
    ...
    item.info.hash = tostring(ProjectRP.Shared.RandomInt(2) .. ProjectRP.Shared.RandomStr(3) .. ProjectRP.Shared.RandomInt(1) ..
        ProjectRP.Shared.RandomStr(2) .. ProjectRP.Shared.RandomInt(3) .. ProjectRP.Shared.RandomStr(4))
    ...
    if Config.Settings.let_players_cutomize_their_pet_after_purchase then
        ...
        TriggerClientEvent('prp-companion:client:initialization_process', src, item, information)
    end
end
...
RegisterNetEvent('prp-companion:server:compelete_initialization_process', function(item, process_type)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    if not Player then return end

    TriggerEvent('prp-companion:server:prp-companion:server:compelete_initialization_process_last_step', source, item, Player, process_type)
    if process_type == 'init' then return end
    Player.Functions.RemoveItem(Config.core_items.groomingkit.item_name, 1)
end)

RegisterNetEvent('prp-companion:server:prp-companion:server:compelete_initialization_process_last_step', function(src, item, Player, process_type)
    local pet_information = find_pet_model_by_item_name(item.name)
    ...
    if process_type == Config.core_items.groomingkit.item_name then
        ...
        if Player.PlayerData.charinfo.phone ~= petData.info.owner.phone then
            TriggerClientEvent('ProjectRP:Notify', src, Lang:t('error.not_owner_of_pet'), 'error', 2500)
            return
        end
    ...
    if process_type == Config.core_items.groomingkit.item_name then
        TriggerClientEvent('ProjectRP:Notify', src, Lang:t('success.successful_grooming'), 'success', 2500)
...
function Update:xp(source, current_pet_data)
    ...
    if current_pet_data.info.XP > current_level_max_xp(level) then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, msg)
    end
end
...
function Update:food(petData, process_type)
    ...
    --         }
    --     end
    --     TriggerClientEvent('ProjectRP:Notify', source, "Pet food value increased too: " .. mData.content)
    --     return
    -- end
...
ProjectRP.Functions.CreateCallback('prp-companion:server:collar_change_owenership', function(source, cb, data)
    if type(data.new_owner_cid) == "string" then data.new_owner_cid = tonumber(data.new_owner_cid) end
    local player_owner = ProjectRP.Functions.GetPlayer(source)
    if player_owner == nil then return end
    local player_new_owner = ProjectRP.Functions.GetPlayer(data.new_owner_cid)
...
function PlayersCooldown:cleanOflinePlayers()
    local onlinePlayers = ProjectRP.Functions.GetPlayers()
