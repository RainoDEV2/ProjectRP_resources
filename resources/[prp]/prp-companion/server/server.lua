...
function Pet:spawnPet(source, model, item)
    ...
    if limit == true then
        TriggerClientEvent('ProjectRP:Notify', source, string.format(Lang:t('error.reached_max_allowed_pet'), maxLimit), 'error', 2500)
        return
    end
    ...
    TriggerClientEvent('prp-companion:client:callCompanion', source, model, owner, item)
end

RegisterNetEvent('prp-companion:server:despwan_not_owned_pet', function(hash)
...
function Pet:despawnPet(source, item, revive)
    ...
    TriggerClientEvent('prp-companion:client:despawn', source, item, revive)
end
...
function Pet:save_all_info(source, hash)
    ...
    if petData.info.health > 100 then
        ...
        TriggerClientEvent('prp-companion:client:forceKill', source, hash, 'hunger')
    end
...
RegisterNetEvent('prp-companion:server:setAsDespawned', function(item)
...
-- food
ProjectRP.Functions.CreateUseableItem(core_items.food.item_name, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('prp-companion:client:start_feeding_animation', source)
end)

RegisterNetEvent('prp-companion:server:increaseFood', function(item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    if Player.Functions.RemoveItem('petfood', 1) ~= true then
        TriggerClientEvent('ProjectRP:Notify', source, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    local petData = Pet:findbyhash(source, item.info.hash)
    petData.info.food = petData.info.food + 50
    TriggerClientEvent('ProjectRP:Notify', source, 'Feeding was successful wait little bit to take effect!', 'success', 2500)
end)

-- change owenership
ProjectRP.Functions.CreateUseableItem(core_items.collar.item_name, function(source, item)
    TriggerClientEvent('prp-companion:client:collar_process', source)
end)

-- rename - name tag
ProjectRP.Functions.CreateUseableItem(core_items.nametag.item_name, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil or item == nil then return end
    TriggerClientEvent('prp-companion:client:rename_name_tag', source, item)
end)

RegisterNetEvent('prp-companion:server:rename_name_tag', function(name)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil then return end

    if Player.Functions.RemoveItem("petnametag", 1) ~= true then
        TriggerClientEvent('ProjectRP:Notify', source, Lang:t('error.failed_to_remove_item_from_inventory'), 'error', 2500)
        return
    end

    TriggerClientEvent("prp-companion:client:rename_name_tagAction", source, name)
end)

-- first aid - revive
ProjectRP.Functions.CreateUseableItem(core_items.firstaid.item_name, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil then return end

end)

ProjectRP.Functions.CreateUseableItem(core_items.groomingkit.item_name, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if Player == nil then return end
    TriggerClientEvent('prp-companion:client:start_grooming_process', source)
end)

RegisterNetEvent('prp-companion:server:grooming_process', function(item)
    ...
    TriggerClientEvent('prp-companion:client:initialization_process', source, item, information)
end)
...
local function fillwater_bottle(source, item)
    ...
    if type(item.info) == "table" then
        if item.info.liter > max_c then
            TriggerClientEvent('ProjectRP:Notify', source, 'could not do that already reached max capacity', 'error', 2500)
            return
        elseif item.info.liter == max_c then
            amount = max_c

            TriggerClientEvent('ProjectRP:Notify', source, 'filling already filled bottle has no effect on capacity', 'error', 2500)
        else
        ...
        TriggerClientEvent('ProjectRP:Notify', source, 'Filled bottle', 'success', 2500)
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'Washing new water bottle!', 'primary', 2500)
    end
    save_info_waterbottle(Player, item, amount)
end

ProjectRP.Functions.CreateUseableItem(core_items.waterbottle.item_name, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    local water_bottle_refill_value = Config.core_items.waterbottle.settings.water_bottle_refill_value
    if Player == nil then return end
    if Player.Functions.RemoveItem('water_bottle', water_bottle_refill_value) ~= true then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, Lang:t('error.failed_to_remove_item_from_inventory'), 'error', 2500)
        TriggerClientEvent('ProjectRP:Notify', source, msg, 'error', 2500)
        return
    end
    TriggerClientEvent('prp-companion:client:filling_animation', source, item)
end)

RegisterNetEvent('prp-companion:server:filling_event', function(item)
    fillwater_bottle(source, item)
end)

ProjectRP.Functions.CreateCallback('prp-companion:server:decrease_thirst', function(source, cb, data)
    local player = ProjectRP.Functions.GetPlayer(source)
    ...
    if pet_water_bottle.info == nil then
        TriggerClientEvent('ProjectRP:Notify', source, 'You should wash water bottle first!', 'error', 2500)
        return
    end
    ...
    if pet_water_bottle.info.liter < 0 then
        TriggerClientEvent('ProjectRP:Notify', source, Lang:t('error.not_enough_water_in_your_bottle'), 'error', 2500)
        return
    end
    ...
    TriggerClientEvent('ProjectRP:Notify', source, Lang:t('success.successful_drinking'), 'success', 2500)
    save_info_waterbottle(player, pet_water_bottle, pet_water_bottle.info.liter)
end)

RegisterNetEvent('prp-companion:server:revivePet', function(item, process_type)
    local src = source
    ...
    if not petData then
        TriggerClientEvent('ProjectRP:Notify', src, Lang:t('error.failed_to_start_procces') .. process_type, 'primary', 2500)
        return
    end
    ...
    if petData.info.health >= pet_maxHealth then
        ...
        TriggerClientEvent('ProjectRP:Notify', src, Lang:t('info.full_life_pet'), 'primary', 2500)
        return
    end

    if not Player.Functions.RemoveItem("firstaidforpet", 1) then
        TriggerClientEvent('ProjectRP:Notify', src, 'Failed to remove from your inventory', 'error', 2500)
        return
    end
    ...
    if process_type and process_type == 'Heal' then
        ...
        TriggerClientEvent('prp-companion:client:update_health_value', src, item, petData.info.health)
        TriggerClientEvent('ProjectRP:Notify', src, msg, 'success', 2500)
        return
    end
...
-- all pets
for key, value in pairs(Config.pets) do
    ProjectRP.Functions.CreateUseableItem(value.name, function(source, item)
        ...
        if type(item.info) ~= "table" or (type(item.info) == "table" and item.info.hash == nil) then
            ...
            TriggerClientEvent('ProjectRP:Notify', source, Lang:t('success.pet_initialization_was_successful'), 'success', 2500)
            return
        end
        ...
        if cooldown > 0 then
            ...
            TriggerClientEvent('ProjectRP:Notify', source, msg, 'primary', 2500)
            return
        end
...
function FindWhereIsItem(Player, item, source)
    if Player.PlayerData.items == nil or next(Player.PlayerData.items) == nil then
        TriggerClientEvent('ProjectRP:Notify', source, "no items in inventory!")
        return false
    end
    ...
    TriggerClientEvent('ProjectRP:Notify', source, "Could not find pet")
    return false
end

RegisterNetEvent('prp-companion:server:updateAllowedInfo', function(item, data)
...
ProjectRP.Functions.CreateCallback('prp-companion:server:renamePet', function(source, cb, item)
    local player = ProjectRP.Functions.GetPlayer(source)
    local current_pet_data = Pet:findbyhash(source, item.hash)
    ...
    if player == nil or current_pet_data == nil or current_pet_data == false or type(item.name) ~= "string" then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, msg, 'error')
        cb(false)
        return
    end
...
ProjectRP.Functions.CreateCallback('prp-companion:server:updatePedData', function(source, cb, clientRes)
    local player = ProjectRP.Functions.GetPlayer(source)
    if player == nil then
        cb(false)
        return
...
RegisterNetEvent('prp-companion:server:onPlayerUnload', function(items)
...
ProjectRP.Commands.Add('addpet', 'add a pet to player inventory (Admin Only)', {}, false, function(source, args)
    local PETname = args[1]
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)

    Player.Functions.AddItem(PETname, 1)
    TriggerClientEvent("inventory:client:ItemBox", src, ProjectRP.Shared.Items[PETname], "add")
end, 'admin')

ProjectRP.Commands.Add('addItem', 'add item to player inventory (Admin Only)', {}, false, function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)
    Player.Functions.AddItem(item[1], 1)
    TriggerClientEvent("inventory:client:ItemBox", source, ProjectRP.Shared.Items[item[1]], "add")
end, 'admin')

ProjectRP.Commands.Add('renamePet', 'rename pet', { { "name", "new pet name" } }, false, function(source, args)
    TriggerClientEvent("prp-companion:client:rename_name_tag", source, args[1])
end, 'admin')
...
RegisterNetEvent('prp-companion:server:ForceRemoveNetEntity', function(netId)
