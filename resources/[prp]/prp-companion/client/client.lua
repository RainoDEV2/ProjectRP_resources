local CoreName = exports['prp-core']:GetCoreObject()
...
function ActivePed:remove(index)
    ...
    TriggerServerEvent('prp-companion:server:ForceRemoveNetEntity', netId)
    self.data[index] = nil
...
function ActivePed:removeAll()
    ...
    for key, value in pairs(ActivePed:petsList()) do
        ...
        TriggerServerEvent('prp-companion:server:updateAllowedInfo', currentItem, {
            key = 'XP',
        })
    end
    TriggerServerEvent('prp-companion:server:onPlayerUnload', tmpHash)
    self.data = {}
    self.onControl = -1
end
...
RegisterNetEvent('prp-companion:client:callCompanion')
AddEventHandler('prp-companion:client:callCompanion', function(modelName, hostileTowardPlayer, item)
    ...
    CoreName.Functions.Progressbar("callCompanion", "Calling companion", Config.Settings.callCompanionDuration * 1000,
        ...
        ProjectRP.Functions.TriggerCallback('prp-companion:server:updatePedData', function(result)
            if hostileTowardPlayer == true then
                -- if player is not owner of pet it will attack player
                ProjectRP.Functions.Notify(Lang:t('error.not_owner_of_pet'), 'error', 5000)
            end
            ClearPedTasks(ped)
            ...
            exports['prp-target']:AddTargetEntity(ped, {
                options = { {
            ...
            if petData.hostile == true then
                TriggerServerEvent('prp-companion:server:despwan_not_owned_pet', petData.itemData.info.hash)
                return
            end
...
function request_healing_process(ped, item, process_type)
    ProjectRP.Functions.TriggerCallback("ProjectRP:HasItem", function(hasitem)
        if not hasitem then ProjectRP.Functions.Notify(Lang:t('error.not_enough_first_aid'), 'error', 5000) return end
        ...
        CoreName.Functions.Progressbar("reviveing", "Reviveing",
            ...
            TriggerServerEvent('prp-companion:server:revivePet', current_pet, process_type)
            TaskFollowTargetedPlayer(ped, plyID, false)
        end)
    end, Config.core_items.firstaid.item_name)
end

RegisterNetEvent('prp-companion:client:update_health_value', function(item, amount)
    ...
    CreateThread(function()
        ...
        while DoesEntityExist(ped) and fninished == false do
            ...
            if IsPedDeadOrDying(savedData.entity) == false and savedData.maxHealth ~= currentHealth and savedData.health ~=
                currentHealth then
                -- ped is still alive
                TriggerServerEvent('prp-companion:server:updateAllowedInfo', {
...
RegisterNetEvent('prp-companion:client:forceKill', function(hash, reason)
...
RegisterNetEvent('prp-companion:client:despawn')
AddEventHandler('prp-companion:client:despawn', function(item, revive)
    if revive ~= nil and revive == true then
        ...
        TriggerServerEvent('prp-companion:server:setAsDespawned', item)
        return
    end
    local plyPed = PlayerPedId()
...
RegisterNetEvent('prp-companion:client:start_feeding_animation', function()
    ...
    if c_health <= 100.0 or current_pet.itemData.info.health <= 100.0 then
        ProjectRP.Functions.Notify(Lang:t('error.your_pet_is_dead'), 'error', 5000)
        return
    end
    ...
    CoreName.Functions.Progressbar("feeding", "Feeding", Config.core_items.food.settings.duration * 1000, false, false, {
        ...
        TriggerServerEvent('prp-companion:server:increaseFood', current_pet.itemData)
    end)
end)

RegisterNetEvent('prp-companion:client:', function()
...
function start_drinking_animation()
    ...
    CoreName.Functions.Progressbar("pet_drinking", "drinking", Config.core_items.waterbottle.settings.duration * 1000, false, false, {
        ...
        ProjectRP.Functions.TriggerCallback('prp-companion:server:decrease_thirst', function(result)

        end, current_pet.itemData)
    end)
end

RegisterNetEvent('prp-companion:client:filling_animation', function(item)
    CoreName.Functions.Progressbar("filling_animation", "filling bottle", Config.core_items.waterbottle.settings.duration * 1000, false, false, {
        ...
        TriggerServerEvent('prp-companion:server:filling_event', item)
    end)
end)

RegisterNetEvent('prp-companion:client:rename_name_tag', function(item)
    if ActivePed:read() == nil then
        ProjectRP.Functions.Notify(Lang:t('error.no_pet_under_control'), 'error', 5000)
        return
    end

    local name = exports['prp-input']:ShowInput({
    ...
    if name then
        ...
        TriggerServerEvent('prp-companion:server:rename_name_tag', name.pet_name)
    end
end)



RegisterNetEvent('prp-companion:client:rename_name_tagAction', function(name)
    ...
    CoreName.Functions.Progressbar("waitingForName", "waiting for Name",
        ...
        ProjectRP.Functions.TriggerCallback('prp-companion:server:renamePet', function(result)
            if type(result) == "string" then
                ProjectRP.Functions.Notify(Lang:t('success.pet_rename_was_successful') .. result, 'success', 5000)
            end
        end, {
...
RegisterNetEvent('prp-companion:client:collar_process', function()
    ...
    if activePed.itemData.info.hash == nil then
        ProjectRP.Functions.Notify(Lang:t('error.failed_to_find_pet'), 'error', 5000)
        return
    end
    ...
    if inputData then
        ...
        CoreName.Functions.Progressbar("waitingForOwenership", "waiting for new owner",
            ...
            function()
                ...
                ProjectRP.Functions.TriggerCallback('prp-companion:server:collar_change_owenership', function(result)
                    if result.state == false then
                        ProjectRP.Functions.Notify(result.msg, 'error', 5000)
                        return
                    end
                    ProjectRP.Functions.Notify(result.msg, 'success', 5000)
                end, {
