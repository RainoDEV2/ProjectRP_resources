...
function getIntoCar()
    ...
    if not IsPedSittingInAnyVehicle(plyped) then
        ProjectRP.Functions.Notify(Lang:t('error.need_to_be_inside_car'), "error", 1500)
        return
    end
    if distance > 8 then
        ProjectRP.Functions.Notify(Lang:t('error.to_far'), "error", 1500)
        return
    end
    ...
    if seatEmpty == 6 then
        ProjectRP.Functions.Notify(Lang:t('error.no_empty_seat'), "error", 1500)
        return
    end
end
    ...
    while true do
        ...
        if IsControlJustReleased(0, 38) then
            ...
            if IsPedAPlayer(entity) == 1 or IsEntityAPed(entity) == false or entity == pet then
                ProjectRP.Functions.Notify(Lang:t('error.could_not_do_that'), "error", 1500)
                return
            end
