$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://prp-burgershot/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://prp-burgershot/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://prp-burgershot/CloseNui', JSON.stringify({}))
        ...
        $.post('http://prp-burgershot/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://prp-burgershot/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://prp-burgershot/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
