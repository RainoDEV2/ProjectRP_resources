...
CreateThread(function()
    ...
    for k,v in pairs(food) do ProjectRP.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('prp-catcafe:client:Eat', source, item.name) end) end
	
	local drinks = { "bobatea", "bbobatea", "gbobatea", "pbobatea", "obobatea", "nekolatte", "mocha" }
    for k,v in pairs(drinks) do ProjectRP.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('prp-catcafe:client:Drink', source, item.name) end) end
	
	local alcohol = { "sake" }
    for k,v in pairs(alcohol) do ProjectRP.Functions.CreateUseableItem(v, function(source, item) TriggerClientEvent('prp-catcafe:client:DrinkAlcohol', source, item.name) end) end
end)

RegisterServerEvent('prp-catcafe:GetFood', function(data)
	local src = source
	...
	if data.craftable ~= nil then
		for k, v in pairs(data.craftable[tonumber(data.tablenumber)][tostring(data.item)]) do
			...
			if v == 0 or v == nil then else
				TriggerClientEvent('inventory:client:ItemBox', src, ProjectRP.Shared.Items[tostring(k)], "remove", v) 
	...
	TriggerClientEvent('inventory:client:ItemBox', src, ProjectRP.Shared.Items[data.item], "add", amount)
	
	if Config.Debug then print("Giving ["..src.."]: x"..amount.." "..data.item) end		
end)

---ITEM REQUIREMENT CHECKS
ProjectRP.Functions.CreateCallback('prp-catcafe:get', function(source, cb, item, tablenumber, craftable)
	local src = source
	local hasitem = nil
	local hasanyitem = nil
		for k, v in pairs(craftable[tonumber(tablenumber)][tostring(item)]) do
			if ProjectRP.Functions.GetPlayer(src).Functions.GetItemByName(k) and ProjectRP.Functions.GetPlayer(src).Functions.GetItemByName(k).amount >= v then 
				hasitem = true
				number = tostring(ProjectRP.Functions.GetPlayer(src).Functions.GetItemByName(k).amount)
			else
				hasitem = false 
				hasanyitem = false
				number = "0"
