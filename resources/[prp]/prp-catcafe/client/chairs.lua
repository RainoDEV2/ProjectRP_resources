...
Citizen.CreateThread(function()
	for k, v in pairs(Config.Chairs) do
		exports['prp-target']:AddBoxZone("CatChair"..k, v.location, v.width, v.height, { name="CatChair"..k, heading = v.heading, debugPoly=Config.Debug, minZ = v.minZ, maxZ = v.maxZ, }, 
			{ options = { { event = "prp-catcafe:Chair", icon = "fas fa-chair", label = "Sit Down", loc = v.location, head = v.heading, seat = v.seat }, },
			  distance = v.distance
		})
	end
end)

RegisterNetEvent('prp-catcafe:Chair', function(data)
	local canSit = true
	local sitting = false
	for k, v in pairs(ProjectRP.Functions.GetPlayersFromCoords(data.loc, 0.6)) do
		local dist = #(GetEntityCoords(GetPlayerPed(v)) - data.loc)
		if dist <= 0.4 then TriggerEvent("ProjectRP:Notify", "Someone is already sitting there..") canSit = false end
	end
	if canSit then
...
AddEventHandler('onResourceStop', function(r) 
	if r == GetCurrentResourceName() then for k, v in pairs(Config.Chairs) do exports['prp-target']:RemoveZone("CatChair"..k) end
	end 
end)