...
Citizen.CreateThread(function()
	for k, v in pairs(PedList) do
		if v.clickable then
			exports['prp-target']:AddBoxZone("CatPat"..k, vector3(v.coords.x, v.coords.y, v.coords.z-0.3), 0.7, 0.7, { name="CatPat"..k, heading = 0, debugPoly=Config.Debug, minZ= v.coords.z-1.5, maxZ=v.coords.z-0.5 }, 
				{ options = { { event = "prp-catcafe:CatPat", icon = "fas fa-paw", label = "Pet the Cat" }, },
				  distance = 1.5
			})
		end
	end
...
RegisterNetEvent('prp-catcafe:CatPat')
AddEventHandler('prp-catcafe:CatPat', function()
	...
	for k,v in pairs (shopPeds) do
		...
		if dist < 2 then
			...
			--TriggerEvent("ProjectRP:Notify", ppCoords.z, "error")
...
AddEventHandler('onResourceStop', function(r) if r == GetCurrentResourceName() then for k, v in pairs(PedList) do exports['prp-target']:RemoveZone("CatPat"..k) end end end)