...
RegisterNetEvent('prp-drugs:client:cornerselling', function(data)
    ProjectRP.Functions.TriggerCallback('prp-drugs:server:cornerselling:getAvailableDrugs', function(result)
...
RegisterNetEvent('prp-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if #availableDrugs <= 0 then
        ProjectRP.Functions.Notify('No more drugs left to sell!', 'error')
        cornerselling = false
        LocalPlayer.state:set("inv_busy", false, true)
    end
end)
...
local function toFarAway()
    ProjectRP.Functions.Notify('Moved Too Far!', 'error')
    LocalPlayer.state:set("inv_busy", false, true)
    cornerselling = false
    hasTarget = false
    busySelling = false
    ...
    if hasTarget then
        while pedDist < 1.5 do
            ...
            if getRobbed == 18 or getRobbed == 9 then
                ...
                ProjectRP.Functions.Notify('You have been robbed and lost '..bagAmount..' bags(\'s) '..availableDrugs[drugType].label, 'error')
                stealingPed = ped
                stealData = {
                ...
                if pedDist < 1.5 and cornerselling then
                    ...
                    if IsControlJustPressed(0, 47) then
                        ProjectRP.Functions.Notify('Offer canceled!', 'error')
                        hasTarget = false
            ...
            if IsEntityDead(stealingPed) then
                ...
                if #(pos - pedpos) < 1.5 then
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        Wait(2000)
                        ClearPedTasks(ped)
                        TriggerServerEvent("ProjectRP:Server:AddItem", stealData.item, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', ProjectRP.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                    end
                end
