...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function()
    ProjectRP.Functions.TriggerCallback('prp-drugs:server:RequestConfig', function(DealerConfig)
                            ...
                            if IsControlJustPressed(0, 47) then
                                if dealer["name"] == "Mystery man" then
                                    ...
                                    if player ~= -1 and distance < 5.0 then
                                        ...
                                        ProjectRP.Functions.Progressbar("hospital_revive", "Put person back on their feet..", 5000, false, true, {
                                            ...
                                            ProjectRP.Functions.Notify("You have helped a person!")
                                        ...
                                        ProjectRP.Functions.Notify("There is no one nearby..", "error")
                                    end
                                else
...
function buyDealerStuff()
    ...
    for k, v in pairs(Config.Dealers[currentDealer]["products"]) do
        if ProjectRP.Functions.GetPlayerData().metadata["dealerrep"] >= Config.Dealers[currentDealer]["products"][k].minrep then
...
function knockDoorAnim(home)
    ...
    local myData = ProjectRP.Functions.GetPlayerData()
    ...
    if home then
        ...
        knockingDoor = false
        Citizen.Wait(1000)
        ProjectRP.Functions.Notify('It looks like no one is home..', 'error', 3500)
    end
end
...
function requestDelivery()
    ...
    SetTimeout(2000, function()
        TriggerServerEvent('prp-phone:server:sendNewMail', {
            ...
            message = "Here is all info about the delivery, <br>Items: <br> "..amount.."x "..ProjectRP.Shared.Items[waitingDelivery["itemData"]["item"]]["label"].."<br><br> be on time!!",
            button = {
                enabled = true,
...
function randomDeliveryItemOnRep()
    local ped = PlayerPedId()
    local myRep = ProjectRP.Functions.GetPlayerData().metadata["dealerrep"]
...
function setMapBlip(x, y)
    SetNewWaypoint(x, y)
    ProjectRP.Functions.Notify('The route to the delivery location has been set on your map.', 'success');
end
...
AddEventHandler('prp-drugs:client:setLocation', function(locationData)
    if activeDelivery == nil then
        ...
        ProjectRP.Functions.Notify('You still have an open delivery...')
        return
    end
        ...
        while true do
            ...
            if activeDelivery ~= nil then
                ...
                if dist < 15 then
                    ...
                    if dist < 1.5 then
                        DrawText3D(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"], activeDelivery["coords"]["z"], '[E] '..activeDelivery["amount"]..'x '..ProjectRP.Shared.Items[activeDelivery["itemData"]["item"]]["label"]..' deliver.')
...
function deliverStuff(activeDelivery)
    if deliveryTimeout > 0 then
        ...
        checkPedDistance()
        ProjectRP.Functions.Progressbar("work_dropbox", "Delivering products..", 3500, false, true, {
...
function checkPedDistance()
    ...
    local closestPed, closestDistance = ProjectRP.Functions.GetClosestPed(coords, PlayerPeds)
...
AddEventHandler('prp-drugs:client:sendDeliveryMail', function(type, deliveryData)
    if type == 'perfect' then
        TriggerServerEvent('prp-phone:server:sendNewMail', {
            ...
            subject = "Delivery",
            message = "You did good work, hope to see you again ;)<br><br>Cheers, "..Config.Dealers[deliveryData["dealer"]]["name"]
        })
...
RegisterNetEvent('prp-drugs:client:CreateDealer')
AddEventHandler('prp-drugs:client:CreateDealer', function(dealerName, minTime, maxTime)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent('prp-drugs:server:CreateDealer', DealerData)
end)

RegisterNetEvent('prp-drugs:client:RefreshDealers')
AddEventHandler('prp-drugs:client:RefreshDealers', function(DealerData)
    Config.Dealers = DealerData
end)

RegisterNetEvent('prp-drugs:client:GotoDealer')
AddEventHandler('prp-drugs:client:GotoDealer', function(DealerData)
    local ped = PlayerPedId()

    SetEntityCoords(ped, DealerData["coords"]["x"], DealerData["coords"]["y"], DealerData["coords"]["z"])
    ProjectRP.Functions.Notify('You have been teleported : '.. DealerData["name"] .. ' Good luck!', 'success')
end)
