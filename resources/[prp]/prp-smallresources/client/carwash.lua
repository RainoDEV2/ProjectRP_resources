...
RegisterNetEvent('prp-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed)
    washingVehicle = true
    ProjectRP.Functions.Progressbar("search_cabin", "Vehicle is being washed ..", math.random(4000, 8000), false, true, {
        ...
        ProjectRP.Functions.Notify("Washing canceled ..", "error")
        washingVehicle = false
    end)
end)
                        ...
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    TriggerServerEvent('prp-carwash:server:washCar')
                                end
                            else
