...
local function TriggerCruiseControl()
    if CruisedSpeed == 0 and IsDriving() then
        if GetVehicleSpeed() > 0 and GetVehicleCurrentGear(GetVehicle()) > 0 then
            ...
            ProjectRP.Functions.Notify("Cruise Activated: " .. CruisedSpeedMph .." MP/H") -- Comment me for mp/h
            -- ProjectRP.Functions.Notify("Cruise Activated: " .. CruisedSpeedKm ..  " km/h") -- Uncomment me for km/h
            CreateThread(function()
                while CruisedSpeed > 0 and IsInVehicle() == Player do
                    ...
                    if not IsTurningOrHandBraking() and GetVehicleSpeed() <
                        ...
                        CruisedSpeed = 0
                        TriggerEvent('seatbelt:client:ToggleCruise')
                        ProjectRP.Functions.Notify("Cruise Deactivated", "error")
                        Wait(2000)
                        break
                    end
...
RegisterCommand('togglecruise', function()
    ...
    if IsDriver() then
        if vehicleClasses[vehClass] then
            ...
            ProjectRP.Functions.Notify("Cruise control unavailable", "error")
        end
    end
end, false)
