...
```lua
exports['prp-target']:AddBoxZone("MissionRowDutyClipboard", vector3(441.7989, -982.0529, 30.67834), 0.45, 0.35, {
...
-- This event is only for the ProjectRP resource prp-policejob, goes inside @prp-policejob/client/job.lua
...
exports['prp-target']:AddTargetModel(Config.Peds, {
	options = {
		{
...
```lua
exports['prp-target']:AddTargetEntity('mule2', {
    options = {
        {
            type = "client",
...
## Passing Item Data
In this example, we define the model of the coffee machines you see around the map, and allow players to purchase a coffee. You'll have to provide your own logic for the purchase, but this is how you would handle it with prp-target, and how you would pass data through to an event for future use. 

This is an example using **exports**
This example is **not** advised to use with the provided config

```lua
exports['prp-target']:AddTargetModel(690372739, {
    options = {
        {
            type = "client",
            event = "coffee:buy",
...
RegisterNetEvent('coffee:buy',function(data)
    ...
    ProjectRP.Functions.Notify("You purchased a " .. data.label .. " for $" .. data.price .. ". Enjoy!", 'success')
end)
```
...
AddEventHandler('plantpotato',function()
	...
	exports['prp-target']:AddEntityZone("potato-growing-"..plant, plant, {
