...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		ProjectRP.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('prp-houses:client:decorate', function()
	Citizen.Wait(500)
	if inside then
		if hasKey then
			...
			ProjectRP.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    ProjectRP.Functions.TriggerCallback('prp-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = not cursorEnabled
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				ProjectRP.Functions.Notify('You have gone out of range', 'error')
			end
		end
	end
end)
