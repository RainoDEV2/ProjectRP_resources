...
AddEventHandler('ProjectRP:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    ProjectRP.Functions.TriggerCallback('prp-scoreboard:server:GetConfig', function(config)
        Config.IllegalActions = config
    end)
end)

RegisterNetEvent('prp-scoreboard:client:SetActivityBusy')
AddEventHandler('prp-scoreboard:client:SetActivityBusy', function(activity, busy)
    Config.IllegalActions[activity].busy = busy
end)

RegisterCommand('scoreboard', function()
    if not scoreboardOpen then
        ProjectRP.Functions.TriggerCallback('prp-scoreboard:server:GetPlayersArrays', function(playerList)
            ProjectRP.Functions.TriggerCallback('prp-scoreboard:server:GetActivity', function(cops, ambulance)
                ProjectRP.Functions.TriggerCallback("prp-scoreboard:server:GetCurrentPlayers", function(Players)
