...
local function GetOnlineStatus(number)
    local Target = ProjectRP.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
function PRPPhone.AddMentionedTweet(citizenid, TweetData)
...
function PRPPhone.SetPhoneAlerts(citizenid, app, alerts)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:GetCallState', function(source, cb, ContactData)
    local Target = ProjectRP.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
ProjectRP.Functions.CreateCallback('prp-phone:server:GetPhoneData', function(source, cb)
    local src = source
    ...
    if Player ~= nil then
        ...
        if garageresult[1] ~= nil then
            for k, v in pairs(garageresult) do
                ...
                if (ProjectRP.Shared.Vehicles[vehicleModel] ~= nil) and (Garages[v.garage] ~= nil) then
                    v.garage = Garages[v.garage].label
                    v.vehicle = ProjectRP.Shared.Vehicles[vehicleModel].name
                    v.brand = ProjectRP.Shared.Vehicles[vehicleModel].brand
                end
...
ProjectRP.Functions.CreateCallback('prp-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = ProjectRP.Functions.GetPlayer(source)
    local SenderPly = ProjectRP.Functions.GetPlayerByCitizenId(sendercitizenid)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:GetContactPictures', function(source, cb, Chats)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:FetchResult', function(source, cb, search)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
ProjectRP.Functions.CreateCallback('prp-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then
        ...
        TriggerClientEvent('ProjectRP:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:CanTransferMoney', function(source, cb, amount, iban)
...
ProjectRP.Functions.CreateCallback('prp-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
...
ProjectRP.Functions.CreateCallback("prp-phone:server:GetWebhook",function(source,cb)
	if WebHook ~= "" then
		cb(WebHook)
	else
		cb(nil)
...
RegisterNetEvent('prp-phone:server:sendNewMailToOffline', function(citizenid, mailData)
    ...
    for k, v in pairs(ProjectRP.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                PRPPhone.SetPhoneAlerts(Player.PlayerData.citizenid, "twitter")
                PRPPhone.AddMentionedTweet(Player.PlayerData.citizenid, TweetMessage)
                ...
                if result[1] ~= nil then
                    ...
                    PRPPhone.AddMentionedTweet(MentionedTarget, TweetMessage)
                end
            end
        end
    end
...
RegisterNetEvent('prp-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = ProjectRP.Functions.GetPlayer(src)
    local Target = ProjectRP.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
...
RegisterNetEvent('prp-phone:server:BillingEmail', function(data, paid)
    for k, v in pairs(ProjectRP.Functions.GetPlayers()) do
        local target = ProjectRP.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. ProjectRP.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. ProjectRP.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
...
RegisterNetEvent('prp-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = ProjectRP.Functions.GetPlayer(src).citizenid
    PRPPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('prp-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('ProjectRP:Notify', src, "This account number doesn't exist!", "error")
    end
end)
...
RegisterNetEvent('prp-phone:server:AddRecentCall', function(type, data)
    ...
    local Trgt = ProjectRP.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
...
RegisterNetEvent('prp-phone:server:CancelCall', function(ContactData)
    local Ply = ProjectRP.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
...
RegisterNetEvent('prp-phone:server:AnswerCall', function(CallData)
    local Ply = ProjectRP.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
...
ProjectRP.Commands.Add("setmetadata", "Set Player Metadata (God Only)", {}, false, function(source, args)
    local Player = ProjectRP.Functions.GetPlayer(source)
    if args[1] then
...
ProjectRP.Commands.Add('bill', 'Bill A Player', {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Fine Amount'}}, false, function(source, args)
