...
$(document).on('click', '.bank-app-account', function(e){
    ...
    PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "Account number. copied!", "#badc58", 1750);
});
...
PRP.Phone.Functions.DoBankOpen = function() {
    PRP.Phone.Data.PlayerData.money.bank = (PRP.Phone.Data.PlayerData.money.bank).toFixed();
    $(".bank-app-account-number").val(PRP.Phone.Data.PlayerData.charinfo.account);
    $(".bank-app-account-balance").html("&#36; "+PRP.Phone.Data.PlayerData.money.bank);
    $(".bank-app-account-balance").data('balance', PRP.Phone.Data.PlayerData.money.bank);
    ...
    if (iban != "" && amount != "") {
            $.post('https://prp-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "You have transfered &#36; "+amount+"!", "#badc58", 1500);
                } else {
                    PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "You don't have enough balance!", "#badc58", 1500);
                }
                PRP.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "Fill out all fields!", "#badc58", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://prp-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.decline-invoice', function(event){
    ...
    $.post('https://prp-phone/DeclineInvoice', JSON.stringify({
        sender: InvoiceData.sender,
        amount: InvoiceData.amount,
        society: InvoiceData.society,
        invoiceId: InvoiceData.id,
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        PRP.Phone.Notifications.Add("fas fa-university", "PRPank", "There is no bank account attached to this number!", "#badc58", 2500);
    }
    PRP.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});