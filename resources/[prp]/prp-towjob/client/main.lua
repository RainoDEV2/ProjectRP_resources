...
AddEventHandler('jobs:client:ToggleNpc', function()
    if ProjectRP.Functions.GetPlayerData().job.name == "tow" then
        if CurrentTow ~= nil then
            ProjectRP.Functions.Notify("First Finish Your Work", "error")
            return
        end
        NpcOn = not NpcOn
        if NpcOn then
...
AddEventHandler('prp-tow:client:TowVehicle', function()
    ...
    if isTowVehicle(vehicle) then
        if CurrentTow == nil then
            --[[ Replaced "ProjectRP.Functions.GetClosestVehicle()" with custom implementation "getVehicleInDirection"
                 ProjectRP native could not return polcice and other vehicles types (NPC) ]]
            ...
            if NpcOn and CurrentLocation ~= nil then
                if GetEntityModel(targetVehicle) ~= GetHashKey(CurrentLocation.model) then
                    ProjectRP.Functions.Notify("This Is Not The Right Vehicle", "error")
                    return
                end
            end
            if not IsPedInAnyVehicle(PlayerPedId()) then
                if vehicle ~= targetVehicle then
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        ProjectRP.Functions.Progressbar("towing_vehicle", "Hoisting the Vehicle...", 5000, false, true, {
                            ...
                            if NpcOn then
                                RemoveBlip(CurrentBlip)
                                ProjectRP.Functions.Notify("Take The Vehicle To LS Impound", "success", 5000)
                                CurrentBlip2 = AddBlipForCoord(-239.2578, -1178.9448, 23.0440)
                                SetBlipColour(CurrentBlip2, 3)
                                SetBlipRoute(CurrentBlip2, true)
                                SetBlipRouteColour(CurrentBlip2, 3)
                                local chance = math.random(1,100)
                                if chance < 26 then
                                    TriggerServerEvent('prp-tow:server:nano')
                                end
                            end
                            ProjectRP.Functions.Notify("Vehicle Towed")
                        end, function() -- Cancel
                            StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
            ...
            ProjectRP.Functions.Progressbar("untowing_vehicle", "Remove The Vehicle", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                CurrentTow = nil
                ProjectRP.Functions.Notify("Vehicle Taken Off")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                ProjectRP.Functions.Notify("Failed", "error")
            end)
        ...
        ProjectRP.Functions.Notify("You Must Have Been In A Towing Vehicle First", "error")
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        if isLoggedIn and ProjectRP ~= nil then
            if PlayerJob.name == "tow" then
                    ...
                    if #(pos - vector3(Config.Locations["main"].coords.x, Config.Locations["main"].coords.y, Config.Locations["main"].coords.z)) < 1.5 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            if JobsDone > 0 then
                                ...
                                ProjectRP.Functions.Notify("You Havent Done Any Work Yet", "error")
                            end
                        end
                ...
                if NpcOn and CurrentLocation ~= nil and next(CurrentLocation) ~= nil then
                    if #(pos - vector3(CurrentLocation.x, CurrentLocation.y, CurrentLocation.z)) < 50.0 and not VehicleSpawned then
                        VehicleSpawned = true
                        ProjectRP.Functions.SpawnVehicle(CurrentLocation.model, function(veh)
                            exports['prp-fuel']:SetFuel(veh, 0.0)
...
function deliverVehicle(vehicle)
    ...
    JobsDone = JobsDone + 1
    VehicleSpawned = false
    ProjectRP.Functions.Notify("You Have Delivered A Vehicle", "success")
    ProjectRP.Functions.Notify("A New Vehicle Can Be Picked Up")
