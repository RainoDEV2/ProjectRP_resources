...
CreateThread(function()
	...
	exports['prp-target']:AddTargetBone(bones, {
		options = {
		{
			type = "client",
			event = "prp-fuel:client:SendMenuToServer",
...
RegisterNetEvent('prp-fuel:client:refuelCanMenu', function()
	exports['prp-menu']:openMenu({
		{
			header = "Gas Station",
			txt = "Buy jerry can. Remember there will be a 10% tax fee.",
			params = {
				event = "prp-fuel:client:refuelCan",
			}
		},
	})
end)
...
RegisterNetEvent('prp-fuel:client:refuelCan', function()
	...
	if GetAmmoInPedWeapon(ped, 883325847) < 4500 then
		...
		while not HasAnimDictLoaded('weapon@w_sp_jerrycan') do Wait(100) end
			...
			TriggerServerEvent('prp-fuel:server:PayForFuel', Config.refuelCost, GetPlayerServerId(PlayerId()))
			SetPedAmmo(ped, 883325847, 4500)
		...
		if inGasStation then
			if isCloseVeh() then
				...
				ProjectRP.Functions.Progressbar("refuel-car", "Refueling", time, false, true, {
					...
					TriggerServerEvent('prp-fuel:server:PayForFuel', refillCost, GetPlayerServerId(PlayerId()))
					SetFuel(vehicle, 100)
					PlaySound(-1, "5_SEC_WARNING", "HUD_MINI_GAME_SOUNDSET", 0, 0, 1)
					StopAnimTask(ped, "weapon@w_sp_jerrycan", "fire", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
				end, function() -- Cancel
