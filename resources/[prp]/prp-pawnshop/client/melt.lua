...
AddEventHandler("ProjectRP:Client:OnPlayerLoaded", function()
    ProjectRP.Functions.TriggerCallback('prp-pawnshop:melting:server:GetConfig', function(IsMelting, MeltTime, CanTake)
                ...
                if not Config.IsMelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, 38) then 
                            ...
                            ScrapAnim(1000)
                            ProjectRP.Functions.Progressbar("drop_golden_stuff", "Grab Items", 1000, false, true, {
                ...
                if GetClockHours() >= 9 and GetClockHours() <= 18 then
                    ...
                    elseif sellItemsSet and hasGold then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            ProjectRP.Functions.Progressbar("sell_gold", "Selling Gold", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
...
function HasPlayerGold()
	local retval = false
	ProjectRP.Functions.TriggerCallback('prp-pawnshop:server:hasGold', function(result)
		retval = result
	end)
    Citizen.Wait(500)
	return retval
...
RegisterNetEvent('prp-pawnshop:client:SetTakeState')
AddEventHandler('prp-pawnshop:client:SetTakeState', function(state)
