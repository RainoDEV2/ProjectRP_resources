...
function MenuGarage()
    ...
    local Vehicles = Config.Vehicles[ProjectRP.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "prp-newsjob:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function MenuHeliGarage()
    ...
    local Helicopters = Config.Helicopters[ProjectRP.Functions.GetPlayerData().job.grade.level]
    for veh, label in pairs(Helicopters) do
        vehicleMenu[#vehicleMenu+1] = {
            header = label,
            txt = "",
            params = {
                event = "prp-newsjob:client:TakeOutHelicopters",
                args = {
                    vehicle = veh
                }
            }
...
RegisterNetEvent('prp-newsjob:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('prp-newsjob:client:TakeOutHelicopters', function(data)
    local vehicle = data.vehicle
