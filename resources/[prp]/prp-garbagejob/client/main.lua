                ...
                if distance < 1.5 then
                    if InVehicle then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ProjectRP.Functions.TriggerCallback('garbagejob:server:EndShift', function(endShift)
                                if endShift then
                                    BringBackCar()
                                    ProjectRP.Functions.Notify("Truck returned, collect your payslip to recieve your pay and deposit back!")
                                else
                                    ProjectRP.Functions.Notify("You have no deposit paid on this vehicle..")
                                    currentStopNum = 0
                                    currentStop = 0
                                end
                            end, pos)
                        ...
                        if IsControlJustReleased(0, 38) then
                            ProjectRP.Functions.TriggerCallback('garbagejob:server:NewShift', function(shouldContinue, firstStop, totalBags)
                                if shouldContinue then

                                    local coords = Config.Locations["vehicle"].coords
                                    ProjectRP.Functions.SpawnVehicle("trash2", function(veh)
                                        ...
                                        isWorking = true
                                        SetGarbageRoute()
                                        ProjectRP.Functions.Notify("You have $"..Config.TruckPrice..", deposit paid!")
                                        ProjectRP.Functions.Notify("You have started working, location marked on GPS!")
                                    end, coords, true)

                                else
                                    ProjectRP.Functions.Notify("You have not enough money for the deposit.. Deposit costs are $"..Config.TruckPrice)
                                end
                            end)
                        end
                    end
                    ...
                    if not hasBag and canTakeBag then
                        ...
                        if DoesEntityExist(garbageVehicle) then
                            ...
                            if TruckDist < 2 then
                                ...
                                if IsControlJustPressed(0, 51) then
                                    ProjectRP.Functions.Progressbar("deliverbag", "Putting bag in trashmaster ..", 2000, false, true, {
                                        ...
                                        if (amountOfBags - 1) == 0 then
                                            ProjectRP.Functions.TriggerCallback('garbagejob:server:NextStop', function(hasMoreStops, nextStop, newBagAmount)
                                                if hasMoreStops and nextStop ~= 0 then
                                                    ...
                                                    SetGarbageRoute()
                                                    ProjectRP.Functions.Notify("All garbage bags are done, proceed to the next location!")
                                                else
                                                    if hasMoreStops and nextStop == currentStop then
                                                        ProjectRP.Functions.Notify("There was an issue at the depot, please return immediately!")
                                                        amountOfBags = 0
                                                    else
                                                        -- You are done with work here.
                                                        ProjectRP.Functions.Notify("You are done working! Go back to the depot.")
                                                        isWorking = false
                                                        RemoveBlip(deliveryBlip)
                                                        SetRouteBack()
                                                        amountOfBags = 0
                                            ...
                                            if amountOfBags > 1 then
                                                ProjectRP.Functions.Notify("There are still "..amountOfBags.." bags left!")
                                            else
                                                ProjectRP.Functions.Notify("There is still "..amountOfBags.." bags over there!")
                                            end
                                            hasBag = false
                                        end
                            ...
                            ProjectRP.Functions.Notify("You have no truck", "error")
