...
ProjectRP.Functions.CreateCallback("garbagejob:server:NewShift", function(source, cb)
    ...
    if Player.Functions.RemoveMoney("bank", Config.TruckPrice, "garbo-truck-dep") then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, 'Not Enough Money ('..Config.TruckPrice..' required)', "error")
    end
    cb(shouldContinue, nextStop, bagNum, totalNumberOfStops)
end)


ProjectRP.Functions.CreateCallback("garbagejob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    local Player = ProjectRP.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    ...
    if(math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, "You found a cryptostick on the floor")
    end
    ...
    if distance <= 10 then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, 'You are too far away from the drop-off point', "error")
    end

    cb(shouldContinue,newStop,newBagAmount)
end)

ProjectRP.Functions.CreateCallback('garbagejob:server:EndShift', function(source, cb, location)
    local Player = ProjectRP.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    ...
    if(distance < 10) then
        ...
        TriggerClientEvent('ProjectRP:Notify', source, 'You are too far away from the dropoff point', "error")
        cb(false)
    end
end)
...
AddEventHandler('garbagejob:server:PayShift', function()
    ...
    if Routes[CitizenId] ~= nil then
        ...
        if tonumber(Routes[CitizenId].stopsCompleted) < tonumber(Routes[CitizenId].totalNumberOfStops) then
            ...
            depositPay = 0
            TriggerClientEvent('ProjectRP:Notify', src, "Due to early finish (Completed: "..Routes[CitizenId].stopsCompleted .." Total: "..Routes[CitizenId].totalNumberOfStops.."), your deposit will not be returned.", "error")
        end
        ...
        TriggerClientEvent('ProjectRP:Notify', src, "You got $"..totalToPay..", your payslip "..payoutDeposit.." got paid to your bank account!", "success")
        Routes[CitizenId] = nil
    else
        TriggerClientEvent('ProjectRP:Notify', source, 'You never clocked on!', "error")
    end
end)

ProjectRP.Commands.Add("cleargarbroutes", "Removes garbo routes for user (admin only)", {{name="id", help="Player ID (may be empty)"}}, false, function(source, args)
    ...
    TriggerClientEvent('ProjectRP:Notify', source, "Cleared users routes they had "..count.." routes stored", "success")
    Routes[CitizenId] = nil
end, "admin")