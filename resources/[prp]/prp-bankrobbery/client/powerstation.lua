...
local currentFires = {}
local currentGate = 0
local requiredItems = {[1] = {name = ProjectRP.Shared.Items["thermite"]["name"], image = ProjectRP.Shared.Items["thermite"]["image"]}}
        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    ProjectRP.Functions.Notify("It seems that the fuses have blown.", "error")
                end
            else
                ProjectRP.Functions.Notify('Minimum Of '..Config.MinimumThermitePolice..' Police Needed', "error")
            end
        end
...
RegisterNetEvent('prp-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function()
    ProjectRP.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
...
RegisterNUICallback('thermitesuccess', function()
    ProjectRP.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            local coords = GetEntityCoords(PlayerPedId())
            while time > 0 do
                ProjectRP.Functions.Notify("Thermite is going off in " .. time .. "..")
                Wait(1000)
                time -= 1
            end
            local randTime = math.random(10000, 15000)
            ...
            if currentStation ~= 0 then
                ProjectRP.Functions.Notify("The fuses are broken", "success")
                TriggerServerEvent("prp-bankrobbery:server:SetStationStatus", currentStation, true)
            elseif currentGate ~= 0 then
                ProjectRP.Functions.Notify("The door is open", "success")
                TriggerServerEvent('prp-doorlock:server:updateState', currentGate, false, false, false, true, false, false)
                currentGate = 0
            end
        end
    end)
