...
$(document).on('click', '#save', function(){
    $.post('https://prp-tunerchip/save', JSON.stringify({
...
$(document).on('click', '#reset', function(){
    $.post('https://prp-tunerchip/reset');
});
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://prp-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://prp-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://prp-tunerchip/SetStancer', JSON.stringify({
...
PRPPTuner.Open = function() {
    $.post('https://prp-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
...
PRPPTuner.Close = function() {
    $('.container').fadeOut(250);
    $.post('https://prp-tunerchip/exit');
}