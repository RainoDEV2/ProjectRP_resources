...
RegisterNUICallback('save', function(data)
    ProjectRP.Functions.TriggerCallback('prp-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            ProjectRP.Functions.Notify('TunerChip v1.05: Vehicle Tuned!', 'error')

            TriggerServerEvent('prp-tunerchip:server:TuneStatus', ProjectRP.Functions.GetPlate(veh), true)
        end
    end)
end)

RegisterNetEvent('prp-tunerchip:client:TuneStatus')
AddEventHandler('prp-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        ProjectRP.Functions.TriggerCallback('prp-tunerchip:server:GetStatus', function(status)
            if status then
                ProjectRP.Functions.Notify('This Vehicle Has Been Tuned', 'success')
            else
                ProjectRP.Functions.Notify('This Vehicle Has Not Been Tuned', 'error')
            end
        end, plate)
    else
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    ProjectRP.Functions.Notify('TunerChip v1.05: Vehicle has been reset!', 'error')
end)
...
AddEventHandler('prp-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        ProjectRP.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Vehicle Has Been Reset!", 2000, false, true, {
