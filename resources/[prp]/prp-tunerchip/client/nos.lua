...
AddEventHandler('ProjectRP:Client:OnPlayerLoaded', function()
    ProjectRP.Functions.TriggerCallback('nitrous:GetNosLoadedVehs', function(vehs)
        VehicleNitrous = vehs
    end)
end)
    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                ProjectRP.Functions.Progressbar("use_nos", "Connecting NOS...", 1000, false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Done
                    TriggerEvent("inventory:client:ItemBox", ProjectRP.Shared.Items['nitrous'], "remove")
                    TriggerServerEvent("ProjectRP:Server:RemoveItem", 'nitrous', 1)
                    local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
                    local Plate = ProjectRP.Functions.GetPlate(CurrentVehicle)
                    TriggerServerEvent('nitrous:server:LoadNitrous', Plate)
                end)
            else
                ProjectRP.Functions.Notify("You cannot do that from this seat!", "error")
            end
        else
            ProjectRP.Functions.Notify('You\'re Not In A Car', 'error')
        end
    else
        ProjectRP.Functions.Notify('You Already Have NOS Active', 'error')
    end
end)
                ...
                if VehicleNitrous[Plate].hasnitro then
                            ...
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    ...
                                    for index,_ in pairs(Fxs) do
                                        ...
                                        TriggerServerEvent('nitrous:server:StopSync', ProjectRP.Functions.GetPlate(CurrentVehicle))
                                        Fxs[index] = nil
                                    end
                                end
                                Citizen.Wait(100)
                    ...
                    if IsControlJustReleased(0, 36) and GetPedInVehicleSeat(CurrentVehicle, -1) == PlayerPedId() then
                        if NitrousActivated then
                            ...
                            for index,_ in pairs(Fxs) do
                                StopParticleFxLooped(Fxs[index], 1)
                                TriggerServerEvent('nitrous:server:StopSync', ProjectRP.Functions.GetPlate(veh))
                                Fxs[index] = nil
                            end
                            StopScreenEffect("RaceTurbo")
...
AddEventHandler('nitrous:client:SyncFlames', function(netid, nosid)
    ...
    if veh ~= 0 then
        ...
        if NOSPFX[ProjectRP.Functions.GetPlate(veh)] == nil then
            NOSPFX[ProjectRP.Functions.GetPlate(veh)] = {}
        end
        if myid ~= nosid then
            for _,bones in pairs(p_flame_location) do
                if NOSPFX[ProjectRP.Functions.GetPlate(veh)][bones] == nil then
                    NOSPFX[ProjectRP.Functions.GetPlate(veh)][bones] = {}
                end
                if GetEntityBoneIndexByName(veh, bones) ~= -1 then
                    if NOSPFX[ProjectRP.Functions.GetPlate(veh)][bones].pfx == nil then
                        RequestNamedPtfxAsset(ParticleDict)
                        while not HasNamedPtfxAssetLoaded(ParticleDict) do
                            Citizen.Wait(0)
                        end
                        SetPtfxAssetNextCall(ParticleDict)
                        UseParticleFxAssetNextCall(ParticleDict)
                        NOSPFX[ProjectRP.Functions.GetPlate(veh)][bones].pfx = StartParticleFxLoopedOnEntityBone(ParticleFx, veh, 0.0, -0.05, 0.0, 0.0, 0.0, 0.0, GetEntityBoneIndexByName(veh, bones), ParticleSize, 0.0, 0.0, 0.0)
...
AddEventHandler('nitrous:client:LoadNitrous', function(Plate)
    ...
    local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
    local CPlate = ProjectRP.Functions.GetPlate(CurrentVehicle)
