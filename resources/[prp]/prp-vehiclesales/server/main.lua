ProjectRP.Functions.CreateCallback('prp-occasions:server:getVehicles', function(source, cb)
...
ProjectRP.Functions.CreateCallback("prp-occasions:server:getSellerInformation", function(source, cb, citizenid)
    local src = source
...
RegisterServerEvent('prp-occasions:server:ReturnVehicle')
AddEventHandler('prp-occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    if result[1] ~= nil then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent("prp-occasions:client:ReturnOwnedVehicle", src, result[1])
            TriggerClientEvent('prp-occasion:client:refreshVehicles', -1)
        else
            TriggerClientEvent('ProjectRP:Notify', src, 'This is not your vehicle', 'error', 3500)
        end
    else
        TriggerClientEvent('ProjectRP:Notify', src, 'Vehicle does not exist', 'error', 3500)
    end
end)

RegisterServerEvent('prp-occasions:server:sellVehicle')
AddEventHandler('prp-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    ...
    TriggerEvent("prp-log:server:sendLog", Player.PlayerData.citizenid, "vehiclesold", {
        model = vehicleData.model,
        vehiclePrice = vehiclePrice
    })
    TriggerEvent("prp-log:server:CreateLog", "vehicleshop", "Vehicle for Sale", "red",
        "**" .. GetPlayerName(src) .. "** has a " .. vehicleData.model .. " priced at " .. vehiclePrice)

    TriggerClientEvent('prp-occasion:client:refreshVehicles', -1)
end)

RegisterServerEvent('prp-occasions:server:sellVehicleBack')
AddEventHandler('prp-occasions:server:sellVehicleBack', function(vData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
...
AddEventHandler('prp-occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = ProjectRP.Functions.GetPlayer(src)
    local result = exports.oxmysql:executeSync('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',
        {vehicleData['plate'], vehicleData["oid"]})
    if result[1] ~= nil and next(result[1]) ~= nil then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            local SellerData = ProjectRP.Functions.GetPlayerByCitizenId(SellerCitizenId)
            ...
            TriggerEvent("prp-log:server:CreateLog", "vehicleshop", "bought", "green", "**" .. GetPlayerName(src) .. "** has bought for " .. result[1].price .. " (" .. result[1].plate ..") from **" .. SellerCitizenId .. "**")
            TriggerClientEvent("prp-occasions:client:BuyFinished", src, result[1])
            TriggerClientEvent('prp-occasion:client:refreshVehicles', -1)
            ...
            TriggerEvent('prp-phone:server:sendNewMailToOffline', SellerCitizenId, {
                sender = 'Larrys RV Sales',
                subject = "You have sold a vehicle!",
                message = 'You made $'..NewPrice..' from the sale of your '..ProjectRP.Shared.Vehicles[result[1].model].name..''
            })
        else
            TriggerClientEvent('ProjectRP:Notify', src, 'You dont have enough money', 'error', 3500)
        end
    end
end)

ProjectRP.Functions.CreateCallback("prp-vehiclesales:server:CheckModelName", function(source, cb, plate)
    if plate then
