...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			ProjectRP.Functions.Notify('The delivery was on point, your GPS will be updated with the next drop off', 'success')
		else
			ProjectRP.Functions.Notify('The Drop Off Failed', 'error')
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			ProjectRP.Functions.Notify('You Are Close To The Drop Off Point')
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			ProjectRP.Functions.Notify('You Took Too Long', 'error')
		end
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dropOff6 < 1.6 and not OxyRun then
			...
			DrawText3Ds(pillWorker['coords']["x"],pillWorker['coords']["y"],pillWorker['coords']["z"], "[E] $700 - Delivery Job (Payment Cash + Oxy)") 
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	firstdeal = true
	CreateOxyVehicle()
	ProjectRP.Functions.Notify('A Car Has Been Provided. Your GPS Will Be Updated With Locations Soon', 'success')
end)
