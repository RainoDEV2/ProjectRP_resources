...
local function loadSettings(settings)
    ...
    ProjectRP.Functions.Notify("HUD Settings Loaded!", "success")
    Wait(1000)
...
local function restartHud()
    ...
    ProjectRP.Functions.Notify("HUD Is Restarting!", "success")
    ...
    Wait(2600)
    ProjectRP.Functions.Notify("HUD is Now Started!", "success")
end
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    ProjectRP.Functions.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            ProjectRP.Functions.Notify("Square Map Loading...", "success")
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            ProjectRP.Functions.Notify("Square Map Has Loaded!...", "success")
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            ProjectRP.Functions.Notify("Circle Map Loading...", "success")
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            ProjectRP.Functions.Notify("Circle Map Loaded!", "success")
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function()
    ...
    if Menu.isCineamticModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            ProjectRP.Functions.Notify("Cinematic Mode Off!", "success")
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            ProjectRP.Functions.Notify("Cinematic Mode On!", "success")
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in prp-core
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent("prp-admin:client:ToggleDevmode", function()
    dev = not dev
end)
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        ProjectRP.Functions.Notify("Engine Shut Down!", "success")
    else
        ProjectRP.Functions.Notify("Engine Started!", "success")
    end
...
local function getFuelLevel(vehicle)
    ...
    if (updateTick - lastFuelUpdate) > 2000 then
        ...
        lastFuelCheck = math.floor(exports['prp-fuel']:GetFuel(vehicle))
    end
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(player) and not IsThisModelABicycle(vehicle) then
                ...
                wasInVehicle = true
                ProjectRP.Functions.TriggerCallback('hud:server:HasHarness', function(hasItem)
                    if hasItem then
                        harness = true
                    else
                        harness = false
    ...
    while true do
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) then
                if exports['prp-fuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        TriggerServerEvent("InteractSound_SV:PlayOnSource", "pager", 0.10)
                        ProjectRP.Functions.Notify("Fuel Level Low!", "error")
