							...
							if IsControlJustReleased(0, 38) then
								if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
									if IsVehicleValid(GetEntityModel(vehicle)) then
										local vehiclePlate = ProjectRP.Functions.GetPlate(vehicle)
										ProjectRP.Functions.TriggerCallback('prp-scrapyard:checkOwnerVehicle',function(retval)
											if retval then
												ScrapVehicle(vehicle)
											else
												ProjectRP.Functions.Notify("You can't smash a vehicle that owns it.", "error")
											end
										end,vehiclePlate)
									else
										ProjectRP.Functions.Notify("This Vehicle Cannot Be Scrapped.", "error")
									end
								else
									ProjectRP.Functions.Notify("You Are Not The Driver", "error")
								end
							end
						end
					end
...
RegisterNetEvent('prp-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then
				local vehicleInfo = ProjectRP.Shared.Vehicles[v]
		...
		ProjectRP.Functions.Notify("You Are Not Allowed To Demolish Vehicles Now", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	ProjectRP.Functions.Progressbar("scrap_vehicle", "Demolish Vehicle", scrapTime, false, true, {
