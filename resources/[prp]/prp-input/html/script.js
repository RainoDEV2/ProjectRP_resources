...
const OpenMenu = (data) => {
    ...
    $("#prp-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#prp-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#prp-input-form").serializeArray();
