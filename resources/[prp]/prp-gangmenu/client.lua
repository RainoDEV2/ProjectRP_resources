...
RegisterNetEvent('ProjectRP:Client:OnGangUpdate')
AddEventHandler('ProjectRP:Client:OnGangUpdate', function(GangInfo)
    PlayerGang = GangInfo
end)
...
RegisterNetEvent('prp-gangmenu:client:openMenu')
AddEventHandler('prp-gangmenu:client:openMenu', function()
    MenuV:OpenMenu(menu)
end)
...
menu_button6:On("select", function()
    ...
    if result ~= nil and PlayerGang.name and PlayerGang.isboss then
        TriggerServerEvent("prp-gangmenu:server:withdrawMoney", tonumber(result))
        UpdateSociety()
    else
        ProjectRP.Functions.Notify('Not High Enough Rank', "error")
    end
end)
...
menu_button7:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("prp-gangmenu:server:depositMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button1:On("select", function()
    ...
    for k,v in pairs(ProjectRP.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
        if v and v ~= PlayerId() then
            local menu_button10 = menu4:AddButton({
                ...
                select = function(btn)
                    local select = btn.Value
                    TriggerServerEvent('prp-gangmenu:server:giveJob', GetPlayerServerId(v))
                end
            })
        end
    end
            ...
            local menu_button9 = manageroptions:AddButton({
                ...
                select = function(btn)
                    ...
                    if values == 'promote' then
                        ...
                        if result ~= nil then
                            TriggerServerEvent('prp-gangmenu:server:updateGrade', employee.empSource, tonumber(result))
                        end
                    else
                        TriggerServerEvent('prp-gangmenu:server:fireEmployee', employee.empSource)
                    end
                end
            })
        end
