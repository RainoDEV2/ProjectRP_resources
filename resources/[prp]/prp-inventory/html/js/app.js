...
function showBlur() {
    $.post('https://prp-inventory/showBlur');
}
...
$(document).on("click", ".weapon-attachments-back", function(e) {
    e.preventDefault();
    $("#prpcore-inventory").css({ display: "block" });
    $("#prpcore-inventory").animate({
            left: 0 + "vw",
        },
        200
    );
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://prp-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://prp-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#prpcore-inventory").animate({
            ...
            200,
            function() {
                $("#prpcore-inventory").css({ display: "none" });
            }
        );
        $(".weapon-attachments-container").animate({
                left: 0 + "vw",
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://prp-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://prp-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://prp-inventory/SetInventoryData",
        JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://prp-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://prp-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://prp-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://prp-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $(".ply-iteminfo-container").css("opacity", "0.0");
        $("#prpcore-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://prp-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#prpcore-inventory").css({ left: "0vw" });
