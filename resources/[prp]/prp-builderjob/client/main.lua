...
function GetCurrentProject()
    ProjectRP.Functions.TriggerCallback('prp-builderjob:server:GetCurrentProject', function(BuilderConfig)
                    ...
                    if TaskData ~= nil then
                        ...
                        if TaskData.completed == TaskData.total then
                            ...
                            if IsControlJustPressed(0, 47) then
                                TriggerServerEvent('prp-builderjob:server:FinishProject')
                            end
                        end
...
function DoTask()
    ...
    local CountDown = 5
    TriggerServerEvent('prp-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, false)
    ...
    if TaskData.type == "hammer" then
        ...
        TriggerServerEvent('prp-builderjob:server:SetTaskState', BuilderData.CurrentTask, true, true)
    end
end

RegisterNetEvent('prp-builderjob:client:SetTaskState')
AddEventHandler('prp-builderjob:client:SetTaskState', function(Task, IsBusy, IsCompleted)
...
RegisterNetEvent('prp-builderjob:client:FinishProject')
AddEventHandler('prp-builderjob:client:FinishProject', function(BuilderConfig)
    Config = BuilderConfig
end)
