# ProjectRP Vanilla Unicorn Job
This is a Vanilla Unicorn Job designed exclusively for ProjectRP at the request of the community.
...
- Stashes
- Toggle Duty
- Complete prp-target & prp-menu support
...
## Setup
1) Add the following into your prp-core > shared.lua, under the Items section:
```
-- Vanilla Unicorn
...
```
2) Once added, add the job into your prp-core > shared.lua like so:
```
['vu'] = {
...
3) You'll also need to setup the boss menu for this job, to do so, go to prp-bossmenu > config.lua and add the following:
```lua
