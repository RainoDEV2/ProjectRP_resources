ProjectRP = exports['prp-core']:GetCoreObject() -- Core
...
RegisterNetEvent('ProjectRP:Client:OnPlayerLoaded', function() -- Core stuff
    PlayerData = ProjectRP.Functions.GetPlayerData()
    onDuty = PlayerData.job.onduty
end)

RegisterNetEvent('ProjectRP:Client:OnJobUpdate', function(JobInfo) -- Core stuff
    PlayerData = ProjectRP.Functions.GetPlayerData()
    PlayerData.job = JobInfo
    onDuty = PlayerData.job.onduty
end)

RegisterNetEvent('prp-vanillaunicorn:Client:signOn', function()
    if LocalPlayer.state['isLoggedIn'] and PlayerData.job.name == "vu" then 
        TriggerServerEvent("ProjectRP:ToggleDuty")
    end
end)

RegisterNetEvent('prp-vanillaunicorn:Client:stash', function()
    if onDuty and PlayerData.job.name == "vu" then
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "vustash_"..ProjectRP.Functions.GetPlayerData().citizenid)
        TriggerEvent("inventory:client:SetCurrentStash", "vustash_"..ProjectRP.Functions.GetPlayerData().citizenid)
    end
end)
...
RegisterNetEvent('prp-vanillaunicorn:Client:accessBarMenu', function(data) -- Access bar menu, exploit checks
    ...
    for i, v in pairs(settings) do
        ...
        if canUse and #(plyCoords - barCoords) < 5 then 
            exports["prp-menu"]:openMenu(barMenu)
        else
...
RegisterNetEvent('prp-vanillaunicorn:Client:purchaseDrink', function(data) -- purchase drinks, exploit check
    name = data.type.name
    ...
    if name and price then 
        barConfirmation = {
            ...
            },
            {
                ...
                params = {
                    isServer = true,
                    event = "prp-vanillaunicorn:Server:handleItem",
                    args = {
                        type = data.type.item,
        ...
        exports["prp-menu"]:openMenu(barConfirmation)  
    else
        print("How did you get here?")
    end
end)

RegisterNetEvent('prp-vanillaunicorn:Client:barMenu', function() -- Open bar menu
    exports["prp-menu"]:openMenu(drinks)
end)
