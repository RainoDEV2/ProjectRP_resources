...
for k, v in pairs(availableKeys) do
    RegisterCommand(v[1], function()
        if isLoggedIn and not keyPressed and GetLastInputMethod(0) then
            local keyMeta = ProjectRP.Functions.GetPlayerData().metadata["commandbinds"]
            local args = {}
            if next(keyMeta) ~= nil then
                if keyMeta[v[2]]["command"] ~= "" then
                    ...
                    TriggerServerEvent('ProjectRP:CallCommand', keyMeta[v[2]]["command"], args)
                    keyPressed = true
                    Citizen.Wait(1000)
                    keyPressed = false
                else
                    ProjectRP.Functions.Notify('There is still nothing ['..v[2]..'] bound, /binds to bind a command', 'primary', 4000)
                end
            else
                ProjectRP.Functions.Notify('You have not bound any commands, /binds to bind a command', 'primary', 4000)
            end
        end
    end, false)
...
RegisterNUICallback('save', function(data)
    ...
    ProjectRP.Functions.Notify('Command bindings have been saved!', 'success')
