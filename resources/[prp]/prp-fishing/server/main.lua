ProjectRP.Functions.CreateCallback('prp-fishing:GetItemData', function(source, cb, itemName)
	local retval = false
...
ProjectRP.Functions.CreateUseableItem("fishingrod", function(source, item)
    local Player = ProjectRP.Functions.GetPlayer(source)

    TriggerClientEvent('prp-fishing:tryToFish', source)
end)

RegisterServerEvent('prp-fishing:receiveFish')
AddEventHandler('prp-fishing:receiveFish', function(cabin, house)
    local src = source
...
RegisterServerEvent("prp-fishing:sellFish")
AddEventHandler("prp-fishing:sellFish", function()
    local src = source
	local Player = ProjectRP.Functions.GetPlayer(src)
	local price = 0
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
		...
		TriggerClientEvent('ProjectRP:Notify', src, "You have sold your fish")
	end
end)