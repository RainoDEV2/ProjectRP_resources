TryToFish = function()
    ProjectRP.Functions.TriggerCallback('prp-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return ProjectRP.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return ProjectRP.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
        if count ~= nil then
            if count == 0 then
                ProjectRP.Functions.Notify("You need both a fishing rod and bait to fish.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    ProjectRP.Functions.Notify("You need to aim towards the water to fish", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        ProjectRP.Functions.Notify('You don\'t have any bait!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        if GetGameTimer() - startedCasting > 5000 then
            ProjectRP.Functions.Notify("You need to cast the bait.", "primary")
    ...
    ProjectRP.Functions.Notify("Waiting for a fish to bite...", "success", "10000")
    TriggerServerEvent('ProjectRP:Server:RemoveItem', "fishingbait", 1)
	TriggerEvent("inventory:client:ItemBox", ProjectRP.Shared.Items["fishingbait"], "remove")
    ...
    if caughtFish then
        TriggerServerEvent("prp-fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('hud:server:RelieveStress', 1)
    else
        ProjectRP.Functions.Notify("The fish got loose.", "error")
    end
...
IsInWater = function()
    ...
    ProjectRP.Functions.Notify("Checking fishing location...", "success", "3000")
...
SellFish = function()
    ProjectRP.Functions.TriggerCallback('prp-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("prp-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
function HasFishingBait()
    local rtval = false
    ProjectRP.Functions.TriggerCallback('ProjectRP:HasItem', function(amount)
        if amount then
            rtval = true
        end
    end, "fishingbait")
