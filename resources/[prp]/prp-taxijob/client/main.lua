    ...
    if whitelistedVehicle() then
       -- if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                ProjectRP.Functions.Notify('The NPC Is Indicated On Your GPS', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    ...
                                    ProjectRP.Functions.Notify('Bring The NPC To The Specified Location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                ProjectRP.Functions.Notify('You Are Already Doing An NPC Mission')
            end
      --  else
         --   ProjectRP.Functions.Notify('No NPCs Are Available')
       -- end
    else
        ProjectRP.Functions.Notify('You Are Not In A Taxi')
    end
end)
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        ProjectRP.Functions.Notify('Person Was Dropped Off', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
AddEventHandler('prp-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            ProjectRP.Functions.Notify('This Vehicle Has No Taxi Meter', 'error')
        end
    else
        ProjectRP.Functions.Notify('You\'re Not In A Vehicle ', 'error')
    end
end)
...
AddEventHandler('prp-taxi:client:enableMeter', function()
    if meterIsOpen then
        ...
        ProjectRP.Functions.Notify('The Taxi Meter Is Not Active', 'error')
    end
end)
...
AddEventHandler('prp-taxi:client:toggleMuis', function()
    ...
    if meterIsOpen then
        ...
        ProjectRP.Functions.Notify('No Taxi Meter In Sight', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    ProjectRP.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
