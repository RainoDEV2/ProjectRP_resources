...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    ProjectRP.Functions.TriggerCallback('hospital:server:HasFirstAid', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                isHealingPerson = true
                ProjectRP.Functions.Progressbar("hospital_revive", "Reviving person..", 20000, false, true, {
                    ...
                    ProjectRP.Functions.Notify("You revived the person!")
...
RegisterNetEvent('hospital:client:TreatWounds', function()
    ProjectRP.Functions.TriggerCallback('hospital:server:HasBandage', function(hasItem)
        if hasItem then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                ...
                ProjectRP.Functions.Progressbar("hospital_healwounds", "Healing wounds..", 5000, false, true, {
                    ...
                    isHealingPerson = false
                    StopAnimTask(PlayerPedId(), healAnimDict, "exit", 1.0)
                    ProjectRP.Functions.Notify("You helped the person!")
