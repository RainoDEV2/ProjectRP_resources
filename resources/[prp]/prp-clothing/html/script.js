...
PRPClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://prp-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e){
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://prp-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://prp-clothing/setCurrentPed', JSON.stringify({ped: newValue}), function(model){
                    $("#current-model").html("<p>"+model+"</p>")
                });
                canChange = true;
                PRPClothing.ResetValues()
                ...
                if (buttonType == "item") {
                    PRPClothing.ResetItemTexture(this, clothingCategory);
                }
            } else {
                if (buttonType == "item") {
...
$(document).on('change', '.item-number', function(){
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://prp-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e){
    ...
    if (selectedCam == null) {
        ...
        $.post('https://prp-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        case 68: // D
            $.post('https://prp-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://prp-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
PRPClothing.ToggleChange = function(bool) {
    canChange = bool;
}
...
$(document).ready(function(){
    window.addEventListener('message', function(event) {
        switch(event.data.action) {
            case "open":
                PRPClothing.Open(event.data);
                break;
            case "close":
                PRPClothing.Close();
                break;
            case "updateMax":
                PRPClothing.SetMaxValues(event.data.maxValues);
                break;
            case "reloadMyOutfits":
                PRPClothing.ReloadOutfits(event.data.outfits);
                break;
            case "toggleChange":
                PRPClothing.ToggleChange(event.data.allow);
                break;
            case "ResetValues":
                PRPClothing.ResetValues();
                break;
...
PRPClothing.ReloadOutfits = function(outfits) {
...
$(document).on('click', "#save-menu", function(e){
    e.preventDefault();
    PRPClothing.Close();
    $.post('https://prp-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e){
    e.preventDefault();
    PRPClothing.Close();
    $.post('https://prp-clothing/resetOutfit');
});

PRPClothing.SetCurrentValues = function(clothingValues) {
...
PRPClothing.Open = function(data) {
    ...
    PRPClothing.SetMaxValues(data.maxValues);
    $(".clothing-menu-header").html("");
    PRPClothing.SetCurrentValues(data.currentClothing);
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e){
    ...
    $.post('https://prp-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e){
    ...
    $.post('https://prp-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
PRPClothing.Close = function() {
    $.post('https://prp-clothing/close');
...
PRPClothing.SetMaxValues = function(maxValues) {
...
PRPClothing.ResetValues = function() {
...
$(document).on('click', '#save-outfit-save', function(e){
    ...
    $.post('https://prp-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e){
    ...
    $.post('https://prp-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});
