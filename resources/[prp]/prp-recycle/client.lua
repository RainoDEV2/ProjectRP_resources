...
RegisterNetEvent("prp-recycle:removeWarehouseProps")
AddEventHandler("prp-recycle:removeWarehouseProps", function()
    CleanUpArea()
end)
...
Citizen.CreateThread(function()
    exports['prp-target']:AddCircleZone("recycleduty", vector3(994.64,-3100.07,-39.0), 2.0, { name="recycleduty", debugPoly=false, useZ=true, },
    { options = { { event = "recycle:dutytoggle", icon = "fas fa-hard-hat", label = "Toggle Recycling Duty", }, },
					distance = 1.5
    })
    exports['prp-target']:AddCircleZone("tradeitems", Config.Locations['Trade'].location, 2.0, { name="tradeitems", debugPoly=false, useZ=true, },
    { options = { { event = "prp-recycle:Trade:Menu", icon = "fas fa-box", label = "Trade Materials", }, },
					distance = 1.5
    })
    exports['prp-target']:AddCircleZone("sellmats", Config.Locations['Recycle'].location, 2.0, { name="sellmats", debugPoly=false, useZ=true, },
    { options = { { event = "prp-recycle:Selling:Menu", icon = "fas fa-box", label = "Sell Materials", }, },
					distance = 2.5
    })
	--Dumpster Third Eye
	exports['prp-target']:AddTargetModel(dumpsters, { options = { { event = "prp-recycle:Dumpsters:Search", icon = "fas fa-dumpster", label = "Search Trash", }, },
					distance = 1.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer", Config.Locations['BottleBank'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer2", Config.Locations['BottleBank2'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer3", Config.Locations['BottleBank3'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer4", Config.Locations['BottleBank4'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer5", Config.Locations['BottleBank5'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
    exports['prp-target']:AddCircleZone("BottleBuyer6", Config.Locations['BottleBank6'].location, 2.0, { name="BottleBuyer", debugPoly=false, useZ=true, }, 
    { options = { { event = "prp-recycle:Bottle:Menu", icon = "fas fa-certificate", label = "Sell Bottles", },	},
					distance = 2.5
    })
end)
    ...
    while true do
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].OutsideLocation.x, Config['delivery'].OutsideLocation.y, Config['delivery'].OutsideLocation.z, true) < 1.3 then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].InsideLocation.x, Config['delivery'].InsideLocation.y, Config['delivery'].InsideLocation.z, true) < 1.3 then
                ...
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('prp-recycle:removeWarehouseProps')
                    DoScreenFadeOut(500)
                    while not IsScreenFadedOut() do
                        Citizen.Wait(10)
                    end
                ...
                if carryPackage == nil then
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, packagePos.x,packagePos.y,packagePos.z, true) < 2.3 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            ProjectRP.Functions.Progressbar("pickup_reycle_package", "Picking up the junk..", 5000, false, true, {}, {}, {}, {}, function() -- Done
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].DropLocation.x, Config['delivery'].DropLocation.y, Config['delivery'].DropLocation.z, true) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            DropPackage()
                            ScrapAnim()
                            ProjectRP.Functions.Progressbar("deliver_reycle_package", "Packing into recyclable box..", 5000, false, true, {
                                ...
                                TriggerServerEvent('prp-recycle:getrecyclablematerial')
                                GetRandomPackage()
                            end)
                        end
                    else
...
AddEventHandler('recycle:dutytoggle', function()
    onDuty = not onDuty
    if onDuty then
		TriggerEvent('ProjectRP:Notify', 'You Went On Duty', 'success')
    else
        TriggerEvent('ProjectRP:Notify', 'You Went Off Duty', 'error')
    end
end)

--Sell Anim small Test
RegisterNetEvent('prp-recycle:SellAnim')
AddEventHandler('prp-recycle:SellAnim', function(data)
	if data == -2 then
	...
	if data == 1 then
		TriggerServerEvent('prp-recycle:Selling:All')
	else
		TriggerServerEvent('prp-recycle:Selling:Mat', data)
	end
...
--Sell Anim small Test
RegisterNetEvent('prp-recycle:TradeAnim')
AddEventHandler('prp-recycle:TradeAnim', function(data)
	local pid = PlayerPedId()
	loadAnimDict("mp_common")
	TriggerServerEvent('prp-recycle:TradeItems', data) -- Had to slip in the sell command during the animation command
	for k,v in pairs (shopPeds) do
        pCoords = GetEntityCoords(PlayerPedId())
        ppCoords = GetEntityCoords(v)
        dist = #(pCoords - ppCoords)
...
--Material Buyer
RegisterNetEvent('prp-recycle:Selling:Menu', function()
    exports['prp-menu']:openMenu({
		{ header = "Material Selling", txt = "Sell batches of materials", isMenuHeader = true },
		{ header = "", txt = "✘ Close", params = { event = "prp-recycle:SellAnim", args = -2 } },
		{ header = "Copper", params = { event = "prp-recycle:SellAnim", args = 'copper' } },
		{ header = "Plastic", params = { event = "prp-recycle:SellAnim", args = 'plastic' } },
		{ header = "Aluminum", params = { event = "prp-recycle:SellAnim", args = 'aluminum' } },
		{ header = "Metal Scraps", params = { event = "prp-recycle:SellAnim", args = 'metalscrap' }  },
		{ header = "Steel", params = { event = "prp-recycle:SellAnim", args = 'steel' } },
		{ header = "Glass", params = { event = "prp-recycle:SellAnim", args = 'glass' } },
		{ header = "Iron", params = { event = "prp-recycle:SellAnim", args = 'iron' } },
		{ header = "Rubber", params = { event = "prp-recycle:SellAnim", args = 'rubber' } },
		{ header = "ALL", params = { event = "prp-recycle:SellAnim", args = 1 } }, 
    })
end)

--Recyclable Trader
RegisterNetEvent('prp-recycle:Trade:Menu', function()
    exports['prp-menu']:openMenu({
		{ header = "Material Trading", txt = "Trade collected materials", isMenuHeader = true },
		{ header = "", txt = "✘ Close", params = { event = "prp-recycle:SellAnim", args = -2 } },
		{ header = "Trade 10 Materials", params = { event = "prp-recycle:TradeAnim", args = 1 } },
		{ header = "Trade 100 Materials", params = { event = "prp-recycle:TradeAnim", args = 2 } },
    })
end)

--Recyclable Trader
RegisterNetEvent('prp-recycle:Bottle:Menu', function()
    exports['prp-menu']:openMenu({
		{ header = "Material Selling", txt = "Sell batches of recyclables", isMenuHeader = true },
		{ header = "", txt = "✘ Close", params = { event = "prp-recycle:SellAnim", args = -2 } },
		{ header = "Sell Bottles", params = { event = "prp-recycle:SellAnim", args = 'bottle' } },
		{ header = "Sell Empty Cans", params = { event = "prp-recycle:SellAnim", args = 'can' } },
    })
end)
...
RegisterNetEvent('prp-recycle:Dumpsters:Remove')
AddEventHandler('prp-recycle:Dumpsters:Remove', function(object)
    for i = 1, #searched do
...
RegisterNetEvent('prp-recycle:Dumpsters:Search')
AddEventHandler('prp-recycle:Dumpsters:Search', function()
    local ped = PlayerPedId()
    local playerPed = PlayerPedId()
    --while true do
        local wait = 1000
                ...
                if dist < 1.8 then
                    ...
                    for i = 1, #searched do
                        ...
                        if i == #searched and dumpsterFound then
							ProjectRP.Functions.Notify('Already Searched.', 'error')
                        ...
                        elseif i == #searched and not dumpsterFound then
							...
							Skillbar.Start({
								...
								ProjectRP.Functions.Notify('You search the Trash Can!','error')
                                startSearching(searchTime, 'amb@prop_human_bum_bin@base', 'base', 'prp-recycle:Dumpsters:Reward')
                                table.insert(searched, dumpster)           
                                Citizen.Wait(1000)
							end, function()
								ProjectRP.Functions.Notify('You couldn\'t find anything.','error')
                                table.insert(searched, dumpster)           
                                ClearPedTasks(ped)
                                Citizen.Wait(1000)
							end)							
