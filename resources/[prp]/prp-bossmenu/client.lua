...
RegisterNetEvent('prp-bossmenu:client:openMenu')
AddEventHandler('prp-bossmenu:client:openMenu', function()
...
menu_button6:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("prp-bossmenu:server:withdrawMoney", tonumber(result))
        UpdateSociety()
    end
end)
...
menu_button7:On("select", function()
    ...
    if result ~= nil then
        TriggerServerEvent("prp-bossmenu:server:depositMoney", tonumber(result))
        UpdateSociety()
    end
end)
    ...
    for k,v in pairs(ProjectRP.Functions.GetPlayersFromCoords(GetEntityCoords(playerPed), 10.0)) do
        if v and v ~= PlayerId() then
            local menu_button10 = menu4:AddButton({
                ...
                select = function(btn)
                    local select = btn.Value
                    TriggerServerEvent('prp-bossmenu:server:giveJob', GetPlayerServerId(v))
                end
            })
        end
    end
            ...
            local menu_button9 = manageroptions:AddButton({
                ...
                select = function(btn)
                    ...
                    if values == 'promote' then
                        ...
                        if result ~= nil then
                            TriggerServerEvent('prp-bossmenu:server:updateGrade', employee.empSource, tonumber(result))
                        end
                    else
                        TriggerServerEvent('prp-bossmenu:server:fireEmployee', employee.empSource)
                    end
                end
            })
        end
