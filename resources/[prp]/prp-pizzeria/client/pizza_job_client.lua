...
AddEventHandler('prp-pizza:start:black:job', function()
	rnd = math.random(1,#DropOffs)
	CreateBlip()
	ProjectRP.Functions.Notify("Deliver this pizza at "..DropOffs[rnd]["info"], "success", 10000)
	if Tasks then
		return
	end
	Tasks = true
	while Tasks do
		...
		if Gebied <= 5.0 then
			ProjectRP.Functions.DrawText3D(DropOffs[rnd]["x"], DropOffs[rnd]["y"], DropOffs[rnd]["z"]+0.1, '~g~E~s~ - Deliver') 
...
function EndJob()
	ProjectRP.Functions.TriggerCallback('ProjectRP:HasItem', function(HasItem)
		if JobBusy == true and HasItem then
			...
			TriggerServerEvent('prp-pizza:server:set:pizza:count', 'Plus', 'register', math.random(100,200))    
		else
