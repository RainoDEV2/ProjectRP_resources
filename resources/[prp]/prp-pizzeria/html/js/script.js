$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('http://prp-pizzeria/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('http://prp-pizzeria/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('http://prp-pizzeria/CloseNui', JSON.stringify({}))
        ...
        $.post('http://prp-pizzeria/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('http://prp-pizzeria/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('http://prp-pizzeria/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
