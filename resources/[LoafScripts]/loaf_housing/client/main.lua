    ...
    for id, house in pairs(Houses) do
        ...
        if Config.Garage and cache.ownedHouses[tostring(id)] then
            ...
            if house.garage then
                if not shouldAddMarker and GetResourceState("prp-garages") == "started" then
                    if not house.garageMarker then
                        house.garageMarker = lib.AddMarker({
                            ...
                            CreateThread(function()
                                while house.inMarker do -- needed due to prp-houses setting closest house every so often..
                                    Wait(50)
                                    TriggerEvent("prp-garages:client:addHouseGarage", id, {
                                        takeVehicle = house.garage.exit,
                                        spawnPoint = house.garage.exit,
                                        label = "Property Garage"
                                    })
                                    TriggerEvent("prp-garages:client:setHouseGarage", id, true)
                                end
                            end)
                        end, nil, function()
