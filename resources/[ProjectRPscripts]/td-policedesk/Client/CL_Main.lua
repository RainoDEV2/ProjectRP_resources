                    ...
                    if dist < 1.5 then
                        if cooldown == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ProjectRP.Functions.TriggerCallback('tdpd:server:getCops', function(cops)
                                    if cops >= 1 then
                                        TriggerServerEvent('tdpd:server:requestPD', blipName)
                                        ProjectRP.Functions.Notify(TDPD.Config.SuccessMessage, 'success')
                                        cooldown = TDPD.Config.RequestCooldown * 1000
                                        Citizen.SetTimeout(TDPD.Config.RequestCooldown * 1000, function()
                                            cooldown = 0
                                        end)
                                    else
                                        ProjectRP.Functions.Notify(TDPD.Config.FailureMessage, 'error')
                                    end
                                end)
                            end
                        else
