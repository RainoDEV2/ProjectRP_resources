...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn and onRadio then
            ProjectRP.Functions.TriggerCallback('prp-radio:server:GetItem', function(hasItem)
                if not hasItem then
                    if RadioChannel ~= 0 then
                        leaveradio()
                    end
...
function connecttoradio(channel)
    ...
    if SplitStr(tostring(channel), ".")[2] ~= nil and SplitStr(tostring(channel), ".")[2] ~= "" then
        ProjectRP.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. ' MHz', 'success')
    else
        ProjectRP.Functions.Notify(Config.messages['joined_to_radio'] ..channel.. '.00 MHz', 'success')
    end
end
...
function leaveradio()
    ...
    ProjectRP.Functions.Notify(Config.messages['you_leave'] , 'error')
end
        ...
        if rchannel <= Config.MaxFrequency and rchannel ~= 0 then
            if rchannel ~= RadioChannel then
                if Config.RestrictedChannels[rchannel] ~= nil then
                    local xPlayer = ProjectRP.Functions.GetPlayerData()
                    if Config.RestrictedChannels[rchannel][xPlayer.job.name] and xPlayer.job.onduty then
                        connecttoradio(rchannel)
                    else
                        ProjectRP.Functions.Notify(Config.messages['restricted_channel_error'], 'error')
                    end
                else
                    connecttoradio(rchannel)
                end
            else
                ProjectRP.Functions.Notify(Config.messages['you_on_radio'] , 'error')
            end
        else
            ProjectRP.Functions.Notify(Config.messages['invalid_radio'] , 'error')
        end
    else
        ProjectRP.Functions.Notify(Config.messages['invalid_radio'] , 'error')
    end
...
RegisterNUICallback('leaveRadio', function(data, cb)
    if RadioChannel == 0 then
        ProjectRP.Functions.Notify(Config.messages['not_on_radio'], 'error')
    else
        leaveradio()
    end
end)
