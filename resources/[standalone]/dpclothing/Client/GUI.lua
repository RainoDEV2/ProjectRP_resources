...
local function DrawGUI()
		...
		if Button and not Cooldown then	-- If the button is clicked we execute the command, just like if the player typed it in chat.
			if v.Sprite == "gloves" then
				if not LastEquipped["Shirt"] then
					...
					Notify("You cannot do this without your shirt on.")
				end
			else
				PushedButton(k)  ExecuteCommand(v.Command)  
			end
	...
	if Cooldown then Text(x, y+0.05, 0.28, "Please wait...", false, false, true) end 		-- Cooldown indicator, if theres a cooldown we display a little text.
	...
	if InfoButton then 			
		...
		Notify("If the button is blue, you have a saved item.")
