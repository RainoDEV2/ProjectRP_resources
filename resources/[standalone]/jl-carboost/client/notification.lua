...
function AlertBoosting(netid, type)
    ...
    elseif type == "prp-dispatch" then
        TriggerServerEvent('cd_dispatch:AddNotification', {
            job_table = {'police'}, 
            coords = carPos,
            title = '10-50 - Car Boosting',
            message = ('[%s]'):format(vehInfo.plate).. " Car boosting in progress",
            flash = 0,
...
RegisterNetEvent('jl-carboost:notifypolice', function (car)
    local Player = ProjectRP.Functions.GetPlayerData()
    if Player.job.name == 'police' then
        ProjectRP.Functions.Notify(Lang:t('info.boosting'), 'info')
    end
end)
