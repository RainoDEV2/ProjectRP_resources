        ...
        if not veh.state.hacked then
            ...
            if success then
                ...
                if trackerLeft == 0 then
                    ...
                    veh.state.hacked = true
                    return ProjectRP.Functions.Notify(Lang:t("success.disable_tracker"))
                else
                    ProjectRP.Functions.Notify(Lang:t('success.tracker_off', {time = randomSeconds, tracker_left = trackerLeft}))
                end
                CreateThread(function ()
                ...
                ProjectRP.Functions.Notify(Lang:t('error.disable_fail'), "error")
            end
        else
            print("ALREADY HACKED")
        end
    else
        ProjectRP.Functions.Notify(Lang:t("error.no_tracker"), "error")
    end
end
...
local function BoostingAlert()
    if Config.Alert == 'prp-dispatch' then
        AlertBoosting(carID, 'prp-dispatch')
    ...
    if Config.BennysSell then
        for k, v in pairs(Config.BennysSell) do
            ...
            if not storeitem[k] then
                if ProjectRP.Shared.Items[v.item] ~= nil then
                    if Config.BennysSell[k].name then
                        name = Config.BennysSell[k].name
                    else
                        if ProjectRP.Shared.Items[v.item] ~= nil then
                            name = ProjectRP.Shared.Items[v.item].label
                        else
                            name = "Unknown"
                    ...
                    storeitem[#storeitem+1] = {
                        name = name,
                        item = v.item,
                        image = Config.Inventory ..ProjectRP.Shared.Items[v.item].image,
                        price = v.price,
                        stock = v.stock
                    }
                else
...
RegisterNUICallback('canStartContract', function (data, cb)
    ...
    if isContractStarted then
        ...
        if data.type == 'vin' then
            ProjectRP.Functions.TriggerCallback('jl-carboost:server:vinmoney', function(result)
...
RegisterNUICallback('startcontract', function (data)
    ...
    if not isContractStarted then
        ...
        Tier = data.data.tier
        ProjectRP.Functions.TriggerCallback('jl-carboost:server:getContractData', function (result)
            if result then
...
RegisterNUICallback('checkout', function (data)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:canBuy', function (result)
        if result then
...
RegisterNUICallback('setupboostapp', function (data, cb)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:getboostdata', function (result)
        if result then
...
RegisterNUICallback('sellcontract', function(data, cb)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:sellContract', function (result)
        if result then
            cb(result)
        else
            cb({
...
RegisterNUICallback('transfercontract', function (data, cb)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:transfercontract', function (result)
        if result and not result.error then
            cb({
                success = result
            })
...
RegisterNUICallback('buycontract', function(data, cb)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:buycontract', function(result)
        if result then
            cb(result)
        end
    end, data)
...
RegisterNetEvent('jl-carboost:client:openMenu', function ()
    if Config.BennysItems[1] then
        local menu = {
            {
                header = "| Benny's Collection Point |",
                isMenuHeader = true
            },
        }
        for _, v in pairs(Config.BennysItems) do
            ...
            menu[#menu+1] = {
                header = ProjectRP.Shared.Items[item.name]["label"],
                id = item.name,
        ...
        ProjectRP.Functions.Notify(Lang:t("error.empty_post"), "error")
    end
end)

RegisterNetEvent('jl-carboost:client:spawnCar', function(data)
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:spawnCar', function(result)
        if result then
            ...
            if carID ~= nil then
                if DoesEntityExist(NetworkGetEntityFromNetworkId(carID)) then
                    ...
                    if not modified then
                        ProjectRP.Functions.SetVehicleProperties(carSpawned, VehProp)
                        modified = true
                    end
    ...
    if vehicle ~= 0 then
        ...
        if #(PlayerPos - vehPos) <= 5.0 then
            ...
            if GetVehicleDoorLockStatus(vehicle) == 1 then
                    ProjectRP.Functions.Progressbar('check_vin', 'Checking VIN Number', 6000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                        ...
                        ProjectRP.Functions.TriggerCallback('jl-carboost:server:checkvin', function(result)
                            if result and result.success then
                                ProjectRP.Functions.Notify(result.message, "primary")
                            else
                                ProjectRP.Functions.Notify("Hmm you can't found the VIN", "error")
                            end
                        end, networkID)
                    ...
                    ProjectRP.Functions.Notify("The vehicle is locked", "error")
                end
            else
                ProjectRP.Functions.Notify("No vehicle nearby", "error")
            end
        end
end)
...
RegisterNetEvent('jl-carboost:client:vinscratch', function(veh)
    ...
    ProjectRP.Functions.Progressbar('vin_scratching', 'Scratching VIN', 7000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        ProjectRP.Functions.TriggerCallback('jl-carboost:server:checkvin', function(result)
            if result and result.owner == PlayerData.citizenid then
                if result.vinscratch == 1 then
                    return ProjectRP.Functions.Notify('You already scratch this vehicle VIN', 'error')
                end
            else
                ProjectRP.Functions.Notify('VIN Scratched, you can change your plate number', 'primary')
                local vehProp = ProjectRP.Functions.GetVehicleProperties(veh)
    ...
    if data.type == 'vin' then
        ...
        scratchpoint:onPointInOut(function ()
            ...
            if inscratchPoint then
                ProjectRP.Functions.Notify(Lang:t("info.in_scratch"))
            else
                ProjectRP.Functions.Notify(Lang:t("info.not_in_scratch"))
            end
        end)
    else
        ...
        zone:onPointInOut(function ()
            ...
            if inZone then
                ProjectRP.Functions.Notify(Lang:t("info.car_inzone"))
            end
        end)
        CreateThread(function ()
            while true do
...
RegisterNetEvent('jl-carboost:client:failedBoosting', function ()
    ...
    isContractStarted = false
    ProjectRP.Functions.Notify(Lang:t("error.no_car"), "error")
    TriggerEvent('jl-carboost:client:refreshQueue')
    TriggerEvent('jl-carboost:client:deleteContract')
end)
...
RegisterNetEvent('jl-carboost:client:refreshContract', function ()
    ProjectRP.Functions.TriggerCallback('jl-carboost:server:getContractData')
    SendNUIMessage({
...
RegisterNetEvent('jl-carboost:client:useHackingDevice', function ()
    ...
    if IsPedInVehicle(playerPed, vehicle) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), 0) ~= 0 then
            if cooldown then
                return ProjectRP.Functions.Notify(Lang:t("error.cannot_use"), "error")
            end
            ...
            return ProjectRP.Functions.Notify(Lang:t('error.not_seat'), "error")
        end
    else
        ProjectRP.Functions.Notify(Lang:t("error.not_on_vehicle"), "error")
    end
end)

RegisterNetEvent('jl-carboost:client:fakeplate', function()
    local veh = ProjectRP.Functions.GetClosestVehicle()
            ...
            ProjectRP.Functions.TriggerCallback('jl-carboost:server:checkvin', function (result)
                if result then
                    if result.owner == PlayerData.citizenid then
                        ProjectRP.Functions.Progressbar('change_plate', 'Changing the plate', 8000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                        ...
                        ProjectRP.Functions.Notify(Lang:t("error.not_owner"), "error")
                    end
                end
            end, vehID)
        else
            ProjectRP.Functions.Notify(Lang:t("error.not_plate"), "error")
        end
    end
end)

function RandomPlate()
	local random = tostring(ProjectRP.Shared.RandomInt(1) .. ProjectRP.Shared.RandomStr(2) .. ProjectRP.Shared.RandomInt(3) .. ProjectRP.Shared.RandomStr(2)):upper()
    return random
end
...
RegisterNetEvent('jl-carboost:client:setPlate', function (vehID)
    ...
    if veh and veh ~= 0 then
        ...
        ProjectRP.Functions.Notify(Lang:t("success.plate_changed", {
            plate = newPlate
        }), "success")
        TriggerEvent('inventory:client:ItemBox', ProjectRP.Shared.Items["fake_plate"], "remove")
        TriggerServerEvent("ProjectRP:Server:RemoveItem", "fake_plate", 1)
...
CreateThread(function ()
    ...
    CreateBlip(vector3(1182.59, -3310.36, 6.25), "Benny's Collection Point", 473)
end)
...
-- exports
exports['prp-target']:AddBoxZone("carboost:takeItem", vector3(1182.59, -3310.36, 6.25), 2, 2, {
...
exports['prp-target']:AddTargetBone('windscreen', {
    options = {
        {
            icon = "fas fa-tally",
            label = "Scratch VIN",
