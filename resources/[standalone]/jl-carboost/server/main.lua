...
RegisterNetEvent('jl-carboost:server:takeItem', function (name, quantity)
   ...
   TriggerClientEvent('inventory:client:ItemBox', src, ProjectRP.Shared.Items[name], "add")
end)
...
RegisterNetEvent('jl-carboost:server:finishBoosting', function (type, tier)
   ...
   if type ~= 'vin' then
      ...
      if Config.Payment == 'crypto' then
         TriggerClientEvent('ProjectRP:Notify', src, Lang:t('info.payment_crypto', {
            amount = amountMoney
         }), 'success')
      end
   end
   ...
   TriggerClientEvent('ProjectRP:Notify', src, Lang:t('info.get_rep', {
      rep = randomRep
   }), "primary")   
end)
...
RegisterNetEvent('jl-carboost:server:saveConfig', function (data)
   ...
   local pData = ProjectRP.Functions.GetPlayer(source)
...
ProjectRP.Commands.Add('settier', 'Set Boosting Tier', {
   ...
   if not args[1] or not type(args[1]) == "string" or Config.Tier[tostring(args[1])] == nil then
      return TriggerClientEvent('ProjectRP:Notify', src, Lang:t('error.invalid_tier'), "error")
   end
   local PlayerID = tonumber(args[2])
   local player
   if PlayerID then
      player = ProjectRP.Functions.GetPlayer(PlayerID)
      if not player or player == nil then
         return TriggerClientEvent('ProjectRP:Notify', src, Lang:t('error.invalid_player'), "error")
      end
   else
      player = ProjectRP.Functions.GetPlayer(src)
   end
   player.Functions.SetMetaData('carboostclass', tostring(args[1]))
   TriggerClientEvent('ProjectRP:Notify', src, Lang:t('success.set_tier', {
      tier = args[1]
   }), "success")
end, 'admin')
...
ProjectRP.Commands.Add('giveContract', 'Give contract, admin only', {
   ...
   if Config.Tier[tostring(args[1])] ~= nil then
      ...
      MySQL.Async.insert('INSERT INTO boost_contract (owner, data, started, expire) VALUES (@owner, @data, NOW(),DATE_ADD(NOW(), INTERVAL @expire HOUR))', {
         ...
         TriggerClientEvent('ProjectRP:Notify', src, Lang:t('success.contract_give', {
...
RegisterNetEvent('jl-carboost:server:takeAll', function (data)
   ...
   for _, v in pairs(data) do
      ...
      print(item.name)
      TriggerClientEvent('inventory:client:itemBox', src, ProjectRP.Shared.Items[item.name], 'add')
   end
   MySQL.Async.execute('UPDATE bennys_shop SET items = @items WHERE citizenid = @citizenid', {
      ['@citizenid'] = Player.PlayerData.citizenid,
      ['@items'] = json.encode({})
   })
   TriggerClientEvent('ProjectRP:Notify', src, Lang:t('success.take_all'), "success")
end)
...
-- Callback
ProjectRP.Functions.CreateCallback('jl-carboost:server:canBuy', function(source, cb, data)
   local src = source
   local pData = ProjectRP.Functions.GetPlayer(src)
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:sellContract', function (source, cb, data)
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:buycontract', function(source, cb, data)
   local src = source
   local Player = ProjectRP.Functions.GetPlayer(src)
   local toPlayer
   ...
   if result[1] then
      ...
      toPlayer = ProjectRP.Functions.GetPlayerByCitizenId(result[1].owner)
      ...
      TriggerClientEvent('ProjectRP:Notify', toPlayer.PlayerData.source, Lang:t('info.contract_buyed', {amount = moneyAmount}), "success")
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:transfercontract', function (source, cb, data)
   local src = source
   local id = tonumber(data.id) 
   local contractid = tonumber(data.contractid)
   local toPlayer = ProjectRP.Functions.GetPlayer(id)
   local Player = ProjectRP.Functions.GetPlayer(src)
   if src == id then
      return cb({
   ...
   if result[1] then
      MySQL.Async.execute('UPDATE boost_contract SET owner = @owner WHERE owner = @citizenid', {
         ...
         TriggerClientEvent('ProjectRP:Notify', src, Lang:t('success.contract_transfer', {
            player = toPlayer.PlayerData.name
         }), "success")
         TriggerClientEvent('ProjectRP:Notify', id, Lang:t('info.receive_transfer', {
            player = Player.PlayerData.name,
         }), "primary")
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:vinmoney', function (source, cb, data)
   local src = source
   local Player = ProjectRP.Functions.GetPlayer(src)
   local money = Player.PlayerData.money[Config.Payment]
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:getboostdata', function (source, cb, citizenid)
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:getContractData', function (source, cb, data)
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:spawnCar', function (source, cb, data)
   local src = source
   local cardata = data
...
ProjectRP.Functions.CreateCallback('jl-carboost:server:checkvin', function (source, cb, data)
   local src = source
...
function Queue()
   if queueNumber ~= 0 then
      ...
      for k, v in pairs(Config.QueueList) do
         local Player = ProjectRP.Functions.GetPlayerByCitizenId(k)
         if not v.getContract and v.status and Player and num <= Config.MaxQueueContract then
            TriggerClientEvent('ProjectRP:Notify', Player.PlayerData.source, Lang:t('info.new_contract'), "primary")
...
function RandomVIN()
   local random = tostring(ProjectRP.Shared.RandomInt(3) .. ProjectRP.Shared.RandomStr(3) .. ProjectRP.Shared.RandomInt(2)):upper()
   return random
end
...
ProjectRP.Functions.CreateUseableItem('laptop' , function(source, item)
   TriggerClientEvent('jl-carboost:client:openLaptop', source)
end)

ProjectRP.Functions.CreateUseableItem('hacking_device',  function (source, item)
   TriggerClientEvent('jl-carboost:client:useHackingDevice', source)
end)

ProjectRP.Functions.CreateUseableItem('fake_plate', function(source, item)
