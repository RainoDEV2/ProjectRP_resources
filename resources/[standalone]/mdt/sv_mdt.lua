...
AddEventHandler("mdt:hotKeyOpen", function()
    ...
    local xPlayer = ProjectRP.Functions.GetPlayer(usource)
	...
	exports.oxmysql:fetch("SELECT * FROM `players` WHERE `charinfo` LIKE ?", {string.lower('%'..query..'%')}, function(result) -- % wildcard, needed to search for all alike results
		...
		for index, data in ipairs(result) do
			if data.charinfo then
				...
				local core = ProjectRP.Functions.GetPlayerByCitizenId(data.citizenid)
...
function GetLicenses(identifier, cb)
	local player = ProjectRP.Functions.GetPlayerByCitizenId(identifier)
	if player ~= nil then
		...
		for type,_ in pairs(playerlicenses) do
			if playerlicenses[type] then
				...
				elseif type == "hunting" then
					licenseType = "hunting_license" label = "Hunting License"	
				elseif type == "fishing" then
					licenseType = "fishing_license" label = "Fishing License"		
				end
