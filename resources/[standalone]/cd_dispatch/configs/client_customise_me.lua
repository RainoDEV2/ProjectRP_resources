...
Citizen.CreateThread(function()
    ...
    elseif Config.Framework == 'qbcore' then
        while ProjectRP == nil do
            TriggerEvent(Config.FrameworkTriggers.main, function(obj) ProjectRP = obj end)
            if ProjectRP == nil then
                ProjectRP = exports[Config.FrameworkTriggers.resource_name]:GetCoreObject()
            end
            Citizen.Wait(100)
        end
        JobData = ProjectRP.Functions.GetPlayerData().job or {}

        RegisterNetEvent(Config.FrameworkTriggers.load)
        AddEventHandler(Config.FrameworkTriggers.load, function()
            JobData = ProjectRP.Functions.GetPlayerData().job or {}
            Onduty_local = ProjectRP.Functions.GetPlayerData().job.onduty
            while not Authorised do Citizen.Wait(1000) end
            TriggerServerEvent('cd_dispatch:PlayerLoaded')
        end)
        ...
        AddEventHandler('ProjectRP:Client:SetDuty', function(boolean)
            Onduty_local = boolean
...
function IsAllowed()
    ...
    if CheckMultiJobs(job) then
        if Config.DutyLocalsMethod then
            return Onduty_local
        else
            return true
        end
    ...
    if notif_type and message then
        ...
        elseif Config.NotificationType.client == 'qbcore' then
            if notif_type == 1 then
                ProjectRP.Functions.Notify(message, 'success')
            elseif notif_type == 2 then
                ProjectRP.Functions.Notify(message, 'primary')
            elseif notif_type == 3 then
                ProjectRP.Functions.Notify(message, 'error')
            end
