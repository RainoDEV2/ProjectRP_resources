...
Citizen.CreateThread(
    function()
        ...
        while ProjectRP.Functions.GetPlayerData().job == nil do
            Citizen.Wait(1000)
        end

        job = ProjectRP.Functions.GetPlayerData().job.name
        ...
        ProjectRP.Functions.TriggerCallback(
...
AddEventHandler("ProjectRP:Client:OnJobUpdate", function(j)
    job = j.name
end)
...
function addBlipsForUnits()
    ProjectRP.Functions.TriggerCallback("core_dispach:getUnits", function(units)
...
function openDispach()
    if Config.JobOne.job == job or Config.JobTwo.job == job or Config.JobThree.job == job then
        ...
        ProjectRP.Functions.TriggerCallback( "core_dispach:getInfo", function(units, calls, ustatus)
