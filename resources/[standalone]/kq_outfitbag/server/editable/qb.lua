if Config.qbSettings.enabled then
    ...
    ProjectRP.Functions.CreateUseableItem(Config.bagItem, function(source)
        TriggerClientEvent('kq_outfitbag:placeBag', source, Config.bagItem)
    end)

    for k, item in pairs(Config.additionalItems) do
        ProjectRP.Functions.CreateUseableItem(item, function(source)
    ...
    function RemoveBagItem(player, item)
        ...
        local xPlayer = ProjectRP.Functions.GetPlayer(tonumber(player))
